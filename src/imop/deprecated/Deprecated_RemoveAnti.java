...
public class Deprecated_RemoveAnti {
	...
	public static void removeAntiDependence(ParallelConstruct parCons) {
		//		outerMost: for (Node node : parCons.getInfo().getCFGInfo().getLexicalIntraProceduralCFGContents()) {
		//			if (!Misc.isCFGLeafNode(node) || node instanceof BeginNode || node instanceof EndNode) {
		...
		//					if (Misc.setIntersection(new Set<Object>(node.getInfo().getNodePhaseInfo().getPhaseSetCopy()),
		//							new Set<Object>(def.getInfo().getNodePhaseInfo().getPhaseSetCopy())).size() != 0) {
		//						deleteList.add(def);
		//					}
		//				}
		...
		//				Symbol newSymbol = new Symbol(newSymbolName, sym.getType(), newSymbolDecl, Main.root);
		...
		//					// been renamed.
		//					Set<Node> notRenamedDefSet = Misc.setMinus(use.getInfo().getFlowEdgeSrcList().get(sym),
		//							defList);
		...
		//						Statement replacementStmt = FrontEnd.parseAlone("{" + unrenamedDef.getInfo().getString() + " "
