/*
 * Copyright (c) 2019 Aman Nougrahiya, V Krishna Nandivada, IIT Madras.
 * This file is a part of the project IMOP, licensed under the MIT license.
 * See LICENSE.md for the full text of the license.
 * 
 * The above notice shall be included in all copies or substantial
 * portions of this file.
 */
//
// Generated by JTB 1.3.2
//

package imop.ast.node.external;

import imop.ast.info.cfgNodeInfo.OmpForConditionInfo;

/**
 * Grammar production:
 * f0 ::= OmpForLTCondition()
 * | OmpForLECondition()
 * | OmpForGTCondition()
 * | OmpForGECondition()
 */
public class OmpForCondition extends Node {
	{
		classId = 349;
	}

	public OmpForCondition() {
	}

	/**
	 * 
	 */
	private static final long serialVersionUID = -7815266110738952903L;
	private NodeChoice f0;

	public OmpForCondition(NodeChoice n0) {
		n0.setParent(this);
		setF0(n0);
	}

	@Override
	public OmpForConditionInfo getInfo() {
		if (info == null) {
			info = new OmpForConditionInfo(this);
		}
		return (OmpForConditionInfo) info;
	}

	@Override
	public boolean isKnownCFGNode() {
		return true;
	}

	@Override
	public boolean isKnownCFGLeafNode() {
		return true;
	}

	@Override
	public void accept(imop.baseVisitor.Visitor v) {
		v.visit(this);
	}

	@Override
	public <R, A> R accept(imop.baseVisitor.GJVisitor<R, A> v, A argu) {
		return v.visit(this, argu);
	}

	@Override
	public <R> R accept(imop.baseVisitor.GJNoArguVisitor<R> v) {
		return v.visit(this);
	}

	@Override
	public <A> void accept(imop.baseVisitor.GJVoidVisitor<A> v, A argu) {
		v.visit(this, argu);
	}

	public NodeChoice getF0() {
		return f0;
	}

	public void setF0(NodeChoice f0) {
		f0.setParent(this);
		this.f0 = f0;
	}
}
