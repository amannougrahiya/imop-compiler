...
public class NodeInfo implements Cloneable {
    ...
    /**
     * Saves hashCode of the original copy of the related node. This field is used to find the duplicates.
     */
    ...
    //	private List<InterTaskEdge> interTaskEdgeList; // Not needed, since we will be creating such edges only between dummy-flush directives.
     ...
     * <br>
     * Note that this field is used only for CFG leaf nodes. Once we add {@link Predicate} to appropriate places in the
     * production rules, we can create appropriate interfaces for CFG leaf and non-leaf nodes, and add this field in the
     * corresponding CFG nodes.
     */
    private CellList readList;

    /**
     * Set of all the cells that may be written in the execution of this node (and the called methods, if any).
     * <br>
     * Note that this field is used only for CFG leaf nodes. Once we add {@link Predicate} to appropriate places in the
     * production rules, we can create appropriate interfaces for CFG leaf and non-leaf nodes, and add this field in the
     * corresponding CFG nodes.
    ...
    /**
     * Set of all the shared cells that may be read in the execution of this node (and the called methods, if any).
     * <br>
     * Note that this field is used only for CFG leaf nodes. Once we add {@link Predicate} to appropriate places in the
     * production rules, we can create appropriate interfaces for CFG leaf and non-leaf nodes, and add this field in the
     * corresponding CFG nodes.
    ...
    /**
     * Set of all the shared cells that may be written in the execution of this node (and the called methods, if any).
     * <br>
     * Note that this field is used only for CFG leaf nodes. Once we add {@link Predicate} to appropriate places in the
     * production rules, we can create appropriate interfaces for CFG leaf and non-leaf nodes, and add this field in the
     * corresponding CFG nodes.
    ...
    /**
     * Information about destinations of various nodes that can read a write happening at this node.
     */
    ...
    /**
     * List of access-expressions (composed up of {@link BaseSyntax} and List{@literal <}{@link Tokenizable}{@literal
    ...
    /**
     * List of access-expressions (composed up of {@link BaseSyntax} and List{@literal <}{@link Tokenizable}{@literal
     * >}) representing array dereferences that may be written anywhere in this node.
     */
    private List<SyntacticAccessExpression> baseAccessExpressionWrites;

    /**
     * List of all the {@link CastExpression} expressions on which dereference may have been made using an asterisk
     * ({@code *}), and whose dereferenced location may have been read within this node.
     */
    private List<CastExpression> pointerDereferencedExpressionReads;
    /**
     * List of all the {@link CastExpression} expressions on which dereference may have been made using an asterisk
     * ({@code *}), and whose dereferenced location may have been written within this node.
     */
    ...
    private void populateForward() {
        ...
        for (NodeWithStack succ : this.getNode().getInfo().getCFGInfo().getInterProceduralLeafSuccessors(new CallStack())) {
    ...
    private void populateBackward() {
        Set<NodeWithStack> endPoints = new HashSet<>();
        Set<NodeWithStack> reachablesWithStack = new HashSet<>();
        for (NodeWithStack pred : this.getNode().getInfo().getCFGInfo().getInterProceduralLeafPredecessors(new CallStack())) {
            Set<NodeWithStack> someEndPoints = new HashSet<>();
            reachablesWithStack.addAll(CollectorVisitor.collectNodesIntraTaskBackwardBarrierFreePath(pred, someEndPoints));
    ...
    /**
     * This method ensures that the points-to flow facts for all nodes have been stabilized, if required. Note that
     * memoization of all access lists is disabled when running the stabilization of points-to.
     * <br>
     * IMPORTANT: This method must always be called before attempting to read any (of the six) memoized access list.
     *
    ...
    /**
     * Checks whether the points-to flow-facts might be stale, without resetting the invalid flag. When this flag is
    ...
    /**
     * For some special cases, if the analysis has not yet been run even once, this method runs the analysis.
     *
     * @param analysisName
    ...
    /**
     * Obtain the IN flow-fact for this node, for {@code analysisName}. <br> Note that this function may return {@code
     ...
     *
     * @param analysisName
     *         name (ID) of the analysis for which the IN flow-fact of this node has to be obtained.
     *
     * @return
     */
    ...
    /**
     * Get the current IN object for give data-flow analysis, without running stabilization pass.
     *
     * @param analysisName
    ...
    /**
     * Get the current OUT object for give data-flow analysis, without running stabilization pass.
     *
     * @param analysisName
    ...
    /**
     * Sets {@code flowFact} as the IN flow-fact for {@code analysisName} for this node.
     *
     * @param analysisName
     *         name (ID) of the analysis for which the OUT flow-fact of this node has to be obtained.
     * @param flowFact
    ...
    /**
     * Obtain the OUT flow-fact for this node, for {@code analysisName}. <br> Note that this function may return {@code
     * null}, which implies the following:
     * <ul>
    ...
    /**
     * Sets {@code flowFact} as the OUT flow-fact for {@code analysisName} for this node.
     *
     * @param analysisName
     *         name (ID) of the analysis for which the OUT flow-fact of this node has to be obtained.
     * @param flowFact
    ...
    /**
     * For a given symbol, this method tells whether the corresponding block of storage is shared or private at this
     * node.
     *
     * @param name
     *
    ...
    public DataSharingAttribute getSharingAttribute(Cell cell) {
        ...
        while (encloser != null) {
            if (encloser instanceof CompoundStatement) {
                ...
                //				if (sym.getName() != null && (sym.getName().equals("r") || sym.getName().equals("k"))) {
                ...
                if (symbolMap.containsKey(sym.getName())) {
                    ...
                    //							"Couldn't find an enclosing scope for the symbol " + sym.getName() + ".", Program.root);
                    return DataSharingAttribute.SHARED;
                }
                ...
                if (encloser instanceof ForConstruct) {
                    ...
                    assert (false);
                    //					String loopIteratorName = ((ParallelForConstruct) encloser).getF5().getF2().getF0().tokenImage;
                    ...
                    for (OmpClause clause : ompClauseList) {
                        if (clause instanceof OmpSharedClause) {
                            ...
                            //							Symbol symVar = Misc.getSymbolEntry(varList.getF0().getTokenImage(), encloser);
                            ...
                            for (Node nodeChoice : varList.getF1().getNodes()) {
                                ...
                                cellVar = Misc.getSymbolOrFreeEntry(((NodeToken) ((NodeSequence) nodeChoice).getNodes().get(1)));
                                if (!(cellVar instanceof Symbol)) {
                                    return DataSharingAttribute.SHARED;
                                } else {
                                    Symbol symVar = (Symbol) cellVar;
    ...
    /**
     * Returns a set of all the cells that may be read in the execution of this node (and the called methods, if any),
     * excluding the nodes that are encountered after the control leaves this node from any exit-point.
     *
     * @return set of all those cells that may be read between the entry to and exit from this node.
     */
    ...
    /**
     * Returns a set of all the cells that may be written in the execution of this node (and the called methods, if
     * any). Note that this method also considers those accesses that may happen in nodes that are not lexically inside
     * the owner node, and those that may be reachable via some jumps (not just calls).
     *
     * <br>
     * Note that this field is used only for CFG leaf nodes. Once we add {@link Predicate} to appropriate places in the
     * production rules, we can create appropriate interfaces for CFG leaf and non-leaf nodes, and add this field in the
     * corresponding CFG nodes.
     *
     * @return set of all those cells that may be written in the execution of this node.
     */
    ...
    /**
     * Checks whether this node (or its corresponding CFG node) may write to a cell of pointer-type.
     *
     * @return true, if this node (or its corresponding CFG node) may write to a cell of pointer-type.
     */
    ...
    /**
     * Returns a set of all the shared cells that may be read in the execution of this node (and the called methods, if
     * any). Note that this method also considers those accesses that may happen in nodes that are not lexically inside
     * the owner node, and those that may be reachable via some jumps (not just calls).
     * <br>
     * Note that this field is used only for CFG leaf nodes. Once we add {@link Predicate} to appropriate places in the
     ...
     * corresponding CFG nodes.
     *
     * @return set of all those shared cells that may be read in the execution of this node.
     */
    ...
    /**
     * Returns a set of all the shared cells that may be written in the execution of this node (and the called methods,
     * if any). Note that this method also considers those accesses that may happen in nodes that are not lexically
     * inside the owner node, and those that may be reachable via some jumps (not just calls).
     * <br>
     * Note that this field is used only for CFG leaf nodes. Once we add {@link Predicate} to appropriate places in the
     * production rules, we can create appropriate interfaces for CFG leaf and non-leaf nodes, and add this field in the
     * corresponding CFG nodes.
     *
     * @return set of all those shared cells that may be written in the execution of this node.
     */
    ...
    /**
     * Returns a set of all the symbols that may be read in the execution of this node (and the called methods, if any),
     * excluding the nodes that are encountered after the control leaves this node from any exit-point.
     *
     * @return set of all those symbols that may be read between the entry to and exit from this node.
     */
    ...
    /**
     * Returns a set of all the symbols that may be written in the execution of this node (and the called methods, if
     * any), excluding the nodes that are encountered after the control leaves this node from any exit-point.
     *
     * @return set of all those symbols that may be written between the entry to and exit from this node.
     */
    ...
    /**
     * Returns a set of all the shared cells that may be read in the execution of this node (and the called methods, if
     * any), excluding all the field cells. Note that this method also considers those accesses that may happen in nodes
     * that are not lexically inside the owner node, and those that may be reachable via some jumps (not just calls).
     * <br>
     * Note that this field is used only for CFG leaf nodes. Once we add {@link Predicate} to appropriate places in the
     * production rules, we can create appropriate interfaces for CFG leaf and non-leaf nodes, and add this field in the
     * corresponding CFG nodes.
     *
     * @return set of all those shared cells that may be read in the execution of this node, excluding all the field
     * cells.
     */
    public CellSet getNonFieldSharedReads() {
        if (Misc.isCFGLeafNode(this.getNode())) {
            ...
            if (update) {
                ...
                } else if (!reads.getReadOnlyInternal().stream().anyMatch(c -> c instanceof FieldCell ||
                        c instanceof HeapCell)) {
                    this.nonFieldReadSet = reads;
                } else {
    ...
    /**
     * Returns a set of all the shared cells that may be written in the execution of this node (and the called methods,
     * if any), excluding all the field cells. Note that this method also considers those accesses that may happen in
     * nodes that are not lexically inside the owner node, and those that may be reachable via some jumps (not just
     * calls).
     * <br>
     * Note that this field is used only for CFG leaf nodes. Once we add {@link Predicate} to appropriate places in the
     * production rules, we can create appropriate interfaces for CFG leaf and non-leaf nodes, and add this field in the
     * corresponding CFG nodes.
     *
     * @return set of all those shared cells that may be written in the execution of this node, excluding all the field
     * cells.
     */
    public CellSet getNonFieldSharedWrites() {
        if (Misc.isCFGLeafNode(this.getNode())) {
            ...
            if (update) {
                ...
                } else if (!writes.getReadOnlyInternal().stream().anyMatch(c -> c instanceof FieldCell ||
                        c instanceof HeapCell)) {
                    this.nonFieldWriteSet = writes;
                } else {
    ...
    /**
     * Obtain the list of access-expressions (composed up of {@link BaseSyntax} and List{@literal <}{@link
     * Tokenizable}{@literal >}) representing array dereferences that may be read anywhere in this node.
     *
     * @return list of access-expressions (composed up of {@link BaseSyntax} and List{@literal <}{@link
     * Tokenizable}{@literal >}) representing array dereferences that may be read anywhere in this node.
     */
    ...
    /**
     * Obtain the list of access-expressions (composed up of {@link BaseSyntax} and List{@literal <}{@link
     * Tokenizable}{@literal >}) representing array dereferences that may be written anywhere in this node.
     *
     * @return list of access-expressions (composed up of {@link BaseSyntax} and List{@literal <}{@link
     * Tokenizable}{@literal >}) representing array dereferences that may be written anywhere in this node.
     */
    ...
    /**
     * Obtain a list of all the {@link CastExpression} expressions on which dereference may have been made using an
     * asterisk ({@code *}), and whose dereferenced location may have been read within this node.
     * <br>
     * For example, in {@code ... = *(e1);}, this method returns a set containing {@code e1}.
     *
     * @return list of all the {@link CastExpression} expressions on which dereference may have been made using an
     * asterisk ({@code *}), and whose dereferenced location may have been read within this node.
     */
    public List<CastExpression> getPointerDereferencedExpressionReads() {
        ...
        if (node instanceof Expression || Misc.isCFGLeafNode(node)) {
            if (pointerDereferencedExpressionReads == null) {
                pointerDereferencedExpressionReads = PointerDereferenceGetter.getPointerDereferencedCastExpressionReads(node);
            }
    ...
    /**
     * Obtain a list of all the {@link CastExpression} expressions on which dereference may have been made using an
     * asterisk ({@code *}), and whose dereferenced location may have been written within this node.
     * <br>
     * For example, in {@code *(e1) = ...;}, this method returns a set containing {@code e1}.
     *
     * @return list of all the {@link CastExpression} expressions on which dereference may have been made using an
    ...
    public List<CastExpression> getPointerDereferencedExpressionWrites() {
        ...
        if (node instanceof Expression || Misc.isCFGLeafNode(node)) {
            if (pointerDereferencedExpressionWrites == null) {
                pointerDereferencedExpressionWrites = PointerDereferenceGetter.getPointerDereferencedCastExpressionWrites(node);
            }
    ...
    /**
     * Replaces all the occurrences of {@link Symbol} having names in {@code freeNames} by {@link FreeVariable}, for the
     * {@link #readList} and {@link #writeList} of the owner leaf node.
     *
     * @param freeNames
     *         list of identifiers that refer to free variables in the owner leaf node.
     */
    public void makeSymbolsFreeInRWList(Set<String> freeNames) {
        ...
        if (Misc.isCFGLeafNode(node)) {
            ...
            // Old code: Commented out after changing the type of sharedReadSet and sharedWriteSet.
            ...
            Misc.warnDueToLackOfFeature("Handling free variables in the read/write lists for non-leaf/non-CFG nodes.", this.getNode());
            return;
        }
    }
    ...
    /**
     * Checks if this node is a self-update node -- one that may read and write to the same location.
     *
     * @return true, if this CFG node is a node that reads as well as writes to the same cell.
     */
    ...
    /**
     * Returns a set of all those cells that may be accessible at the given node, exclusively -- i.e., if this node is a
     * {@link Scopeable} object, then we do not include the set of those cells that are created within this scope.
     *
     * @return a set of all those cells that may be accessible at the given node, exclusively.
     */
    ...
    /**
     * Returns the set of symbol names that are accessible at this node, exclusively -- i.e., if this node is a {@link
     * Scopeable} object, then the names from the symbol table of this object are not added to the return set.
     *
    ...
    /**
     * Returns the set of all those function-definitions (including the current node itself, if it's a
     * function-definition), that may get executed during the processing of this node.
     *
     * @return set of all those function-definitions that may get executed during the execution of this node (including
    ...
    public Set<FunctionDefinition> getReachableCallGraphNodes() {
        ...
        returnSet.addAll(CollectorVisitor.collectNodeSetInGenericGraph(startSet, endSet, (f) -> false, (f) -> f.getInfo().getCallGraphSuccessors()));
    ...
    /**
     * Obtain a set of all those function-definitions that may be the target of any call-statement that is lexically
     * enclosed within the owner node.
     *
     * @return set of all those function-definitions that may be the target of any call-statement that is lexically
     * enclosed within the owner node.
     */
    ...
    /**
     * Returns the outermost enclosing non-leaf node for the owner node (exclusively).
     *
    ...
    /**
     * Obtain a set of all the non-leaf nodes that lexically enclose the node {@code node}.
     *
     * @param node
     *         node of whose all non-leaf enclosers have to be found.
     *
     * @return a set of all those non-leaf nodes that lexically enclose the node {@code node}
     */
    public Set<Node> getAllLexicalNonLeafEnclosersExclusive() {
        Set<Node> allEnclosers = CollectorVisitor.collectNodeSetInGenericGraph(this.node, null, (n) -> (n ==
                null), (n) -> {
    ...
    /**
     * Returns the list of all the non-leaf nodes that enclose the owner node, starting with the immediately enclosing
     * non-leaf encloser (exclusively). Note that root of the tree would be the last element.
     *
     * @return the list of all the non-leaf nodes that enclose {@code node}, starting with the immediately enclosing
    ...
    public List<Node> getNonLeafNestingPathExclusive() {
        ...
        nonLeafEnclosingPathExclusive = CollectorVisitor.collectNodeListInGenericGraph(this.node, null, (n) -> (n ==
                null), (n) -> {
            List<Node> neighbourSet = new LinkedList<>();
    ...
    /**
     * Get a set of all those non-leaf nodes that may enclose the given statement, including those that may reside in
     * the callers of the function in which the given node exists.
     *
     * @return a set of all those non-leaf nodes that may enclose the given statement, including those that may reside
    ...
    /**
     * Find whether the control can flow in and out of this AST node except from the beginning and end of it,
     * respectively.
     *
     * @return
     */
    public boolean isControlConfined() {
        ...
        for (Object labeledObject : cfgContents.stream().filter(n -> (n instanceof Statement &&
                ((Statement) n).getInfo().hasLabelAnnotations())).toArray()) {
    ...
    /**
     * Returns a set of those nodes which are sources of (intra-procedural) edges that enter this node from somewhere
     * apart from BeginNode.
     *
     * @return a set of those nodes which are sources of (intra-procedural) edges that enter this node from somewhere
     * apart from BeginNode.
     */
    ...
    /**
     * Returns a set of those nodes which are destinations of (intra-procedural) edges that enter this node from
     * somewhere apart from BeginNode.
     *
     * @return a set of those nodes which are destinations of (intra-procedural) edges that enter this node from
     * somewhere apart from BeginNode.
     */
    ...
    /**
     * Returns a set of those nodes which are destinations of (intra-procedural) edges that leave this node from
     * somewhere apart from EndNode.
     *
     * @return a set of those nodes which are destinations of (intra-procedural) edges that leave this node from
     * somewhere apart from EndNode.
     */
    ...
    /**
     * Removes the unnecessary scoping of statements within this node, without resorting to renaming of the nested
     * variables.
     * <p>
    ...
    /**
     * Obtain an internal CFG node in owner {@code node}, that contains the label {@code l}.
     *
     * @param l
    ...
    /**
     * Obtain the set of nodes which may represent definitions that are reachable at the owner node, for cell {@code
     * cell}.
     *
     * @param cell
     *         whose reaching definition has to be found .
     *
     * @return set of nodes which may represent definitions that are reachable at the owner node, for cell {@code cell}.
     */
    ...
    /**
     * Obtain the set of nodes which may represent definitions that are reachable at the owner node.
     *
     * @return set of nodes which may represent definitions that are reachable at the owner node.
     */
    public Set<Definition> getReachingDefinitions() {
        ...
        // Old code: This method depends upon data-dependence forward approach. (Too slow, though.)
    ...
    public Set<Node> getDominators() {
        ...
        //		assert (retSet.contains(this.getNode())) : "Could not find " + this.getNode().toString() + ", a "
    ...
    /**
     * Obtain the set of nodes which may represent definitions that may be reachable to instances of the owner node in
     * given phase {@code ph}.
     *
     * @param ph
     *         phase in which the owner node exists, and for which the reaching-definitions have to be found.
     * @param phaseInsensitiveReachingDefinitions
     *         set of nodes that are reaching-definitions of this node across all phases.
     *
     * @return
     */
    public Set<Node> getPhaseSensitiveReachingDefinitions(Phase ph, Set<Node> phaseInsensitiveReachingDefinitions) {
        ...
        //			Node startingNode = bpp.getNode(); // this can be a BeginNode or a BarrierDirective.
    ...
    public Set<Node> getFlowDestinations() {
        ...
        // Old Code:
        //		DataDependenceBackwardFF ff = (DataDependenceBackwardFF) this.getOUT(AnalysisName.DATA_DEPENDENCE_BACKWARD);
    ...
    /**
     * Obtain the set of all those nodes for which there exists a symbol {@code s} such that the value of {@code s} read
     * by that node is same as the value visible at the owner node.
     *
     * @return set of all those nodes for which there exists a symbol {@code s} such that the value of {@code s} read by
    ...
    /**
     * Obtain the set of all those nodes for which there exists a symbol {@code s} such that the value of {@code s}
     * written by that node is same as the value visible at owner node.
     *
     * @return set of all those nodes for which there exists a symbol {@code s} such that the value of {@code s} written
     * by that node is same as the value visible at the owner node.
     */
    ...
    /**
     * Populate the data structures {@link NodeInfo#readDestinations} and {@link NodeInfo#writeDestinations} using the
    ...
    public static void setReadWriteDestinations() {
        ...
        for (Node n1 : Program.getRoot().getInfo().getAllLeafNodesInTheProgram()) {
            DataDependenceForwardFF n1FF = (DataDependenceForwardFF) n1.getInfo().getIN(AnalysisName.DATA_DEPENDENCE_FORWARD);
            if (n1FF == null) {
                continue;
            }
            ...
            if (n1Reads.isUniversal()) {
                ...
                //								NodeInfo.addNodeToMap(n2.getInfo().getReadDestinations(), Cell.getGenericCell(), n1);
                //							} else {
            ...
            //				if (n1FF.readSources.isUniversal()) {
            //					for (Node n2 : n1FF.readSources.get(Cell.getGenericCell()).getReadOnlySet()) {
            //						NodeSet n2Set = n2.getInfo().getReadDestinations().get(Cell.getGenericCell());
            //						if (n2Set == null) {
            ...
            //						NodeSet n2Set = n2.getInfo().getWriteDestinations().get(Cell.getGenericCell());
            //						if (n2Set == null) {
            //							n2Set = new NodeSet(new HashSet<>(4));
            //						}
            //						n2Set.addNode(n1);
    ...
    /**
     * Collect first nodes reachable on all forward paths that may kill the value(s) for any cell in {@code atStack}, as
     * visible at the exit to this node.
     *
     * @param atStake
     *         a set of cells whose killers have to be found.
     *
     * @return set of all those first nodes reachable on all forward paths that may kill the value(s) for any cell in
    ...
    public Set<Node> getFirstPossibleKillersForwardExclusively(CellSet atStake) {
        ...
        }, (n) -> n.getNode().getInfo().getCFGInfo().getInterTaskLeafSuccessorNodesForVariables(n.getCallStack(), atStake, Program.sveSensitive));
        return nodeSet;
    }

    /**
     * Collect all those forward reachable leaf nodes that may read same value for the cell {@code w} as is visible at
     * the exit to this node.
     * <br>
     * Note that in this version, a node that both reads as well as writes to {@code w} is not added to the return set.
     *
     * @param w
     *         a cell.
     *
     * @return set of all those forward reachable leaf nodes that may read same value for the cell {@code w} as is
     * visible at the exit to this node.
     */
    public Set<Node> getAllUsesForwardsExclusively(Cell w) {
        ...
        }, n -> n.getNode().getInfo().getCFGInfo().getInterTaskLeafSuccessorNodesForVariables(n.getCallStack(), focusSet, Program.sveSensitive));
        return readSet;
    }

    /**
     * A traversal based liveness information. This seems to be a much better alternative than a costly IDFA pass, for
     * many scenarios.
     *
     * @param w
     *         a cell.
     *
     * @return true, if there may be any use of {@code w} on any path without it being overwritten, starting at the exit
     * of this node.
     */
    ...
    /**
     * Checks if there is a forward reachable leaf node that may read same value for the cell {@code w} as is visible at
     * the exit to this node.
     * <br>
     ...
     *         a cell.
     *
     * @return true if there exists a forward reachable leaf node that may read same value for the cell {@code w} as is
     * visible at the exit to this node.
     */
    ...
    /**
     * Collect all those backwards reachable leaf nodes that may read same value for the cell {@code w} as is visible at
     * the entry to this node.
     *
     * @param w
     *         a cell.
     *
     * @return set of all those backward reachable leaf nodes that may read same value for the cell {@code w} as is
    ...
    public Set<Node> getAllUsesBackwardsExclusively(Cell w) {
        ...
        }, n -> n.getNode().getInfo().getCFGInfo().getInterTaskLeafPredecessorNodesForVariables(n.getCallStack(), focusSet, Program.sveSensitive));
        return readSet;
    }

    /**
     * Checks if there is a backward reachable leaf node that may read same value for the cell {@code w} as is visible
     * at the entry to this node.
     *
     * @param w
     *         a cell.
     *
     * @return true if there exists a backward reachable leaf node that may read same value for the cell {@code w} as is
     * visible at the exit to this node.
     */
    ...
    /**
     * Obtain a list of {@code #pragma imop ....} annotations. Note that this field is applicable only for CFG nodes.
    ...
    /**
     * Given a set of nodes (possibly non-leaf), this method checks whether the owner expression is a predicate of a
     * some non-leaf node that (i) is SCOPed, and (ii) does not contain any element from the given set.
     *
     * @param nodeSet
     *
     * @return
    ...
    /**
     * Given a nodes, this method checks whether the owner expression is a predicate of some non-leaf node that (i) is
     ...
     * <p>
     * Note: If you wish to remove types and typedefs as well, use {@link NodeInfo#removeUnusedElements()} instead.
     */
                ...
                for (Symbol sym : symbolSet) {
                    if (!usedCells.contains(sym)) {
                        ...
                        if (declaringNode instanceof Declaration) {
                            ...
                            ElementsOfTranslation elemOfTranslation = Misc.getEnclosingNode(declaringNode, ElementsOfTranslation.class);
     ...
     * <p>
     * Note: If you wish to remove symbols and typedefs as well, use {@link NodeInfo#removeUnusedElements()} instead.
     */
     ...
     * <p>
     * Note: If you wish to remove symbols and types as well, use {@link NodeInfo#removeUnusedElements()} instead.
     */
            ...
            if (scopeNode instanceof TranslationUnit) {
                ...
                for (Typedef tDef : typedefSet) {
                    if (!usedTypedefs.contains(tDef)) {
                        ...
                        ElementsOfTranslation elemOfTranslation = Misc.getEnclosingNode(definingNode, ElementsOfTranslation.class);
                        scope.getF0().removeNode(elemOfTranslation);
    ...
    /**
     * For the owner node, this method returns a list of {@link Assignment} that may possibly happen during the
     * execution of the owner node, and are lexically within the node. Note that the assignment of arguments to
     * parameters, and returned value to the returned receiver are not considered as lexically enclosed assignments.
     ...
     *
     * @return a list of {@link Assignment}s that may get executed, and are lexically present within the owner node.
     */
    ...
    /**
     * For the owner node, this method returns a list of {@link Assignment} that may possibly happen during the
     * execution of the owner node. This list will also contain assignments that may happen in any of the called
     * procedures, and also the assignments of arguments to parameters, and returned values to return receiver. Hence,
    ...
    /**
     * Obtain string equivalent of the owner node, with the string of {@code changeSource} replaced by the string {@code
     * replacementString} (if the replacement would actually be possible in the AST).
     * <br>
     * Note that no resulting changes are done to the AST of the owner node, or the {@code changeSource} node. Also note
     * that this method is very different from a simple substring replacement.
     *
     * @param changeSource
     *         node whose string equivalent should be replaced with {@code changeDestination}, in the return value.
     * @param replacementString
     *         string that should replace the string of {@code changeSource} in the return value.
     *
     * @return string of the owner node, with the string of {@code changeSource}, replaced by {@code changeDestination}.
     */
    ...
    /**
     * Checks whether the associated node is connected to the main AST (AST of the program under consideration).
     *
     * @return {@code true}, if the node is connected to the main AST; {@code false} otherwise.
     */
     ...
     * <br>
     * We assume that a leaf CFG node performs a clean write to a cell, if it does not perform any other write, and if
    ...
    /**
     * List of locks that are taken before execution of the current node. <br> Note that the list is maintained in the
     * increasing order of closeness of locks to the node -- the immediately-enclosing lock should be present as the
    ...
    /**
     * A relatively faster version of clone. However, note that this method works on an existing object. This method
     * should be called first from the overridden setInfo method in the subclasses of Info.
     *
     * @param oldInfo
     */
    @Deprecated
     ...
     * respectively.
     *
     * @return true, if the control can flow in and out of this AST node except from the beginning and end of it,
     * respectively.
     *
    ...
    public List<Definition> deprecated_getDefinitionList() {
        //		Deprecated_AllDefinitionGetter defGetter = new Deprecated_AllDefinitionGetter();
    ...
    /**
     * Returns true if a parallel-flow-fact IN object exists for the owner node, corresponding to the specified analysis
     * name.
     *
     * @param analysisName
     *         enumeration constant corresponding to the analysis for which the object has to be checked.
     *
    ...
    /**
     * Returns true if a parallel-flow-fact OUT object exists for the owner node, corresponding to the specified
     * analysis name.
     *
     * @param analysisName
     *         enumeration constant corresponding to the analysis for which the object has to be checked.
    ...
    /**
     * Returns true if a flow-fact IN object exists for the owner node, corresponding to the specified analysis name.
     *
     * @param analysisName
     *         enumeration constant corresponding to the analysis for which the object has to be checked.
     *
    ...
    /**
     * Returns true if a flow-fact OUT object exists for the owner node, corresponding to the specified analysis name.
     *
     * @param analysisName
     *         enumeration constant corresponding to the analysis for which the object has to be checked.
     *
    ...
    /**
     * Returns the parallel-flow-fact IN object stored with the key {@code analysisName} in {@code parallelFlowFactsIN}
     * map of the owner node.
     *
     * @param analysisName
     *         Enumerator constant that corresponds to the analysis for which the parallel flow-fact IN object is
     *         needed.
     * @param pass
     *         An object of the analysis of type {@code analysisName</code>}.
     *
     * @return The parallel-flow-fact IN object corresponding to the analysis specified with <code>analysisName</code>
     */
    @Deprecated
    public Deprecated_FlowFact getParallelFlowFactIN(AnalysisName analysisName, Deprecated_InterProceduralCFGPass<? extends Deprecated_FlowFact> pass) {
    ...
    /**
     * Returns the parallel-flow-fact OUT object stored with the key {@code analysisName} in {@code
     * parallelFlowFactsOUT} map of the owner node.
     *
     * @param analysisName
     *         Enumerator constant that corresponds to the analysis for which the parallel flow-fact OUT object is
     *         needed.
     * @param pass
     *         An object of the analysis of type {@code analysisName</code>}.
     *
     * @return The parallel-flow-fact OUT object corresponding to the analysis specified with <code>analysisName</code>
     */
    @Deprecated
    public Deprecated_FlowFact getParallelFlowFactOUT(AnalysisName analysisName, Deprecated_InterProceduralCFGPass<? extends Deprecated_FlowFact> pass) {
     ...
     * node.
     * <p>
     * If there is no entry for {@code analysisName}, this method initializes the entry with {@code Top} element
     * corresponding to the specified {@code pass}.
     *
     * @param analysisName
     *         Enumerator constant that corresponds to the analysis for which the flow-fact IN object is needed.
     * @param pass
     *         An object of the analysis of type <code>analysisName</code>.
     *
     * @return The flow-fact IN object corresponding to the specified {@code pass}, else <code>Top</code> corresponding
     * to the specified {@code pass}.
     */
    @Deprecated
    public Deprecated_FlowFact getFlowFactIN(AnalysisName analysisName, Deprecated_InterProceduralCFGPass<? extends Deprecated_FlowFact> pass) {
        if (deprecated_flowFactsIN == null) {
            deprecated_flowFactsIN = new HashMap<>();
        }
        if (!deprecated_flowFactsIN.containsKey(analysisName)) {
            if (this.copySourceNode != null) {
                Deprecated_FlowFact copiedFlowFactIN = copySourceNode.getInfo().getFlowFactIN(analysisName, pass).getCopy();
     ...
     *         Enumerator constant that corresponds to the analysis for which the flow-fact IN object is needed.
     *
     * @return The flow-fact IN object corresponding to the specified {@code pass}, else <code>null</code>.
     */
    @Deprecated
     ...
     *
     * @param analysisName
     *         Enumerator constant that corresponds to the analysis for which the flow-fact OUT object is needed.
     * @param pass
     *         An object of the analysis of type <code>analysisName</code>.
     *
     * @return The flow-fact OUT object corresponding to the specified {@code pass}, else <code>Top</code> corresponding
     * to the specified {@code pass}.
     */
    @Deprecated
    public Deprecated_FlowFact getFlowFactOUT(AnalysisName analysisName, Deprecated_InterProceduralCFGPass<? extends Deprecated_FlowFact> pass) {
        if (deprecated_flowFactsOUT == null) {
            deprecated_flowFactsOUT = new HashMap<>();
        }
        if (!deprecated_flowFactsOUT.containsKey(analysisName)) {
            if (this.copySourceNode != null) {
                Deprecated_FlowFact copiedFlowFactOUT = copySourceNode.getInfo().getFlowFactOUT(analysisName, pass).getCopy();
     ...
     *         Enumerator constant that corresponds to the analysis for which the flow-fact OUT object is needed.
     *
     * @return The flow-fact OUT object corresponding to the specified {@code pass}, else {@code null}.
     */
    @Deprecated
    ...
    /**
     * Stores the provided flow-fact object, <code>flowFact</code>, with the specified analysis
     * <code>analysisName</code>, as a key in the {@code parallelFlowFactsIN} map.
     *
     * @param analysisName
     *         Enumerator constant that corresponds to the analysis for which the parallel-flow-fact IN object has to be
     *         stored.
     * @param flowFact
     *         The parallel-flow-fact IN object corresponding to the analysis specified with <code>analysisName</code>.
     */
    @Deprecated
     ...
     *
     * @param analysisName
     *         Enumerator constant that corresponds to the analysis for which the flow-fact OUT object has to be
     *         stored.
     * @param flowFact
     *         The flow-fact OUT object corresponding to the analysis specified with <code>analysisName</code>.
     */
    @Deprecated
     ...
     *
     * @param analysisName
     *         Enumerator constant that corresponds to the analysis for which the flow-fact IN object has to be stored.
     * @param flowFact
     *         The flow-fact IN object corresponding to the analysis specified with <code>analysisName</code>.
     */
    @Deprecated
    ...
    /**
     * Loses all references to the information related to particular analysis, thereby helping the GC.
     *
     * @param type
     *         Analysis related to which the complete information has to be deleted.
     */
    @Deprecated
