...
public class NodeInfo implements Cloneable {
	 ...
	 * This field is used to find the duplicates.
	 */
	...
	/**
	 * Set of all the cells that may be read in the execution of this node
	 ...
	 * create appropriate interfaces for CFG leaf and non-leaf nodes, and add
	 * this field in the corresponding CFG nodes.
	 */
	private CellList readList;

	/**
	 * Set of all the cells that may be written in the execution of this node
	 * (and the called methods, if any).
	 * <br>
	 * Note that this field is used only for CFG leaf nodes. Once we add
	 * {@link Predicate} to appropriate places in the production rules, we can
	...
	/**
	 * Set of all the shared cells that may be written in the execution of this
	 * node (and the called methods, if any), excluding all field accesses.
	 * <br>
	 * Note that this field is used only for CFG leaf nodes. Once we add
	 * {@link Predicate} to appropriate places in the production rules, we can
	 * create appropriate interfaces for CFG leaf and non-leaf nodes, and add
	...
	/**
	 * List of all the {@link CastExpression} expressions on which dereference
	 * may have been made using an asterisk ({@code *}), and whose dereferenced
	 * location may have been read within this node.
	 */
	private List<CastExpression> pointerDereferencedExpressionReads;
	/**
	 * List of all the {@link CastExpression} expressions on which dereference
	 * may have been made using an asterisk ({@code *}), and whose dereferenced
	 * location may have been written within this node.
	 */
	 ...
	 * disabled when running the stabilization of points-to.
	 * <br>
	 * IMPORTANT: This method must always be called before attempting to read
	 * any (of the six) memoized access list.
	 * 
	 * @return
	 *         true, if any change might have been made.
	 */
	@Deprecated
	...
	/**
	 * Checks whether the points-to flow-facts might be stale, without resetting
	 * the invalid flag.
	 * When this flag is set, none of the memoized access lists are used as they
	 * are.
	 * 
	 * @return
	 */
	...
	/**
	 * For some special cases, if the analysis has not yet been run even once,
	 * this method runs the analysis.
	 * 
	 * @param analysisName
	 *            an analysis name; check the method body to add more analyses
	 *            to the list of allowed analyses here.
	 */
	 ...
	 * Note that this function may return {@code null}, which implies the
	 * following:
	 * <ul>
	 ...
	 * 
	 * @param analysisName
	 *            name (ID) of the analysis for which the IN flow-fact of this
	 *            node has to be obtained.
	 * @return
	 */
	 ...
	 * @return
	 *         (unstable) IN flow-fact for {@code analysisName}.
	 */
	 ...
	 * @return
	 *         (unstable) OUT flow-fact for {@code analysisName}.
	 */
	...
	/**
	 * Sets {@code flowFact} as the IN flow-fact for {@code analysisName} for
	 * this node.
	 * 
	 * @param analysisName
	 *            name (ID) of the analysis for which the OUT flow-fact of this
	 *            node has to be obtained.
	 * @param flowFact
	...
	/**
	 * Sets {@code flowFact} as the OUT flow-fact for {@code analysisName} for
	 * this node.
	 * 
	 * @param analysisName
	 *            name (ID) of the analysis for which the OUT flow-fact of this
	 ...
	 * this node (and the called methods, if any).
	 * Note that this method also considers those accesses that may happen in
	 * nodes that are not lexically inside the owner node, and those that may be
	 * reachable via some jumps (not just calls).
	 * 
	 * <br>
	 * Note that this field is used only for CFG leaf nodes. Once we add
	 * {@link Predicate} to appropriate places in the production rules, we can
	 ...
	 * 
	 * @return
	 *         set of all those cells that may be written in the execution of
	 *         this node.
	 */
	...
	/**
	 * Checks whether this node (or its corresponding CFG node) may write to a
	 ...
	 * of this node (and the called methods, if any).
	 * Note that this method also considers those accesses that may happen in
	 * nodes that are not lexically inside the owner node, and those that
	 * may be reachable via some jumps (not just calls).
	 * <br>
	 * Note that this field is used only for CFG leaf nodes. Once we add
	 * {@link Predicate} to appropriate places in the production rules, we can
	 * create appropriate interfaces for CFG leaf and non-leaf nodes, and add
	 ...
	 * 
	 * @return
	 *         set of all those shared cells that may be read in the execution
	 *         of this node.
	 */
	 ...
	 * execution of this node (and the called methods, if any).
	 * Note that this method also considers those accesses that may happen in
	 * nodes that are not lexically inside the owner node, and those that
	 * may be reachable via some jumps (not just calls).
	 * <br>
	 ...
	 * 
	 * @return
	 *         set of all those shared cells that may be written in the
	...
	/**
	 * Returns a set of all the symbols that may be read in the execution of
	 * this node (and the called methods, if any), excluding the nodes that are
	 * encountered after the control leaves this node from any exit-point.
	 * 
	 * @return
	 *         set of all those symbols that may be read between the entry to
	...
	/**
	 * Returns a set of all the symbols that may be written in the execution of
	 * this node (and the called methods, if any), excluding the nodes that are
	 * encountered after the control leaves this node from any exit-point.
	 * 
	 * @return
	 *         set of all those symbols that may be written between the entry to
	 *         and exit from this node.
	 */
	...
	/**
	 * Returns a set of all the shared cells that may be read in the
	 * execution of this node (and the called methods, if any), excluding all
	 * the field cells.
	 * Note that this method also considers those accesses that may happen in
	 * nodes that are not lexically inside the owner node, and those that
	 * may be reachable via some jumps (not just calls).
	 ...
	 * 
	 * @return
	 *         set of all those shared cells that may be read in the
	 *         execution of this node, excluding all the field cells.
	 */
	...
	/**
	 * Obtain the list of access-expressions (composed up of {@link BaseSyntax}
	 * and List{@literal <}{@link Tokenizable}{@literal >}) representing
	 * array dereferences that may be read anywhere in this node.
	 * 
	 * @return
	 *         list of access-expressions (composed up of {@link BaseSyntax} and
	 *         List{@literal <}{@link Tokenizable}{@literal >})
	 *         representing array dereferences that may be read anywhere in this
	 *         node.
	 */
	 ...
	 * Obtain the list of access-expressions (composed up of {@link BaseSyntax}
	 * and List{@literal <}{@link Tokenizable}{@literal >}) representing
	 * array dereferences that may be written anywhere in this node.
	 * 
	 * @return
	 *         list of access-expressions (composed up of {@link BaseSyntax} and
	 *         List{@literal <}{@link Tokenizable}{@literal >})
	 *         representing array dereferences that may be written anywhere in
	 *         this node.
	 */
	 ...
	 * dereferenced location may have been read within this node.
	 * <br>
	 ...
	 * 
	 * @return
	 *         list of all the {@link CastExpression} expressions on which
	 ...
	 * Obtain a list of all the {@link CastExpression} expressions on which
	 * dereference may have been made using an asterisk ({@code *}), and whose
	 * dereferenced location may have been written within this node.
	 * <br>
	 * For example, in {@code *(e1) = ...;}, this method returns a set
	 * containing {@code e1}.
	 * 
	 ...
	 *         list of all the {@link CastExpression} expressions on which
	 *         dereference may have been made using an asterisk ({@code *}), and
	 *         whose dereferenced location may have been written within this
	 *         node.
	 */
	 ...
	 * {@code freeNames} by {@link FreeVariable},
	 * for the {@link #readList} and {@link #writeList} of the owner leaf node.
	 * 
	 * @param freeNames
	 *            list of identifiers that refer to free variables in the owner
	 *            leaf node.
	 */
	...
	/**
	 * Checks if this node is a self-update node -- one that may read and write
	 * to the same location.
	 * 
	 * @return
	 *         true, if this CFG node is a node that reads as well as writes to
	 *         the same cell.
	 */
	...
	/**
	 * Returns a set of all those cells that may be accessible at the given
	 * node, exclusively -- i.e., if this node is a {@link Scopeable} object,
	 * then we do not include the set of those cells that are created within
	 * this scope.
	 *
	 * @return
	 *         a set of all those cells that may be accessible at the given
	 *         node, exclusively.
	 */
	...
	/**
	 * Returns the set of symbol names that are accessible at this node,
	 * exclusively -- i.e., if this node is a {@link Scopeable} object, then the
	 * names from the symbol table of this object are not added to the return
	 * set.
	 * 
	 * @return
	 *         set of symbol names that are accessible at this node,
	 *         exclusively.
	 */
	...
	/**
	 * Returns the set of symbols that are accessible at this node
	 * (inclusively).
	 * 
	...
	/**
	 * Returns the set of all those function-definitions (including the current
	 * node itself, if it's a function-definition), that may get executed during
	 * the processing of this node.
	 * 
	 * @return
	 *         set of all those function-definitions that may get executed
	 *         during the execution of this node (including the node itself, if
	...
	/**
	 * Obtain a set of all those function-definitions that may be the target of
	 * any call-statement that is lexically enclosed within the owner node.
	 * 
	 * @return
	 *         set of all those function-definitions that may be the target of
	 *         any call-statement that is lexically enclosed within the owner
	 *         node.
	 */
	 ...
	 * 
	 * @return
	 *         the outermost enclosing node for a given node.
	 */
	public Node getOuterMostEncloser() {
		List<Node> outerList = (CollectorVisitor.collectNodeListInGenericGraph(this.node, null, (n) -> (n == null),
				(n) -> {
	 ...
	 * 
	 * @return
	 *         the outermost enclosing non-leaf node for a given node
	 *         (exclusively).
	 */
	 ...
	 * @return
	 *         a set of all those non-leaf nodes that lexically enclose the node
	 *         {@code node}
	 */
	 ...
	 * starting with the immediately enclosing non-leaf encloser (exclusively).
	 * Note that root of the tree would be the last element.
	 * 
	 * @return
	 *         the list of all the non-leaf nodes that enclose {@code node},
	...
	/**
	 * Get a set of all those non-leaf nodes that may enclose the given
	 * statement, including those that may reside in the callers of the function
	 * in which the given node exists.
	 * 
	 * @return
	 *         a set of all those non-leaf nodes that may enclose the given
	...
	/**
	 * Find whether the control can flow in and out of this AST node except from
	 ...
	 * 
	 * @return
	 *         a set of those nodes which are sources of (intra-procedural)
	 *         edges that enter this node from somewhere apart from BeginNode.
	 */
	 ...
	 * 
	 * @return
	 *         a set of those nodes which are destinations of (intra-procedural)
	 *         edges that enter this node from somewhere apart from BeginNode.
	 */
	 ...
	 * 
	 * @return
	 *         set of nodes from which control may flow inside this node.
	 */
	 ...
	 * 
	 * @return
	 *         set of nodes from which control may flow outside this node.
	 */
	...
	public String getDebugString() {
		...
		commentors.add((n) -> {
			...
			//			}
			for (Phase ph : n.getInfo().getNodePhaseInfo().getStalePhaseSet()) {
	...
	/**
	 * Obtain an internal CFG node in owner {@code node}, that contains the
	 * label {@code l}.
	 * 
	 * @param l
	 *            label that has to be searched for.
	 * @return
	 *         CFG node that has a label-annotation of {@code l}; null,
	 *         otherwise.
	 */
	...
	/**
	 * Obtain the set of nodes which may represent definitions that are
	 * reachable at the owner node, for cell {@code cell}.
	 * 
	 * @param
	 * cell
	 *            whose reaching definition has to be found .
	 * @return
	 *         set of nodes which may represent definitions that are reachable
	 *         at the owner node, for cell {@code cell}.
	 */
	...
	/**
	 * 
	 * Obtain the set of nodes which may represent definitions that may be
	 * reachable to instances of the owner node in given phase {@code ph}.
	 * 
	 * @param ph
	 *            phase in which the owner node exists, and for which the
	 *            reaching-definitions have to be found.
	 * @param phaseInsensitiveReachingDefinitions
	 *            set of nodes that are reaching-definitions of this node across
	 *            all phases.
	 * @return
	 */
	...
	/**
	 * Obtain the set of all those nodes for which there exists a symbol
	 * {@code s} such that the value of {@code s} read by that node is same as
	 * the value visible at the owner node.
	 * 
	 * @return
	 *         set of all those nodes for which there exists a symbol
	 *         {@code s} such that the value of {@code s} read by that node is
	 *         same as the value visiable at the owner node.
	 */
	...
	/**
	 * Obtain the set of all those nodes for which there exists a symbol
	 * {@code s} such that the value of {@code s} written by that node is same
	 * as the value visible at owner node.
	 * 
	 * @return
	 *         set of all those nodes for which there exists a symbol
	 *         {@code s} such that the value of {@code s} written by that node
	 *         is same as the value visible at the owner node.
	 */
	 ...
	 * {@link NodeInfo#writeDestinations}
	 * using the forward data-dependence information, for all the nodes.
	 */
	...
	/**
	 * Collect first nodes reachable on all forward paths that may kill the
	 * value(s) for any cell in {@code atStack}, as visible at the exit
	 * to this node.
	 * 
	 * @param atStake
	 *            a set of cells whose killers have to be found.
	 * @return
	 *         set of all those first nodes reachable on all forward paths that
	 *         may kill the value(s) for any cell in {@code atStack}, as visible
	 *         at the exit to this node.
	 * 
	 */
	...
	/**
	 * Collect all those forward reachable leaf nodes that may read same value
	 * for the cell {@code w} as is visible at the exit to this node.
	 * <br>
	 ...
	 *            a cell.
	 * @return
	 *         set of all those forward reachable leaf nodes that may read same
	 *         value for the cell {@code w} as is visible at the exit to this
	 *         node.
	 */
	 ...
	 *            a cell.
	 * @return
	 *         true, if there may be any use of {@code w} on any path without it
	...
	/**
	 * Checks if there is a forward reachable leaf node that may read same value
	 * for the cell {@code w} as is visible at the exit to this node.
	 * <br>
	 ...
	 *            a cell.
	 * @return
	 *         true if there exists a forward reachable leaf node that may read
	 *         same value for the cell {@code w} as is visible at the exit to
	 *         this node.
	 */
	...
	/**
	 * Collect all those backwards reachable leaf nodes that may read same value
	 * for the cell {@code w} as is visible at the entry to this node.
	 * 
	 * @param w
	 *            a cell.
	 * @return
	 *         set of all those backward reachable leaf nodes that may read same
	 *         value for the cell {@code w} as is visible at the entry to this
	 *         node.
	 */
	...
	/**
	 * Checks if there is a backward reachable leaf node that may read same
	 * value for the cell {@code w} as is visible at the entry to this node.
	 * 
	 * @param w
	 *            a cell.
	 * @return
	 *         true if there exists a backward reachable leaf node that may read
	 *         same value for the cell {@code w} as is visible at the exit to
	 *         this node.
	 */
	...
	/**
	 * Obtain a list of {@code #pragma imop ....} annotations.
	 * Note that this field is applicable only for CFG nodes. For other nodes,
	 * we simply return null.
	 * 
	 * @return
	 *         list of pragma imop annotations, corresponding to the owner CFG
	 *         node.
	 */
	 ...
	 * owner expression is a predicate of a some non-leaf node that (i) is
	 * SCOPed, and (ii) does not contain any element from the given set.
	 * 
	 * @param nodeSet
	 * @return
	 */
	...
	/**
	 * Given a nodes, this method checks whether the
	 * owner expression is a predicate of some non-leaf node that (i) is
	...
	/**
	 * For the owner node, this method returns a list of
	 * {@link Assignment} that may
	 * possibly happen during the execution of the owner node,
	 ...
	 * Hence, Note an assignment may represent any of the following form:
	 * <ol>
	 * <li>NonConditionalExpressions, e.g., {@code x = y + 3}</li>
	 * <li>Initializations, e.g., {@code int x = y + 3}</li>
	 ...
	 * {@code changeSource} replaced by the string {@code replacementString} (if
	 * the replacement would actually be possible in the AST).
	 * <br>
	 * Note that no resulting changes are done to the AST of the owner node, or
	 * the {@code changeSource} node.
	 ...
	 *            in the return value.
	 * @return
	 *         string of the owner node, with the string of
	...
	/**
	 * Checks whether the associated node is connected to the main AST (AST of
	 * the program under consideration).
	 * 
	 * @return
	 *         {@code true}, if the node is connected to the main AST;
	...
	/**
	 * Used to obtain the cell, if any, that has been written cleanly in this
	 * node.
	 * <br>
	 * We assume that a leaf CFG node performs a clean write to a cell, if it
	 * does not perform any other write, and if it does not read from that cell.
	 * 
	 * @param n
	 *            node that has to be tested.
	 * @return
	 *         cell that has been cleanly written in this node, otherwise null.
	 */
	 ...
	 *            node that has to be tested.
	 * @return
	 *         true, if this node is a clean-write to the node n.
	 */
	 ...
	 * locks to the node
	 * -- the immediately-enclosing lock should be present as the last element
	 * of this list.
	 */
	@Deprecated
	...
	/**
	 * A relatively faster version of clone. However, note that this method
	 * works on an existing object.
	 * This method should be called first from the overridden setInfo method in
	 * the subclasses of Info.
	 * 
	 * @param oldInfo
	 */
	@Deprecated
	 ...
	 * 
	 * @return
	 *         true, if the control can flow in and out of this AST node except
	 *         from the beginning and end of it, respectively.
	 */
	@Deprecated
	 ...
	 * 
	 * @param analysisName
	 *            enumeration constant corresponding to the analysis for which
	 *            the object has to be checked.
	...
	/**
	 * Returns true if a parallel-flow-fact OUT object exists for the owner
	 * node,
	 * corresponding to the specified analysis name.
	 * 
	 * @param analysisName
	 ...
	 * {@code analysisName} in {@code parallelFlowFactsIN} map of the owner
	 * node.
	 * 
	 * @param analysisName
	 *            Enumerator constant that corresponds to the analysis for which
	 *            the parallel flow-fact IN object is needed.
	 * @param pass
	 ...
	 * {@code analysisName} in {@code parallelFlowFactsOUT} map of the owner
	 * node.
	 * 
	 * @param analysisName
	 *            Enumerator constant that corresponds to the analysis for which
	 *            the parallel flow-fact OUT object is needed.
	 * @param pass
	 *            An object of the analysis of type {@code analysisName</code>}.
	 ...
	 * node.
	 * <p>
	 * If there is no entry for {@code analysisName}, this method initializes
	 * the entry with {@code Top} element corresponding to the specified
	 * {@code pass}.
	 * 
	 * @param analysisName
	 *            Enumerator constant that corresponds to the analysis for which
	 *            the flow-fact IN object is needed.
	 * @param pass
	 *            An object of the analysis of type <code>analysisName</code>.
	 * @return The flow-fact IN object corresponding to the specified
	 *         {@code pass},
	 *         else <code>Top</code> corresponding to the specified
	 *         {@code pass}.
	 */
	@Deprecated
	 ...
	 * @param analysisName
	 *            Enumerator constant that corresponds to the analysis for which
	 *            the flow-fact OUT object is needed.
	 * @param pass
	 *            An object of the analysis of type <code>analysisName</code>.
	 * @return The flow-fact OUT object corresponding to the specified
	 *         {@code pass},
	 *         else <code>Top</code> corresponding to the specified
	 *         {@code pass}.
	 */
	 ...
	 * @param analysisName
	 *            Enumerator constant that corresponds to the analysis for which
	 *            the parallel-flow-fact IN object has to be stored.
	 * @param flowFact
	 *            The parallel-flow-fact IN object corresponding to the analysis
	 ...
	 * @param analysisName
	 *            Enumerator constant that corresponds to the analysis for which
	 *            the flow-fact OUT object has to be stored.
	 * @param flowFact
	 ...
	 * @param analysisName
	 *            Enumerator constant that corresponds to the analysis for which
	 *            the flow-fact IN object has to be stored.
	 * @param flowFact
