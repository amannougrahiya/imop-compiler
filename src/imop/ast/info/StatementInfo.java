...
public class StatementInfo extends NodeInfo {
    ...
    /**
     * Adds a labeled annotation to the owner statement at the specified index. Adds incompleteness if the enclosing
     * SwitchStatement is missing, for case and default labels. Performs no other updates. This method should be called
    ...
    public void initAddLabelAnnotation(Label newLabel) {
        ...
        //					new IncompleteEdge(TypeOfIncompleteness.UNKNOWN_CASE_SOURCE, this.getNode(), (CaseLabel) newLabel));
        ...
        //					.add(new IncompleteEdge(TypeOfIncompleteness.UNKNOWN_DEFAULT_SOURCE, this.getNode()));
        //		}
    }
    ...
    /**
     * Removes all those labels from the relevant context (enclosing function or switch) which are similar to <b>but not
     * same as</b> the given {@code newLabel}.
     *
     * @param newLabel
     *         label whose similar (not same) labels have to be removed from the relevant context.
     */
     ...
     *
     * @param caseString
     *         string of constant-expression of the case-label that has to be removed.
     *
     * @return true, if the label was removed.
     */
    ...
    /**
     * This method performs required updates to various data structures upon addition of label {@code newLabel} to a
    ...
    private void updateUponLabelAddition(Label newLabel) {
        ...
        //		if (newLabel instanceof SimpleLabel) {
        //			FunctionDefinition enclosingFunction = (FunctionDefinition) Misc.getEnclosingFunction(this.getNode());
        //			if (enclosingFunction != null) {
        //				StatementInfo.removeLabelFromAllInFunction(enclosingFunction, newLabel, this.getNode());
        //			}
        //		} else {
        //			SwitchStatement enclosingSwitch = Misc.getEnclosingSwitch(this.getNode()); // can be null.
        //			if (enclosingSwitch != null) {
        //				StatementInfo.removeLabelFromAllInSwitch(enclosingSwitch, newLabel, this.getNode());
        //			}
        //		}
    ...
    /**
     * Re-adjusts the edges upon addition of {@code label} to this statement. This method assumes that any pre-existing
     * labels with the same name have been removed already. Currently, this method performs adjustment of semantics
     ...
     *         when equal to WILL_ALL_BE_UPDATED, exactly one edge is always ensured between the switch-predicate and
     *         this node, if it is a case/default statement.
     *
     * @return a set of predecessors that have been added to the current node as a result of insertion of {@code label}.
     */
    public Set<Node> adjustSemanticsForLabelAddition(Label label) {
        ...
        if (label instanceof SimpleLabel) {
            ...
            if (label instanceof CaseLabel) {
                ...
                // OLD CODE: Now, we do not need to explicitly check about the existence of a CFG edge
                ...
                //					}
                //				}
                //				if (hasMoreCaseOrDefaultLabels && labelAdditionMode != LabelUpdateMode.WILL_ALL_BE_UPDATED) {
                //					// Do not add an edge between predicate and thisStmt.
                //				} else {
                /*
                 ...
                 */
                //				if (!predicate.getInfo().getCFGInfo().getSuccBlocks().contains(this.getNode())) {
                Expression predicate = switchStatement.getInfo().getCFGInfo().getPredicate();
                CFGInfo.connectAndAdjustEndReachability(predicate, this.getNode());
    ...
    /**
     * This method performs required updates to various data structures upon removal of label {@code oldLabel} to a
    ...
    /**
     * Readjusts the edges upon removal of {@code label} from this statement.<br> Currently, this method performs
     ...
     *
     * @param label
     *         label that has to be removed from this statement. Note that this label should be removed only after the
     *         call to this method returns.
     * @param labelRemovalMode
     *         when equal to WILL_ALL_BE_UPDATED, the CFG edge from a switch-predicate to a case/default statement is
     *         always removed. Otherwise, the edge is removed only if the statement has exactly one case/default label.
     *
     * @return a set of GotoStatements (for SimpleLabel), or Expression (for Case/DefaultLabels) due to which ls) due to
        ...
        if (label instanceof SimpleLabel) {
            ...
            for (Node pred : new ArrayList<>(predList)) {
                if (pred instanceof GotoStatement) {
                    ...
                    //								.add(new IncompleteEdge(TypeOfIncompleteness.UNKNOWN_GOTO_DESTINATION, gotoStmt));
    ...
    /**
     * Performs various automated updates owing to labels of a newly added node. Note that this method does not itself
     * add the label/node. The label might already be present in {@code annotatedLabels} of the owner node, but the node
    ...
    public void updateUponLabeledNodeAddition(Label newLabel) {
        ...
        // Update CFG, Incompleteness, FlowFacts, and other data structures.
        Set<Node> newPredecessors = this.adjustSemanticsForLabelAddition(newLabel); //, LabelUpdateMode.WILL_ALL_BE_UPDATED);
        AutomatedUpdater.adjustPhaseAndInterTaskEdgesUponLabelUpdate(newPredecessors);
    ...
    /**
     * This method performs required updates to various data structures upon removal of the node annotated with label
     * {@code oldLabel}. Note that this method does not itself remove the node. It helps in modeling the scenario where
    ...
    /**
     * Flag used to ensure that the task of populating annotations is carried out exactly once before finishing the
    ...
    /**
     * Obtain an List of all the labels that may be annotated to the owner statement {@code n} (or rather, that of the
    ...
    /**
     * Check whether there are any immediate labels that annotate the owner, or the CFG node that it wraps.
     *
