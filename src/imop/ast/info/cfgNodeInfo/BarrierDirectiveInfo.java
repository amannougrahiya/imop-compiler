...
public class BarrierDirectiveInfo extends OmpDirectiveInfo {
    ...
    /**
     * This method refills the $movableNodes$ with those statements (reachable from this barrier on a barrier-free path)
    ...
    public void updateMovableList(ParallelConstruct parConstruct) {
        ...
        ReachableNodeMarker reacher = new ReachableNodeMarker((BarrierDirective) getNode(), parConstruct, getReachableNodes());
        ...
        MovableNodeMarker movableMarker = new MovableNodeMarker((BarrierDirective) getNode(), getMovableNodes(), getNonMovableNodes(), getReachableNodes());
        for (Node succ : this.getCFGInfo().getSuccBlocks()) {
    ...
    /**
     * <br>
     * CREATED/UPDATED: 09-Dec-2017 8:16:52 AM (Edited by: Aman) <br> Percolates code from below to above, while making
     * sure that no code is moved upwards beyond the entry point of {@code itStmt}.
     * <br>
     * Note that this barrier might not even be present inside or reachable from within {@code itStmt} (in which case,
     * it must share a synchronization set with some other barrier in {@code itStmt}).
     *
     * @param itStmt
     *         loop above whose entry point the code should not be moved.
     *
     * @return true, if code involving read/write of any shared variable is moved above this node.
     */
    public boolean percolateCodeUpwards(IterationStatement itStmt) {
        ...
        outer:
        while (movingPointer < elemList.size()) {
            ...
            if (Misc.isCFGNonLeafNode(element)) {
                Set<NodeWithStack> internalNodes = element.getInfo().getCFGInfo().getIntraTaskCFGLeafContentsOfSameParLevel(new CallStack());
                if (internalNodes.stream().filter(ns -> ns.getNode() instanceof BarrierDirective).count() > 0) {
                    ...
                    if (element instanceof IfStatement) {
                        ...
                        CollectorVisitor.collectNodeSetInGenericGraph(new NodeWithStack(startingEndNode, new CallStack()), endingElements, (n) -> {
                            ...
                            return false;
                        }, (n) -> n.getNode().getInfo().getCFGInfo().getParallelConstructFreeInterProceduralLeafPredecessors(n.getCallStack()));
    ...
    /**
     * Moves this barrier down the succeeding fork instruction, if any, if the forking predicate leaves the enclosing
     * loop {@code itStmt}. This step is followed by calling {@link #percolateCodeUpwards(IterationStatement)}, on all
     ...
     *
     * @return true, if the barrier was forked down. Note that this implies code motion of predicate across the barrier.
     */
    ...
    /**
     * This method attempts to perform aggressive code motion by moving code around the loop. With the help of {@link
     * NodeInfo#getJumpedPredicates()}, we ensure that each statement moves only once across a predicate across its
     * lifetime.
     *
     * @param itStmt
    ...
    /**
     * This function moves some of the statements within the body of a loop to the top of the loop, and returns the
     * index of locations up till which (inclusively), the code should be moved across the back-edges. Note that this
     * call may have side-effects of re-shuffling the code within first phase of the loop.
     *
     * @param whileStmt
     *         loop whose statements have to be moved across the back-edges.
     *
     * @return index up till which code should be moved across the back-edge of the loop.
     */
    ...
    /**
     * This method tests whether a node at index {@code movingPointer} can be shifted uptil immediately below the
     * barrier at index {@code indexOfBarrier}.
     * <br>
     * NOTE: This method assumes that there are no barriers in any of the intermediate nodes.
     *
     * @param elemList
    ...
    public static boolean checkPotentialToCrossUpwards(List<Node> elemList, int indexOfBarrier, int movingPointer) {
        ...
        for (int i = movingPointer - 1; i > indexOfBarrier; i--) {
            ...
            if (controlConfined) {
                 ...
                 * false.
                 */
                for (Node bothWaysReachableLeaf : ancestor.getInfo().getCFGInfo().getBothBeginAndEndReachableIntraTaskLeafNodes()) {
    ...
    /**
     * Assuming that there is no data dependence between both nodes for nodes that are reachable via both ends of the
     * {@code aboveNode}, this method informs whether upward motion of {@code belowNode} may kill some value on any path
     * going out from {@code aboveNode}. Note that the {@code belowNode} is assumed to contain no barriers. Furthermore,
     * conservatively, we assume that the {@code aboveNode} does not have any inJumpDestinations.
     *
     * @param aboveNode
     * @param belowNode
     *
    ...
    public static boolean killedIfPushedUpwards(Node aboveNode, Node belowNode) {
        ...
        if (belowNode.getInfo().getNodePhaseInfo().getPhaseSet().stream().anyMatch(p -> p.getNodeSet().stream().anyMatch(n -> (
                !belowSet.contains(n) && Misc.doIntersect(belowSharedWrites, n.getInfo().getSharedReads()) &&
        ...
        for (Node outJumpSrc : unreachableHeaders) {
            ...
            for (Cell w : belowWrites) {
                ...
                CollectorVisitor.collectNodeSetInGenericGraph(startPoint, endPoints, (n) -> {
                    ...
                    Set<NodeWithStack> successors = (dsa ==
                            DataSharingAttribute.SHARED) ? n.getNode().getInfo().getCFGInfo().getInterTaskLeafSuccessorNodes(n.getCallStack()) : n.getNode().getInfo().getCFGInfo().getInterProceduralLeafSuccessors(n.getCallStack());
    ...
    /**
     * Assuming that there is no data dependence between both nodes, this method informs whether motion of one above the
     * other may kill some value on any path. Note that the second node is assumed to be control-confined, and does not
    ...
    public static boolean killedIfPushedBelow(Node aboveNode, Node belowNode) {
        ...
        for (Node outJumpSrc : aboveNode.getInfo().getOutJumpSources()) {
            ...
            for (Cell w : belowWrites) {
                CollectorVisitor.collectNodeSetInGenericGraph(startPoint, endPoints, (n) -> {
                    ...
                    Set<NodeWithStack> retSet = new HashSet<>();
                    for (NodeWithStack succNode : n.getNode().getInfo().getCFGInfo().getInterTaskLeafSuccessorNodes(n.getCallStack())) {
                        CellSet writesOfN = new CellSet(n.getNode().getInfo().getWrites());
                        if (writesOfN.size() == 1 && writesOfN.contains(w)) {
                            // Skip.
                        } else {
        ...
        //					LivenessFlowFact lff = (LivenessFlowFact) outJumpSrc.getInfo().getOUT(AnalysisName.LIVENESS);
        //					assert (lff != null) : "Problem with liveness information of: " + outJumpSrc + "@line#"
        //							+ Misc.getLineNum(outJumpSrc);
        //					Set<Node> outJumpSucc = outJumpSrc.getInfo().getCFGInfo().getInterProceduralLeafSuccessors();
    ...
    /**
     * Assuming that there is no data dependence between both nodes, this method informs whether motion of one below the
     * other may kill some value on any path. Note that the {@code aboveNode} is assumed to be control-confined, and
        ...
        for (Node outJumpSrc : belowNode.getInfo().getOutJumpSources()) {
            ...
            for (Cell w : aboveWrites) {
                CollectorVisitor.collectNodeSetInGenericGraph(startPoint, endPoints, (n) -> {
                    ...
                    Set<NodeWithStack> retSet = new HashSet<>();
                    for (NodeWithStack succNode : n.getNode().getInfo().getCFGInfo().getInterTaskLeafPredecessorNodes(n.getCallStack())) {
    ...
    /**
     * This method percolates the movable statements above the barrier. It ensures that the code motion is accompanied
        ...
        while (currentStmtIndex < containingList.size()) {
            ...
            while (newPosition > indexOfBarrier) {
                if (newPosition > indexOfBarrier + 1) {
                    ...
                    // Note that various constituents of the moved statement may be present individually
                    ...
                    for (Node movedNode : movedNodes) {
                        movedNode.getInfo().getNodePhaseInfo().crudeSetPhaseSet(new HashSet<>(this.getNodePhaseInfo().getPhaseSet()));
                    }
