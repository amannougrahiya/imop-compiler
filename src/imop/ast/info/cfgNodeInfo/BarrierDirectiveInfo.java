...
public class BarrierDirectiveInfo extends OmpDirectiveInfo {
	...
	/**
	 * This method refills the $movableNodes$ with those statements (reachable
	 * from
	 * this barrier on a barrier-free path) which can be moved up this barrier.
	 * 
	 * @param parConstruct
	 ...
	 * Percolates code from below to above, while making sure that no code is
	 * moved upwards beyond the entry point of {@code itStmt}.
	 * <br>
	 * Note that this barrier might not even be present inside or reachable from
	 * within {@code itStmt} (in which case, it must share a synchronization set
	 * with some other barrier in {@code itStmt}).
	 * 
	 * @param itStmt
	 *            loop above whose entry point the code should not be moved.
	 * @return
	 *         true, if code involving read/write of any shared variable is
	 *         moved above this node.
	 */
	 ...
	 * forking predicate leaves the enclosing loop {@code itStmt}.
	 * This step is followed by calling
	 * {@link #percolateCodeUpwards(IterationStatement)}, on all the newly
	 * created copies of this barrier on the forked paths, if any.
	 * 
	 * @param itStmt
	 *            iteration-statement beyond which the code should not move.
	 * @return
	 *         true, if the barrier was forked down.
	 *         Note that this implies code motion of predicate across the
	 *         barrier.
	 */
	...
	/**
	 * This method attempts to perform aggressive code motion by moving code
	 * around the loop.
	 * With the help of {@link NodeInfo#getJumpedPredicates()}, we ensure that
	 * each statement moves only once across a predicate across its lifetime.
	 * 
	 * @param itStmt
	...
	/**
	 * This function moves some of the statements within the body of a loop to
	 * the top of the loop, and returns the index of locations up till which
	 * (inclusively), the code should be moved across the back-edges.
	 * Note that this call may have side-effects of re-shuffling the code within
	 * first phase of the loop.
	 * 
	 * @param whileStmt
	 *            loop whose statements have to be moved across the back-edges.
	 * @return
	 *         index up till which code should be moved across the back-edge of
	 *         the loop.
	 */
	 ...
	 * shifted uptil immediately below the barrier at index
	 * {@code indexOfBarrier}.
	 * <br>
	...
	/**
	 * Assuming that there is no data dependence between both nodes for nodes
	 * that are reachable via both ends of the {@code aboveNode}, this
	 * method informs whether upward motion of {@code belowNode} may kill some
	 * value on any path going out from {@code aboveNode}. Note that the
	 * {@code belowNode} is assumed to contain no barriers.
	 * Furthermore, conservatively, we assume that the {@code aboveNode} does
	 * not have any inJumpDestinations.
	 * 
	 * @param aboveNode
	 * @param belowNode
	 * @return
	...
	public Set<Node> getExplicitSiblings() {
		...
		for (Phase ph : new HashSet<>(barrier.getInfo().getNodePhaseInfo().getPhaseSet())) {
			...
			for (PhasePoint endingPhasePoint : ph.getEndPoints()) {
				...
				if (Program.sveSensitive == SVEDimension.SVE_SENSITIVE
	...
	/**
	 * Assuming that there is no data dependence between both nodes, this
	 * method informs whether motion of one above the other may kill some value
	 * on any path. Note that the second node is assumed to be control-confined,
	 * and does not contain barriers or flushes.
	 * 
	 * @param aboveNode
	 * @param belowNode
	 * @return
	...
	/**
	 * Assuming that there is no data dependence between both nodes, this
	 * method informs whether motion of one below the other may kill some value
	 * on any path. Note that the {@code aboveNode} is assumed to be
	 * control-confined,
	 * and does not contain barriers or flushes.
	 * 
	 * @param aboveNode
	...
	/**
	 * This method percolates the movable statements above the barrier. It
	 * ensures that the code motion is accompanied with updates in the relevant
	 * data structures.
	 * 
	 * @return
	 */
