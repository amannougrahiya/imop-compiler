...
public class WhileStatementInfo extends IterationStatementInfo {
    ...
    public void unrollLoop(int unrollingFactor) {
        ...
        for (int i = 0; i < unrollingFactor; i++) {
            ...
            newBodyString += ((CompoundStatement) loop.getInfo().getCFGInfo().getBody()).getInfo().getCopyForUnrolling();
        }
        newBodyString += "}";
    ...
    public List<SideEffect> changePredicateToConstantTrue() {
        ...
        //		Statement predBreak;
        //		for (Node predicateSucc : predicate.getInfo().getCFGInfo().getLeafSuccessors()) {
        //			if (this.hasConstantPredicate()) {
        //				assert (this.predicateIsConstantFalse()); // Else we would have returned already.
        ...
        //				continue;
        //			}
        //			sideEffectList.addAll(InsertOnTheEdge.insertAggressive(predicate, predicateSucc, predBreak));
        //		}
