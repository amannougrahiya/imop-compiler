...
public class DummyFlushDirectiveInfo extends FlushDirectiveInfo {
	...
	/**
	 * If this node does not share any other common phase with its neighbour
	 * DFD, then remove the link with the neighbour.
	 * 
	 * @param phase
	 *            phase that has to be removed.
	 */
	public void flushMHPData(Phase phase) {
		Set<Phase> thisPhaseSet = this.getNodePhaseInfo().getStalePhaseSet();
		...
		for (InterTaskEdge ite : new HashSet<>(this.incomingInterTaskEdges)) {
			...
			Set<Phase> thatPhaseSet = thatDFD.getInfo().getNodePhaseInfo().getStalePhaseSet();
	 ...
	 * 
	 * @return
	 *         a set of only those dummy flush directives to which communication
	 *         is made via at least one symbol.
	 */
