...
public class DummyFlushDirectiveInfo extends FlushDirectiveInfo {
    ...
    /**
     * If this node does not share any other common phase with its neighbour DFD, then remove the link with the
     * neighbour.
     *
     * @param phase
    ...
    public void flushMHPData(AbstractPhase<?, ?> phase) {
        ...
        for (InterTaskEdge ite : new HashSet<>(this.incomingInterTaskEdges)) {
            ...
            foundACommonPhase = thisPhaseSet.stream().filter(thisPh -> thisPh !=
                    phase).anyMatch(thatPhaseSet::contains);
    ...
    /**
     * Returns a set of only those dummy flush directives to which communication is made via at least one symbol.
     *
     * @return a set of only those dummy flush directives to which communication is made via at least one symbol.
     */
