...
public class RedundantSynchronizationRemovalForYA {
    ...
    public static void removeBarriers(Node root) {
        for (Node barrierNode : Misc.getInheritedEncloseeList(root, BarrierDirective.class)) {
            ...
            if (removable) {
                ...
                //				System.err.println("** Replacing the barrier at line #" + Misc.getLineNum(barrier) + " with a flush.");
    ...
    public static boolean phasesConflictForBarrier(YPhase ph1, YPhase ph2, BarrierDirective barrier) {
        ...
        for (Node n1 : nodesInP1) {
            if (n1 instanceof BeginNode) {
                ...
                if (beginNode.getParent() instanceof MasterConstruct) {
                    ...
                    // Conservatively check that this master construct applies to the parallel-construct of ph1.
        ...
        for (Node n2 : nodesInP2) {
            if (n2 instanceof BeginNode) {
                ...
                if (beginNode.getParent() instanceof MasterConstruct) {
                    MasterConstruct master = (MasterConstruct) beginNode.getParent();
                    // Conservatively check that this master construct applies to the parallel-construct of ph2.
                    ParallelConstruct parentPar = Misc.getEnclosingNode(master, ParallelConstruct.class);
        ...
        if (!masterNodesInP1.isEmpty()) {
            if (RedundantSynchronizationRemovalForYA.setsConflictForBarrier(ph1, masterNodesInP1, barrier, ph2, nodesInP2)) {
                return true;
            }
        }
        if (!masterNodesInP2.isEmpty()) {
            if (RedundantSynchronizationRemovalForYA.setsConflictForBarrier(ph1, nodesInP1, barrier, ph2, masterNodesInP2)) {
                return true;
            }
        }
    ...
    public static boolean setsConflictForBarrier(YPhase phAbove, Set<Node> nodesAbove, BarrierDirective barrier, YPhase phBelow, Set<Node> nodesBelow) {
        //		/*
        //		 * Debug code starts.
        //		 */
        //		boolean DF = false;
        //		if (nodesAbove.stream().anyMatch(n -> n.toString().contains("diff_imopVarPre77 += mydiff;"))
        //				&& nodesBelow.stream().anyMatch(n -> n.toString().contains("(float) diff_imopVarPre77"))) {
        //			DF = true;
