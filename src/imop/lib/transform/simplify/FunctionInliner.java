...
public class FunctionInliner {
    ...
    /**
     * Attempts to inline all those inlineable call-sites in the CFG (and CG) of {@code root} that contain a
     * parallel-construct or a barrier within them.
     *
     * @param root
     *         root node under which we will attempt to inline all call-sites that may help further barrier removal.
     */
     ...
     * <br>
     * For now, we consider a call-site to be profitable for inlining if it contains at least one barrier/parallel
     * construct, and in case of former, if it contains at least one shared access.
     *
     * @param callStmt
     *         a call-statement to be inspected.
     *
     * @return true, if the call-statement may contain any barrier or parallel construct, along with at least one shared
    ...
    /**
     * For now, this method does not suggest any renaming of the types, and returns false if there is any namespace
     * collision for types of the given function definition with the type table visible at the call-statement.
     * <br>
     * Later, we should modify this function such that it removes all found conflicts by type renaming.
     *
     * @param funcDef
        ...
        for (String typeName : body.getInfo().getTypeTable().keySet()) {
            if (typeNameAtCallStmt.contains(typeName)) {
                Misc.warnDueToLackOfFeature(
                        "Unable to inline a function due to collision of type named " + typeName, null);
                return false;
            }
        }
        return true;
    ...
    /**
     * For now, this method does not suggest any renaming of the typedef's, and returns false if there is any namespace
     * collision for typedef of the given function definition with the typedef table visible at the call-statement.
     * <br>
     * Later, we should modify this function such that it removes all found conflicts by typedef renaming.
     *
     * @param funcDef
     *         target function definition of {@code callStmt}.
     * @param callStmt
    ...
    /**
     * For now, this method does not suggest any renaming of the labels, and returns false if there is any namespace
     * collision for labels of the given function definition with the labels at the caller.
     * <br>
     * Later, we should modify this function such that it removes all found conflicts by label renaming.
     *
     * @param funcDef
     *         target function definition of {@code callStmt}.
     * @param callStmt
    ...
    private static boolean handleLabelNameConflicts(CallStatement callStmt, FunctionDefinition funcDef) {
        ...
        if (caller == null || !(caller instanceof FunctionDefinition)) {
            Misc.warnDueToLackOfFeature("Cannot inline a callStatement that is not enclosed within a function-definition.", null);
            return false;
        }
        ...
        if (Misc.doIntersect(callerLabels, calleeLabels)) {
            Misc.warnDueToLackOfFeature(
                    "Unable to inline a function due to conflict of the labels at the caller and calle:" +
                            callerLabels + " conflicts with " + calleeLabels, null);
            return false;
        }
        return true;
    }
    ...
    /**
     * Ensures that there are no static declarations in the given function-definition. Note that this function may
     * update the function-definition.
     *
     * @param funcDef
     *         a function-definition.
     *
     * @return true, if all static declarations, if any, have been pushed up to the global scope, with proper renaming,
     * if required.
     */
    ...
    /**
     * Attempts to inline the function called at {@code callStmt}. If binding of the given call-statement cannot be
     * determined unambiguously, then this method performs no change and returns {@code false}.
     * <br>
     * Note that labels attached to the {@code callStmt} are shifted to the first executable statement in the inline
     * code.
     *
     * @param callStmt
     *         a call-statement to be inlined.
     *
     * @return {@code true}, if the inlining was successful; {@code false} otherwise.
     */
    private static boolean inlineFunctionDefinition(CallStatement callStmt) {
        ...
        FunctionInliner.performInliningWithLocalVariableConflictRemoval(callStmt, funcDef, newDeclarationList, renamingMap);
        ProfileSS.nextCP();
    ...
    /**
     * Ensures that there are no name-conflicts of parameters with variables visible at the call-site.
     *
     * @param funcDef
     * @param callStmt
     * @param newDeclarationList
     *
     * @return true, if all parameter name-conflicts have been resolved in the provided body.
     */
    private static boolean handleParameterNameConflicts(CallStatement callStmt, FunctionDefinition funcDef, List<Declaration> newDeclarationList, HashMap<String, String> renamingMap) {
        CompoundStatement compStmt = funcDef.getInfo().getCFGInfo().getBody();
    ...
    private static Declaration obtainDeclarationForLocal(Declaration localDecl, CallStatement callStmt, FunctionDefinition funcDef, HashMap<String, String> renamingMap, Set<String> allSymbolNamesAtCaller) {
    ...
    private static void performInliningWithLocalVariableConflictRemoval(CallStatement callStmt, FunctionDefinition funcDef, List<Declaration> parameterDeclarationList, HashMap<String, String> parameterRenamingMap) {
        ...
        for (Declaration declForParam : parameterDeclarationList) {
            List<SideEffect> sideEffects = callerScope.getInfo().getCFGInfo().addDeclaration(callStmtIndex++, declForParam);
            ProfileSS.nextCP();
        ...
        if (!retStmt.isEmpty()) {
            if (retStmt.size() == 1 && retStmt.get(0) == calleeBody.getInfo().getCFGInfo().getElementList().get(
                    calleeBody.getInfo().getCFGInfo().getElementList().size() - 1)) {
        ...
        for (Node element : calleeBody.getInfo().getCFGInfo().getElementList()) {
            if (element instanceof Declaration) {
                Declaration newDecl = FunctionInliner.obtainDeclarationForLocal((Declaration) element, callStmt, funcDef, allRenamingMap, allSymbolNamesAtCaller);
                callerScope.getInfo().getCFGInfo().addDeclaration(callStmtIndex++, newDecl);
                ProfileSS.nextCP();
            } else if (element instanceof Statement) {
                for (Statement newStmt : FunctionInliner.getStmtToInline((Statement) element, callStmt, allRenamingMap, jumpLabel)) {
                    List<SideEffect> sideEffects = callerScope.getInfo().getCFGInfo().addElement(callStmtIndex++, newStmt);
                    ProfileSS.nextCP();
                    for (SideEffect sideEffect : sideEffects) {
                        if (sideEffect.getClass().getSimpleName().equals("IndexIncremented") ||
                        sideEffect instanceof AddedDFDPredecessor || sideEffect instanceof AddedDFDSuccessor ||
    ...
    /**
     * Handles dummy-flushes, variable renaming, and external return statements on unconnected nodes, to obtain list of
     * nodes to be added.
     * <br>
     * Note that later we should unify this method with {@link #handleInternalReturnStatements(CallStatement, String,
     ...
     * @return
     */
    private static List<Statement> getStmtToInline(Statement element, CallStatement callStmt, HashMap<String, String> allRenamingMap, String jumpLabel) {
