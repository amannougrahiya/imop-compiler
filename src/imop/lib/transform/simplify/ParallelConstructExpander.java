...
public class ParallelConstructExpander {
    ...
    public static void mergeParallelRegions(Node node) {
        ...
        do {
            ...
            for (ParallelConstruct parCons : parConsList) {
                 ...
                 */
                if (!parCons.getInfo().getCFGNestingNonLeafNodes().stream().anyMatch(e -> (e instanceof IterationStatement))) {
                    continue;
                }
                /*
                 * New code above.
    ...
    public static boolean expandDownward(ParallelConstruct parCons, CompoundStatement scope, int index) {
        ...
        do {
            ...
            if (nextNode instanceof Declaration) {
                ...
                Set<NodeWithStack> contentsWithStack = nextNode.getInfo().getCFGInfo().getIntraTaskCFGLeafContentsOfSameParLevel(new CallStack());
                for (NodeWithStack nS : contentsWithStack) {
                    ...
                    if (node instanceof BeginNode) {
                        ...
                        if (bn.getParent() instanceof OmpConstruct) {
                            // Do not fuse this node above, if there exists a nested omp construct inside it.
                            return changed;
                        }
                    }
                    if (node instanceof OmpDirective) {
                        // Do not fuse this node above, if there exists a nested omp directive inside it.
                        return changed;
                    }
    ...
    public static MasterConstruct setAndGetLastMaster(ParallelConstruct parCons) {
        CompoundStatementCFGInfo bodyCFGInfo = ((CompoundStatement) parCons.getInfo().getCFGInfo().getBody()).getInfo().getCFGInfo();
        ...
        do {
            ...
            if (Misc.doIntersect(clauseReadCells, new CellList(prevWriteCellList))) {
                if (prevNode instanceof Declaration) {
                    if (!BasicTransform.pushDeclarationUp((Declaration) prevNode).stream().anyMatch(s -> s instanceof SyntacticConstraint)) {
            ...
            // If previous node is a write to a list item that belongs to the list of a firstprivate,
            // copyin, or reduction clause.
            // then, for now, we will disable the fusion. Later, we can handle the case more aggressively.
            ...
            if (prevNode instanceof Declaration) {
                ...
                if (Misc.doIntersect(writtenCells, accessedLater)) {
                    if (!BasicTransform.pushDeclarationUp(decl).stream().anyMatch(s -> s instanceof SyntacticConstraint)) {
                        //						ProfileSS.nextCP();
                        if (testedDecls.contains(prevNode)) {
                            return changed;
                        } else {
                 ...
                 */
                Set<NodeWithStack> contentsWithStack = prevNode.getInfo().getCFGInfo().getIntraTaskCFGLeafContentsOfSameParLevel(new CallStack());
                for (NodeWithStack nS : contentsWithStack) {
                    Node node = nS.getNode();
                    if (node instanceof BeginNode) {
                        BeginNode bn = (BeginNode) node;
                        if (bn.getParent() instanceof OmpConstruct) {
                            // Do not move this node above, if there exists a nested omp construct inside it.
                            return changed;
                        }
                    }
                    if (node instanceof OmpDirective) {
                        // Do not move this node above, if there exists a nested omp directive inside it.
                        return changed;
                    }
                    if (node instanceof UnknownCpp || node instanceof UnknownPragma) {
                        return changed;
    ...
    /**
     * Checks if a symbol, or any of its elements (in case of stack arrays) are being written anywhere in the parallel
     * construct.
     *
     * @param sym
     ...
     *
     * @return true, if any leaf node within {@code parCons} can write to {@code sym} (or its {@link FieldCell}, in case
     ...
     *
     * @return true, if there are no accesses to {@code sym} by any threads other than the master thread.
     */
    ...
    /**
     * Interchanges a parallel construct with enclosing loops and selection statements, if appropriate.
     *
     * @param parCons
     *
    ...
    public static boolean interchangeUp(ParallelConstruct parCons) {
        ...
        if (scope.getInfo().getCFGInfo().getElementList().size() == 1) {
            assert !(encloser instanceof CompoundStatement) : "The parCons was normalized already!"; // ..since we have already normalized the parCons.
            if (encloser instanceof OmpConstruct) {
                ...
                if (encloser instanceof IfStatement) {
                    ...
                    if (scope == ifStmt.getInfo().getCFGInfo().getThenBody()) {
                        if (!ifStmt.getInfo().getCFGInfo().hasElseBody()) {
                            ...
                            if (elseElements.size() == 1) {
                                ...
                                if (elseNode instanceof ParallelConstruct) {
                                    ...
                                    newIfStmt.getInfo().getCFGInfo().setThenBody(parBody);
                                    newIfStmt.getInfo().getCFGInfo().setPredicate(ifStmt.getInfo().getCFGInfo().getPredicate());
                                    newIfStmt.getInfo().getCFGInfo().setElseBody(elsePar.getInfo().getCFGInfo().getBody());
                                    return true;
                                }
                            }
                            ...
                            //							ProfileSS.nextCP();
                            MasterConstruct master = (MasterConstruct) newElseBody.getInfo().getCFGInfo().getElementList().get(0);
                        ...
                        //						ProfileSS.nextCP();
                        MasterConstruct master = (MasterConstruct) newThenBody.getInfo().getCFGInfo().getElementList().get(0);
                        //						ProfileSS.nextCP();
                ...
                if (encloser instanceof DoStatement) {
                    ...
                    CellSet readsInPred = predicate.getInfo().getSymbolReads();
                    if (predicate.getInfo().mayWrite()) {
                        // TODO: Simplify predicate of do. Future code. Should be uncommented and tested after setup of Insert* methods is complete.
                        ...
                        //						Statement tempVal = FrontEnd.parseAndNormalize(predSimpleName + " = " + predicate + ";",
                        //								Statement.class);
                    ...
                    if (initExp != null) {
                        ...
                        if (initExp.getInfo().mayWrite()) {
                            // TODO: Instead of simplifying the expression, we can move the expression outside the loop. Future code. Uncomment and test this, once Insert* methods are settled down.
                    ...
                    if (stepExp != null) {
                        ...
                        if (stepExp.getInfo().mayWrite()) {
                            // TODO: Instead of simplifying the expression, we can move the expression outside the loop header. Future code. Uncomment and test this, once Insert* methods are settled down.
                            //						Expression newStepExp = FrontEnd.parseAndNormalize(stepExp.toString(), Expression.class);
                    ...
                    if (predicate != null) {
                        CellSet readsInPred = predicate.getInfo().getSymbolReads();
                        if (predicate.getInfo().mayWrite()) {
                            // TODO: Simplify the predicate. Long process. Future code. Uncomment and test this, once Insert* methods are set.
                            //							Expression newPred = FrontEnd.parseAndNormalize(predicate.toString(), Expression.class);
                    ...
                    newForStmt.getInfo().getCFGInfo().setInitExpression(forStmt.getInfo().getCFGInfo().getInitExpression());
                    newForStmt.getInfo().getCFGInfo().setTerminationExpression(forStmt.getInfo().getCFGInfo().getTerminationExpression());
                    newForStmt.getInfo().getCFGInfo().setStepExpression(forStmt.getInfo().getCFGInfo().getStepExpression());
                    return true;
                } else {
                    return false;
                }
        ...
        if (encloser instanceof FunctionDefinition) {
            ...
            if (scope.getInfo().getCFGInfo().getElementList().size() == 1) {
                ...
                CellSet readsInPred = predicate.getInfo().getSymbolReads();
                if (predicate.getInfo().mayWrite()) {
                    // TODO: Simplify predicate of while. Future code. Should be uncommented and tested after setup of Insert* methods is complete.
    ...
    private static boolean bodyFusable(ParallelConstruct parCons, CompoundStatement body) {
        ...
        Set<NodeWithStack> contentsWithStack = body.getInfo().getCFGInfo().getIntraTaskCFGLeafContentsOfSameParLevel(new CallStack());
        for (NodeWithStack nS : contentsWithStack) {
            Node node = nS.getNode();
            if (node instanceof BeginNode) {
                BeginNode bn = (BeginNode) node;
    ...
    private static boolean functionSwappableAggressive(FunctionDefinition func, ParallelConstruct parCons) {
        ...
        //			// More aggressive check later, which looks into allowing greater variety of fusionable constructs.
        ...
        System.err.println(
                "\t -- Swapping up the parallel construct above " + func.getInfo().getFunctionName() + "().");
        /*
        ...
        for (CallStatement callStmt : func.getInfo().getCallersOfThis()) {
            if (callStmt.getPostCallNode().hasReturnReceiver()) {
                ...
                // Most probably this race would be removed by privatization of the return receiver anyway.
                //				SimplePrimaryExpression retRec = callStmt.getPostCallNode().getReturnReceiver();
                //				// Write return value to a private variable, to avoid (benign) data race.
                //				String privName = Builder.getNewTempName();
                //				Symbol retSym = Misc.getSymbolEntry(retRec.getIdentifier().toString(), callStmt);
    ...
    public static boolean remainingElementsFusableAcrossFunction(FunctionDefinition funcDef, ParallelConstruct parCons) {
        ...
        // These checks below have been extended from expandDownward. Note the subtle differences.
        for (int i = parIndex + 1; i < elementList.size(); i++) {
            Node nextNode = elementList.get(i);
            if (nextNode instanceof Declaration || nextNode instanceof UnknownCpp ||
                    nextNode instanceof UnknownPragma || nextNode instanceof OmpDirective ||
                    nextNode instanceof OmpConstruct) {
                return false;
            }
        ...
        List<OmpClause> clauseList = parCons.getInfo().getOmpClauseList();
        // These checks below have been extended from expandUpward. Note the subtle differences.
        // Also note that this loop must go from parIndex-1 to 0, and not the other way round.
        for (int i = parIndex - 1; i >= 0; i--) {
            ...
            if (prevNode instanceof UnknownCpp || prevNode instanceof UnknownPragma ||
                    prevNode instanceof OmpDirective || prevNode instanceof OmpConstruct) {
                return false;
            }
    ...
    public static boolean remainingElementsFusableAcrossIteration(IterationStatement itStmt, ParallelConstruct parCons) {
    ...
    private static boolean mergeDataAttributeListsBelow(ParallelConstruct parConsAbove, ParallelConstruct parConsBelow) {
        boolean changed = false;
        ...
        // If either of the parallel constructs contain the default clause, make sure that they both are
