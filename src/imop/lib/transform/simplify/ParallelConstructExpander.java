...
public class ParallelConstructExpander {
	...
	public static boolean expandParRegion(ParallelConstruct parCons) {
		...
		System.err.println("\t -- Processing the parallel construct at line #" + lineNum);
		boolean changed = false;
	...
	public static boolean expandDownward(ParallelConstruct parCons, CompoundStatement scope, int index) {
		...
		do {
			...
			if (nextNode instanceof Declaration) {
				...
				InsertImmediatePredecessor.insert(masterCons.getInfo().getCFGInfo().getNestedCFG().getEnd(),
						nextNode);
	...
	public static boolean expandUpward(ParallelConstruct parCons, CompoundStatement scope, int index) {
		...
		outer: do {
			...
			if (prevNode instanceof Declaration) {
				...
				assert (idList.size() == 1) : "Encountered code where a declaration declares more than one symbol! \n "
						+ decl;
				...
				InsertImmediateSuccessor.insert(masterCons.getInfo().getCFGInfo().getNestedCFG().getBegin(),
						prevNode);
	 ...
	 * @return
	 *         true, if any leaf node within {@code parCons} can write to
	 *         {@code sym} (or its {@link FieldCell}, in case of arrays); false,
	 *         otherwise.
	 */
	 ...
	 * @return
	 *         true, if there are no accesses to {@code sym} by any threads
	 *         other than the master thread.
	 */
	 ...
	 * @return
	 *         true, if the merger was successful.
	 */
