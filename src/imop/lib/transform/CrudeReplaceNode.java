/*
 * Copyright (c) 2019 Aman Nougrahiya, V Krishna Nandivada, IIT Madras.
 * This file is a part of the project IMOP, licensed under the MIT license.
 * See LICENSE.md for the full text of the license.
 * 
 * The above notice shall be included in all copies or substantial
 * portions of this file.
 */
package imop.lib.transform;

import imop.ast.node.external.*;
import imop.ast.node.internal.*;
import imop.baseVisitor.DepthFirstVisitor;
import imop.lib.util.Misc;

public class CrudeReplaceNode {

	private CrudeReplaceNode() {
	}

	/**
	 * Replaces the node oldNode with newNode, with no other updates.
	 * 
	 * @param oldNode
	 * @param newNode
	 * @return
	 */
	public static <T extends Node> boolean replace(T oldNode, T newNode) {
		// assert
		// (oldNode.getClass().getSimpleName().equals(newNode.getClass().getSimpleName()))
		// : "Trying to replace a "
		// + oldNode.getClass().getSimpleName() + " with a " +
		// newNode.getClass().getSimpleName() + "!";
		Node parent = oldNode.getParent();
		CrudeNodeReplacer replacer = new CrudeNodeReplacer(oldNode, newNode);
		if (parent == null) {
			Misc.warnDueToLackOfFeature("Replacing an oldNode with newNode, where oldNode\'s parent is null.", null);
			return false;
		}
		parent.accept(replacer);
		if (replacer.replaced) {
			return true;
		} else {
			return false;
		}
	}

	private static class CrudeNodeReplacer extends DepthFirstVisitor {
		boolean replaced = false;
		private Node oldNode;
		private Node newNode;

		public <T extends Node> CrudeNodeReplacer(T oldNode, T newNode) {
			this.oldNode = oldNode;
			this.newNode = newNode;
		}

		//
		// Auto class visitors--probably don't need to be overridden.
		//
		@Override
		public void visit(NodeList n) {
			int index = n.getNodes().indexOf(oldNode);
			if (index != -1) {
				n.removeNode(index);
				n.addNode(index, newNode);
			}
		}

		@Override
		public void visit(NodeListOptional n) {
			if (n.present()) {
				int index = n.getNodes().indexOf(oldNode);
				if (index != -1) {
					n.removeNode(index);
					n.addNode(index, newNode);
				}
			}
		}

		@Override
		public void visit(NodeOptional n) {
			if (n.present()) {
				if (n.getNode() == oldNode) {
					n.setNode(newNode);
				}
			}
		}

		@Override
		public void visit(NodeSequence n) {
			int index = n.getNodes().indexOf(oldNode);
			if (index != -1) {
				n.removeNode(index);
				n.addNode(index, newNode);
			}
		}

		@Override
		public void visit(NodeChoice n) {
			if (n.getChoice() == oldNode) {
				n.setChoice(newNode);
				replaced = true;
				return;
			}
		}

		@Override
		public void visit(NodeToken n) {
		}

		//
		// User-generated visitor methods below
		//

		/**
		 * Special Nodes: BeginNode and EndNode
		 */
		@Override
		public void visit(BeginNode n) {
		}

		@Override
		public void visit(EndNode n) {
		}

		/**
		 * f0 ::= ( ElementsOfTranslation() )+
		 */
		@Override
		public void visit(TranslationUnit n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeList) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ExternalDeclaration()
		 * | UnknownCpp()
		 * | UnknownPragma()
		 */
		@Override
		public void visit(ElementsOfTranslation n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= Declaration()
		 * | FunctionDefinition()
		 * | DeclareReductionDirective()
		 * | ThreadPrivateDirective()
		 */
		@Override
		public void visit(ExternalDeclaration n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( DeclarationSpecifiers() )?
		 * f1 ::= Declarator()
		 * f2 ::= ( DeclarationList() )?
		 * f3 ::= CompoundStatement()
		 */
		@Override
		public void visit(FunctionDefinition n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((Declarator) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((CompoundStatement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= DeclarationSpecifiers()
		 * f1 ::= ( InitDeclaratorList() )?
		 * f2 ::= ";"
		 */
		@Override
		public void visit(Declaration n) {
			if (n.getF0() == oldNode) {
				n.setF0((DeclarationSpecifiers) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( Declaration() )+
		 */
		@Override
		public void visit(DeclarationList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeList) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( ADeclarationSpecifier() )+
		 */
		@Override
		public void visit(DeclarationSpecifiers n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeList) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= StorageClassSpecifier()
		 * | TypeSpecifier()
		 * | TypeQualifier()
		 */
		@Override
		public void visit(ADeclarationSpecifier n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <AUTO>
		 * | <REGISTER>
		 * | <STATIC>
		 * | <EXTERN>
		 * | <TYPEDEF>
		 */
		@Override
		public void visit(StorageClassSpecifier n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( <VOID> | <CHAR> | <SHORT> | <INT> | <LONG> | <FLOAT> |
		 * <DOUBLE> |
		 * <SIGNED> | <UNSIGNED> | StructOrUnionSpecifier() | EnumSpecifier() |
		 * TypedefName() )
		 */
		@Override
		public void visit(TypeSpecifier n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <RESTRICT>
		 * | <CONST>
		 * | <VOLATILE>
		 * | <INLINE>
		 * | <CCONST>
		 * | <CINLINED>
		 * | <CINLINED2>
		 * | <CSIGNED>
		 * | <CSIGNED2>
		 */
		@Override
		public void visit(TypeQualifier n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * 
		 * f0 ::= ( StructOrUnionSpecifierWithList() |
		 * StructOrUnionSpecifierWithId()
		 * )
		 */
		@Override
		public void visit(StructOrUnionSpecifier n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= StructOrUnion()
		 * f1 ::= ( <IDENTIFIER> )?
		 * f2 ::= "{"
		 * f3 ::= StructDeclarationList()
		 * f4 ::= "}"
		 */
		@Override
		public void visit(StructOrUnionSpecifierWithList n) {
			if (n.getF0() == oldNode) {
				n.setF0((StructOrUnion) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((StructDeclarationList) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= StructOrUnion()
		 * f1 ::= <IDENTIFIER>
		 */
		@Override
		public void visit(StructOrUnionSpecifierWithId n) {
			if (n.getF0() == oldNode) {
				n.setF0((StructOrUnion) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <STRUCT>
		 * | <UNION>
		 */
		@Override
		public void visit(StructOrUnion n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( StructDeclaration() )+
		 */
		@Override
		public void visit(StructDeclarationList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeList) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= InitDeclarator()
		 * f1 ::= ( "," InitDeclarator() )*
		 */
		@Override
		public void visit(InitDeclaratorList n) {
			if (n.getF0() == oldNode) {
				n.setF0((InitDeclarator) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= Declarator()
		 * f1 ::= ( "=" Initializer() )?
		 */
		@Override
		public void visit(InitDeclarator n) {
			if (n.getF0() == oldNode) {
				n.setF0((Declarator) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= SpecifierQualifierList()
		 * f1 ::= StructDeclaratorList()
		 * f2 ::= ";"
		 */
		@Override
		public void visit(StructDeclaration n) {
			if (n.getF0() == oldNode) {
				n.setF0((SpecifierQualifierList) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((StructDeclaratorList) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( ASpecifierQualifier() )+
		 */
		@Override
		public void visit(SpecifierQualifierList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeList) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= TypeSpecifier()
		 * | TypeQualifier()
		 */
		@Override
		public void visit(ASpecifierQualifier n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= StructDeclarator()
		 * f1 ::= ( "," StructDeclarator() )*
		 */
		@Override
		public void visit(StructDeclaratorList n) {
			if (n.getF0() == oldNode) {
				n.setF0((StructDeclarator) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= StructDeclaratorWithDeclarator()
		 * | StructDeclaratorWithBitField()
		 */
		@Override
		public void visit(StructDeclarator n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= Declarator()
		 * f1 ::= ( ":" ConstantExpression() )?
		 */
		@Override
		public void visit(StructDeclaratorWithDeclarator n) {
			if (n.getF0() == oldNode) {
				n.setF0((Declarator) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ":"
		 * f1 ::= ConstantExpression()
		 */
		@Override
		public void visit(StructDeclaratorWithBitField n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((ConstantExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= EnumSpecifierWithList()
		 * | EnumSpecifierWithId()
		 */
		@Override
		public void visit(EnumSpecifier n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <ENUM>
		 * f1 ::= ( <IDENTIFIER> )?
		 * f2 ::= "{"
		 * f3 ::= EnumeratorList()
		 * f4 ::= "}"
		 */
		@Override
		public void visit(EnumSpecifierWithList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((EnumeratorList) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <ENUM>
		 * f1 ::= <IDENTIFIER>
		 */
		@Override
		public void visit(EnumSpecifierWithId n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= Enumerator()
		 * f1 ::= ( "," Enumerator() )*
		 */
		@Override
		public void visit(EnumeratorList n) {
			if (n.getF0() == oldNode) {
				n.setF0((Enumerator) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= ( "=" ConstantExpression() )?
		 */
		@Override
		public void visit(Enumerator n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( Pointer() )?
		 * f1 ::= DirectDeclarator()
		 */
		@Override
		public void visit(Declarator n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((DirectDeclarator) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= IdentifierOrDeclarator()
		 * f1 ::= DeclaratorOpList()
		 */
		@Override
		public void visit(DirectDeclarator n) {
			if (n.getF0() == oldNode) {
				n.setF0((IdentifierOrDeclarator) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((DeclaratorOpList) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( ADeclaratorOp() )*
		 */
		@Override
		public void visit(DeclaratorOpList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= DimensionSize()
		 * | ParameterTypeListClosed()
		 * | OldParameterListClosed()
		 */
		@Override
		public void visit(ADeclaratorOp n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "["
		 * f1 ::= ( ConstantExpression() )?
		 * f2 ::= "]"
		 */
		@Override
		public void visit(DimensionSize n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "("
		 * f1 ::= ( ParameterTypeList() )?
		 * f2 ::= ")"
		 */
		@Override
		public void visit(ParameterTypeListClosed n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "("
		 * f1 ::= ( OldParameterList() )?
		 * f2 ::= ")"
		 */
		@Override
		public void visit(OldParameterListClosed n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * | "(" Declarator() ")"
		 */
		@Override
		public void visit(IdentifierOrDeclarator n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( "*" | "^" )
		 * f1 ::= ( TypeQualifierList() )?
		 * f2 ::= ( Pointer() )?
		 */
		@Override
		public void visit(Pointer n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( TypeQualifier() )+
		 */
		@Override
		public void visit(TypeQualifierList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeList) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ParameterList()
		 * f1 ::= ( "," "..." )?
		 */
		@Override
		public void visit(ParameterTypeList n) {
			if (n.getF0() == oldNode) {
				n.setF0((ParameterList) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ParameterDeclaration()
		 * f1 ::= ( "," ParameterDeclaration() )*
		 */
		@Override
		public void visit(ParameterList n) {
			if (n.getF0() == oldNode) {
				n.setF0((ParameterDeclaration) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= DeclarationSpecifiers()
		 * f1 ::= ParameterAbstraction()
		 */
		@Override
		public void visit(ParameterDeclaration n) {
			if (n.getF0() == oldNode) {
				n.setF0((DeclarationSpecifiers) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((ParameterAbstraction) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= Declarator()
		 * | AbstractOptionalDeclarator()
		 */
		@Override
		public void visit(ParameterAbstraction n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( AbstractDeclarator() )?
		 */
		@Override
		public void visit(AbstractOptionalDeclarator n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= ( "," <IDENTIFIER> )*
		 */
		@Override
		public void visit(OldParameterList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= AssignmentExpression()
		 * | ArrayInitializer()
		 */
		@Override
		public void visit(Initializer n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "{"
		 * f1 ::= InitializerList()
		 * f2 ::= ( "," )?
		 * f3 ::= "}"
		 */
		@Override
		public void visit(ArrayInitializer n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((InitializerList) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= Initializer()
		 * f1 ::= ( "," Initializer() )*
		 */
		@Override
		public void visit(InitializerList n) {
			if (n.getF0() == oldNode) {
				n.setF0((Initializer) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= SpecifierQualifierList()
		 * f1 ::= ( AbstractDeclarator() )?
		 */
		@Override
		public void visit(TypeName n) {
			if (n.getF0() == oldNode) {
				n.setF0((SpecifierQualifierList) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= AbstractDeclaratorWithPointer()
		 * | DirectAbstractDeclarator()
		 */
		@Override
		public void visit(AbstractDeclarator n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= Pointer()
		 * f1 ::= ( DirectAbstractDeclarator() )?
		 */
		@Override
		public void visit(AbstractDeclaratorWithPointer n) {
			if (n.getF0() == oldNode) {
				n.setF0((Pointer) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= AbstractDimensionOrParameter()
		 * f1 ::= DimensionOrParameterList()
		 */
		@Override
		public void visit(DirectAbstractDeclarator n) {
			if (n.getF0() == oldNode) {
				n.setF0((AbstractDimensionOrParameter) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((DimensionOrParameterList) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= AbstractDeclaratorClosed()
		 * | DimensionSize()
		 * | ParameterTypeListClosed()
		 */
		@Override
		public void visit(AbstractDimensionOrParameter n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "("
		 * f1 ::= AbstractDeclarator()
		 * f2 ::= ")"
		 */
		@Override
		public void visit(AbstractDeclaratorClosed n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((AbstractDeclarator) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( ADimensionOrParameter() )*
		 */
		@Override
		public void visit(DimensionOrParameterList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= DimensionSize()
		 * | ParameterTypeListClosed()
		 */
		@Override
		public void visit(ADimensionOrParameter n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 */
		@Override
		public void visit(TypedefName n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( LabeledStatement() | ExpressionStatement() |
		 * CompoundStatement()
		 * | SelectionStatement() | IterationStatement() | JumpStatement() |
		 * UnknownPragma() | OmpConstruct() | OmpDirective() | UnknownCpp() )
		 */
		@Override
		public void visit(Statement n) {
			if (n.getStmtF0() == oldNode) {
				n.setStmtF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "#"
		 * f1 ::= <UNKNOWN_CPP>
		 */
		@Override
		public void visit(UnknownCpp n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <OMP_CR>
		 * | <OMP_NL>
		 */
		@Override
		public void visit(OmpEol n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ParallelConstruct()
		 * | ForConstruct()
		 * | SectionsConstruct()
		 * | SingleConstruct()
		 * | ParallelForConstruct()
		 * | ParallelSectionsConstruct()
		 * | TaskConstruct()
		 * | MasterConstruct()
		 * | CriticalConstruct()
		 * | AtomicConstruct()
		 * | OrderedConstruct()
		 */
		@Override
		public void visit(OmpConstruct n) {
			if (n.getOmpConsF0() == oldNode) {
				n.setOmpConsF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= BarrierDirective()
		 * | TaskwaitDirective()
		 * | TaskyieldDirective()
		 * | FlushDirective()
		 */
		@Override
		public void visit(OmpDirective n) {
			if (n.getOmpDirF0() == oldNode) {
				n.setOmpDirF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= ParallelDirective()
		 * f2 ::= Statement()
		 */
		@Override
		public void visit(ParallelConstruct n) {
			if (n.getParConsF0() == oldNode) {
				n.setParConsF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getParConsF1() == oldNode) {
				n.setParConsF1((ParallelDirective) newNode);
				replaced = true;
				return;
			}
			if (n.getParConsF2() == oldNode) {
				n.setParConsF2((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "#"
		 * f1 ::= <PRAGMA>
		 * f2 ::= <OMP>
		 */
		@Override
		public void visit(OmpPragma n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "#"
		 * f1 ::= <PRAGMA>
		 * f2 ::= <UNKNOWN_CPP>
		 */
		@Override
		public void visit(UnknownPragma n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <PARALLEL>
		 * f1 ::= UniqueParallelOrDataClauseList()
		 * f2 ::= OmpEol()
		 */
		@Override
		public void visit(ParallelDirective n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((UniqueParallelOrDataClauseList) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpEol) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( AUniqueParallelOrDataClause() )*
		 */
		@Override
		public void visit(UniqueParallelOrDataClauseList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= UniqueParallelClause()
		 * | DataClause()
		 */
		@Override
		public void visit(AUniqueParallelOrDataClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= IfClause()
		 * | NumThreadsClause()
		 */
		@Override
		public void visit(UniqueParallelClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IF>
		 * f1 ::= "("
		 * f2 ::= Expression()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(IfClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <NUM_THREADS>
		 * f1 ::= "("
		 * f2 ::= Expression()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(NumThreadsClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPrivateClause()
		 * | OmpFirstPrivateClause()
		 * | OmpLastPrivateClause()
		 * | OmpSharedClause()
		 * | OmpCopyinClause()
		 * | OmpDfltSharedClause()
		 * | OmpDfltNoneClause()
		 * | OmpReductionClause()
		 */
		@Override
		public void visit(DataClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <PRIVATE>
		 * f1 ::= "("
		 * f2 ::= VariableList()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(OmpPrivateClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((VariableList) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <FIRSTPRIVATE>
		 * f1 ::= "("
		 * f2 ::= VariableList()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(OmpFirstPrivateClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((VariableList) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <LASTPRIVATE>
		 * f1 ::= "("
		 * f2 ::= VariableList()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(OmpLastPrivateClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((VariableList) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <SHARED>
		 * f1 ::= "("
		 * f2 ::= VariableList()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(OmpSharedClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((VariableList) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <COPYIN>
		 * f1 ::= "("
		 * f2 ::= VariableList()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(OmpCopyinClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((VariableList) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <DFLT>
		 * f1 ::= "("
		 * f2 ::= <SHARED>
		 * f3 ::= ")"
		 */
		@Override
		public void visit(OmpDfltSharedClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <DFLT>
		 * f1 ::= "("
		 * f2 ::= <NONE>
		 * f3 ::= ")"
		 */
		@Override
		public void visit(OmpDfltNoneClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <REDUCTION>
		 * f1 ::= "("
		 * f2 ::= ReductionOp()
		 * f3 ::= ":"
		 * f4 ::= VariableList()
		 * f5 ::= ")"
		 */
		@Override
		public void visit(OmpReductionClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((ReductionOp) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((VariableList) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= ForDirective()
		 * f2 ::= OmpForHeader()
		 * f3 ::= Statement()
		 */
		@Override
		public void visit(ForConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((ForDirective) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpForHeader) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <FOR>
		 * f1 ::= UniqueForOrDataOrNowaitClauseList()
		 * f2 ::= OmpEol()
		 */
		@Override
		public void visit(ForDirective n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((UniqueForOrDataOrNowaitClauseList) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpEol) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( AUniqueForOrDataOrNowaitClause() )*
		 */
		@Override
		public void visit(UniqueForOrDataOrNowaitClauseList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= UniqueForClause()
		 * | DataClause()
		 * | NowaitClause()
		 */
		@Override
		public void visit(AUniqueForOrDataOrNowaitClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <NOWAIT>
		 */
		@Override
		public void visit(NowaitClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <ORDERED>
		 * | UniqueForClauseSchedule()
		 * | UniqueForCollapse()
		 */
		@Override
		public void visit(UniqueForClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <COLLAPSE>
		 * f1 ::= "("
		 * f2 ::= Expression()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(UniqueForCollapse n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <SCHEDULE>
		 * f1 ::= "("
		 * f2 ::= ScheduleKind()
		 * f3 ::= ( "," Expression() )?
		 * f4 ::= ")"
		 */
		@Override
		public void visit(UniqueForClauseSchedule n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((ScheduleKind) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <STATIC>
		 * | <DYNAMIC>
		 * | <GUIDED>
		 * | <RUNTIME>
		 */
		@Override
		public void visit(ScheduleKind n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <FOR>
		 * f1 ::= "("
		 * f2 ::= OmpForInitExpression()
		 * f3 ::= ";"
		 * f4 ::= OmpForCondition()
		 * f5 ::= ";"
		 * f6 ::= OmpForReinitExpression()
		 * f7 ::= ")"
		 */
		@Override
		public void visit(OmpForHeader n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpForInitExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((OmpForCondition) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF6() == oldNode) {
				n.setF6((OmpForReinitExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF7() == oldNode) {
				n.setF7((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "="
		 * f2 ::= Expression()
		 */
		@Override
		public void visit(OmpForInitExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpForLTCondition()
		 * | OmpForLECondition()
		 * | OmpForGTCondition()
		 * | OmpForGECondition()
		 */
		@Override
		public void visit(OmpForCondition n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "<"
		 * f2 ::= Expression()
		 */
		@Override
		public void visit(OmpForLTCondition n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "<="
		 * f2 ::= Expression()
		 */
		@Override
		public void visit(OmpForLECondition n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= ">"
		 * f2 ::= Expression()
		 */
		@Override
		public void visit(OmpForGTCondition n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= ">="
		 * f2 ::= Expression()
		 */
		@Override
		public void visit(OmpForGECondition n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= PostIncrementId()
		 * | PostDecrementId()
		 * | PreIncrementId()
		 * | PreDecrementId()
		 * | ShortAssignPlus()
		 * | ShortAssignMinus()
		 * | OmpForAdditive()
		 * | OmpForSubtractive()
		 * | OmpForMultiplicative()
		 */
		@Override
		public void visit(OmpForReinitExpression n) {
			if (n.getOmpForReinitF0() == oldNode) {
				n.setOmpForReinitF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "++"
		 */
		@Override
		public void visit(PostIncrementId n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "--"
		 */
		@Override
		public void visit(PostDecrementId n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "++"
		 * f1 ::= <IDENTIFIER>
		 */
		@Override
		public void visit(PreIncrementId n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "--"
		 * f1 ::= <IDENTIFIER>
		 */
		@Override
		public void visit(PreDecrementId n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "+="
		 * f2 ::= Expression()
		 */
		@Override
		public void visit(ShortAssignPlus n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "-="
		 * f2 ::= Expression()
		 */
		@Override
		public void visit(ShortAssignMinus n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "="
		 * f2 ::= <IDENTIFIER>
		 * f3 ::= "+"
		 * f4 ::= AdditiveExpression()
		 */
		@Override
		public void visit(OmpForAdditive n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((AdditiveExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "="
		 * f2 ::= <IDENTIFIER>
		 * f3 ::= "-"
		 * f4 ::= AdditiveExpression()
		 */
		@Override
		public void visit(OmpForSubtractive n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((AdditiveExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= "="
		 * f2 ::= MultiplicativeExpression()
		 * f3 ::= "+"
		 * f4 ::= <IDENTIFIER>
		 */
		@Override
		public void visit(OmpForMultiplicative n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((MultiplicativeExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <SECTIONS>
		 * f2 ::= NowaitDataClauseList()
		 * f3 ::= OmpEol()
		 * f4 ::= SectionsScope()
		 */
		@Override
		public void visit(SectionsConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NowaitDataClauseList) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((SectionsScope) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( ANowaitDataClause() )*
		 */
		@Override
		public void visit(NowaitDataClauseList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= NowaitClause()
		 * | DataClause()
		 */
		@Override
		public void visit(ANowaitDataClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "{"
		 * f1 ::= ( Statement() )?
		 * f2 ::= ( ASection() )*
		 * f3 ::= "}"
		 */
		@Override
		public void visit(SectionsScope n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeListOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <SECTION>
		 * f2 ::= OmpEol()
		 * f3 ::= Statement()
		 */
		@Override
		public void visit(ASection n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <SINGLE>
		 * f2 ::= SingleClauseList()
		 * f3 ::= OmpEol()
		 * f4 ::= Statement()
		 */
		@Override
		public void visit(SingleConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((SingleClauseList) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( ASingleClause() )*
		 */
		@Override
		public void visit(SingleClauseList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= NowaitClause()
		 * | DataClause()
		 * | OmpCopyPrivateClause()
		 */
		@Override
		public void visit(ASingleClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <COPYPRIVATE>
		 * f1 ::= "("
		 * f2 ::= VariableList()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(OmpCopyPrivateClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((VariableList) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <TASK>
		 * f2 ::= ( TaskClause() )*
		 * f3 ::= OmpEol()
		 * f4 ::= Statement()
		 */
		@Override
		public void visit(TaskConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeListOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= DataClause()
		 * | UniqueTaskClause()
		 */
		@Override
		public void visit(TaskClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= IfClause()
		 * | FinalClause()
		 * | UntiedClause()
		 * | MergeableClause()
		 */
		@Override
		public void visit(UniqueTaskClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <FINAL>
		 * f1 ::= "("
		 * f2 ::= Expression()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(FinalClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <UNTIED>
		 */
		@Override
		public void visit(UntiedClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <MERGEABLE>
		 */
		@Override
		public void visit(MergeableClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <PARALLEL>
		 * f2 ::= <FOR>
		 * f3 ::= UniqueParallelOrUniqueForOrDataClauseList()
		 * f4 ::= OmpEol()
		 * f5 ::= OmpForHeader()
		 * f6 ::= Statement()
		 */
		@Override
		public void visit(ParallelForConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((UniqueParallelOrUniqueForOrDataClauseList) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((OmpForHeader) newNode);
				replaced = true;
				return;
			}
			if (n.getF6() == oldNode) {
				n.setF6((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( AUniqueParallelOrUniqueForOrDataClause() )*
		 */
		@Override
		public void visit(UniqueParallelOrUniqueForOrDataClauseList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= UniqueParallelClause()
		 * | UniqueForClause()
		 * | DataClause()
		 */
		@Override
		public void visit(AUniqueParallelOrUniqueForOrDataClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <PARALLEL>
		 * f2 ::= <SECTIONS>
		 * f3 ::= UniqueParallelOrDataClauseList()
		 * f4 ::= OmpEol()
		 * f5 ::= SectionsScope()
		 */
		@Override
		public void visit(ParallelSectionsConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((UniqueParallelOrDataClauseList) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((SectionsScope) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <MASTER>
		 * f2 ::= OmpEol()
		 * f3 ::= Statement()
		 */
		@Override
		public void visit(MasterConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <CRITICAL>
		 * f2 ::= ( RegionPhrase() )?
		 * f3 ::= OmpEol()
		 * f4 ::= Statement()
		 */
		@Override
		public void visit(CriticalConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "("
		 * f1 ::= <IDENTIFIER>
		 * f2 ::= ")"
		 */
		@Override
		public void visit(RegionPhrase n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <ATOMIC>
		 * f2 ::= ( AtomicClause() )?
		 * f3 ::= OmpEol()
		 * f4 ::= Statement ()
		 */
		@Override
		public void visit(AtomicConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((ExpressionStatement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <READ>
		 * | <WRITE>
		 * | <UPDATE>
		 * | <CAPTURE>
		 */
		@Override
		public void visit(AtomicClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <FLUSH>
		 * f2 ::= ( FlushVars() )?
		 * f3 ::= OmpEol()
		 */
		@Override
		public void visit(FlushDirective n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((OmpEol) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "("
		 * f1 ::= VariableList()
		 * f2 ::= ")"
		 */
		@Override
		public void visit(FlushVars n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((VariableList) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <ORDERED>
		 * f2 ::= OmpEol()
		 * f3 ::= Statement()
		 */
		@Override
		public void visit(OrderedConstruct n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpEol) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <BARRIER>
		 * f2 ::= OmpEol()
		 */
		@Override
		public void visit(BarrierDirective n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpEol) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <TASKWAIT>
		 * f2 ::= OmpEol()
		 */
		@Override
		public void visit(TaskwaitDirective n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpEol) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <TASKYIELD>
		 * f2 ::= OmpEol()
		 */
		@Override
		public void visit(TaskyieldDirective n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((OmpEol) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <THREADPRIVATE>
		 * f2 ::= "("
		 * f3 ::= VariableList()
		 * f4 ::= ")"
		 * f5 ::= OmpEol()
		 */
		@Override
		public void visit(ThreadPrivateDirective n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((VariableList) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((OmpEol) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= OmpPragma()
		 * f1 ::= <DECLARE>
		 * f2 ::= <REDUCTION>
		 * f3 ::= "("
		 * f4 ::= ReductionOp()
		 * f5 ::= ":"
		 * f6 ::= ReductionTypeList()
		 * f7 ::= ":"
		 * f8 ::= Expression()
		 * f9 ::= ")"
		 * f10 ::= ( InitializerClause() )?
		 * f11 ::= OmpEol()
		 */
		@Override
		public void visit(DeclareReductionDirective n) {
			if (n.getF0() == oldNode) {
				n.setF0((OmpPragma) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((ReductionOp) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF6() == oldNode) {
				n.setF6((ReductionTypeList) newNode);
				replaced = true;
				return;
			}
			if (n.getF7() == oldNode) {
				n.setF7((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF8() == oldNode) {
				n.setF8((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF9() == oldNode) {
				n.setF9((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF10() == oldNode) {
				n.setF10((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF11() == oldNode) {
				n.setF11((OmpEol) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( TypeSpecifier() )*
		 */
		@Override
		public void visit(ReductionTypeList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= AssignInitializerClause()
		 * | ArgumentInitializerClause()
		 */
		@Override
		public void visit(InitializerClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <INITIALIZER>
		 * f1 ::= "("
		 * f2 ::= <IDENTIFIER>
		 * f3 ::= "="
		 * f4 ::= Initializer()
		 * f5 ::= ")"
		 */
		@Override
		public void visit(AssignInitializerClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((Initializer) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <INITIALIZER>
		 * f1 ::= "("
		 * f2 ::= <IDENTIFIER>
		 * f3 ::= "("
		 * f4 ::= ExpressionList()
		 * f5 ::= ")"
		 * f6 ::= ")"
		 */
		@Override
		public void visit(ArgumentInitializerClause n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((ExpressionList) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF6() == oldNode) {
				n.setF6((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * | "+"
		 * | "*"
		 * | "-"
		 * | "&"
		 * | "^"
		 * | "|"
		 * | "||"
		 * | "&&"
		 */
		@Override
		public void visit(ReductionOp n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= ( "," <IDENTIFIER> )*
		 */
		@Override
		public void visit(VariableList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= SimpleLabeledStatement()
		 * | CaseLabeledStatement()
		 * | DefaultLabeledStatement()
		 */
		@Override
		public void visit(LabeledStatement n) {
			if (n.getLabStmtF0() == oldNode) {
				n.setLabStmtF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * f1 ::= ":"
		 * f2 ::= Statement()
		 */
		@Override
		public void visit(SimpleLabeledStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <CASE>
		 * f1 ::= ConstantExpression()
		 * f2 ::= ":"
		 * f3 ::= Statement()
		 */
		@Override
		public void visit(CaseLabeledStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((ConstantExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <DFLT>
		 * f1 ::= ":"
		 * f2 ::= Statement()
		 */
		@Override
		public void visit(DefaultLabeledStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( Expression() )?
		 * f1 ::= ";"
		 */
		@Override
		public void visit(ExpressionStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "{"
		 * f1 ::= ( CompoundStatementElement() )*
		 * f2 ::= "}"
		 */
		@Override
		public void visit(CompoundStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= Declaration()
		 * | Statement()
		 */
		@Override
		public void visit(CompoundStatementElement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= IfStatement()
		 * | SwitchStatement()
		 */
		@Override
		public void visit(SelectionStatement n) {
			if (n.getSelStmtF0() == oldNode) {
				n.setSelStmtF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IF>
		 * f1 ::= "("
		 * f2 ::= Expression()
		 * f3 ::= ")"
		 * f4 ::= Statement()
		 * f5 ::= ( <ELSE> Statement() )?
		 */
		@Override
		public void visit(IfStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((Statement) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <SWITCH>
		 * f1 ::= "("
		 * f2 ::= Expression()
		 * f3 ::= ")"
		 * f4 ::= Statement()
		 */
		@Override
		public void visit(SwitchStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= WhileStatement()
		 * | DoStatement()
		 * | ForStatement()
		 */
		@Override
		public void visit(IterationStatement n) {
			if (n.getItStmtF0() == oldNode) {
				n.setItStmtF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <WHILE>
		 * f1 ::= "("
		 * f2 ::= Expression()
		 * f3 ::= ")"
		 * f4 ::= Statement()
		 */
		@Override
		public void visit(WhileStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <DO>
		 * f1 ::= Statement()
		 * f2 ::= <WHILE>
		 * f3 ::= "("
		 * f4 ::= Expression()
		 * f5 ::= ")"
		 * f6 ::= ";"
		 */
		@Override
		public void visit(DoStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((Statement) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF6() == oldNode) {
				n.setF6((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <FOR>
		 * f1 ::= "("
		 * f2 ::= ( Expression() )?
		 * f3 ::= ";"
		 * f4 ::= ( Expression() )?
		 * f5 ::= ";"
		 * f6 ::= ( Expression() )?
		 * f7 ::= ")"
		 * f8 ::= Statement()
		 */
		@Override
		public void visit(ForStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF4() == oldNode) {
				n.setF4((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF5() == oldNode) {
				n.setF5((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF6() == oldNode) {
				n.setF6((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF7() == oldNode) {
				n.setF7((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF8() == oldNode) {
				n.setF8((Statement) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= GotoStatement()
		 * | ContinueStatement()
		 * | BreakStatement()
		 * | ReturnStatement()
		 */
		@Override
		public void visit(JumpStatement n) {
			if (n.getJumpStmtF0() == oldNode) {
				n.setJumpStmtF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <GOTO>
		 * f1 ::= <IDENTIFIER>
		 * f2 ::= ";"
		 */
		@Override
		public void visit(GotoStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <CONTINUE>
		 * f1 ::= ";"
		 */
		@Override
		public void visit(ContinueStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <BREAK>
		 * f1 ::= ";"
		 */
		@Override
		public void visit(BreakStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <RETURN>
		 * f1 ::= ( Expression() )?
		 * f2 ::= ";"
		 */
		@Override
		public void visit(ReturnStatement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= AssignmentExpression()
		 * f1 ::= ( "," AssignmentExpression() )*
		 */
		@Override
		public void visit(Expression n) {
			if (n.getExpF0() == oldNode) {
				n.setExpF0((AssignmentExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getExpF1() == oldNode) {
				n.setExpF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= NonConditionalExpression()
		 * | ConditionalExpression()
		 */
		@Override
		public void visit(AssignmentExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= UnaryExpression()
		 * f1 ::= AssignmentOperator()
		 * f2 ::= AssignmentExpression()
		 */
		@Override
		public void visit(NonConditionalExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((UnaryExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((AssignmentOperator) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((AssignmentExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "="
		 * | "*="
		 * | "/="
		 * | "%="
		 * | "+="
		 * | "-="
		 * | "<<="
		 * | ">>="
		 * | "&="
		 * | "^="
		 * | "|="
		 */
		@Override
		public void visit(AssignmentOperator n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= LogicalORExpression()
		 * f1 ::= ( "?" Expression() ":" ConditionalExpression() )?
		 */
		@Override
		public void visit(ConditionalExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((LogicalORExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ConditionalExpression()
		 */
		@Override
		public void visit(ConstantExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((ConditionalExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= LogicalANDExpression()
		 * f1 ::= ( "||" LogicalORExpression() )?
		 */
		@Override
		public void visit(LogicalORExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((LogicalANDExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= InclusiveORExpression()
		 * f1 ::= ( "&&" LogicalANDExpression() )?
		 */
		@Override
		public void visit(LogicalANDExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((InclusiveORExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ExclusiveORExpression()
		 * f1 ::= ( "|" InclusiveORExpression() )?
		 */
		@Override
		public void visit(InclusiveORExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((ExclusiveORExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ANDExpression()
		 * f1 ::= ( "^" ExclusiveORExpression() )?
		 */
		@Override
		public void visit(ExclusiveORExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((ANDExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= EqualityExpression()
		 * f1 ::= ( "&" ANDExpression() )?
		 */
		@Override
		public void visit(ANDExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((EqualityExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= RelationalExpression()
		 * f1 ::= ( EqualOptionalExpression() )?
		 */
		@Override
		public void visit(EqualityExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((RelationalExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= EqualExpression()
		 * | NonEqualExpression()
		 */
		@Override
		public void visit(EqualOptionalExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "=="
		 * f1 ::= EqualityExpression()
		 */
		@Override
		public void visit(EqualExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((EqualityExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "!="
		 * f1 ::= EqualityExpression()
		 */
		@Override
		public void visit(NonEqualExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((EqualityExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ShiftExpression()
		 * f1 ::= ( RelationalOptionalExpression() )?
		 */
		@Override
		public void visit(RelationalExpression n) {
			if (n.getRelExpF0() == oldNode) {
				n.setRelExpF0((ShiftExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getRelExpF1() == oldNode) {
				n.setRelExpF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= RelationalLTExpression()
		 * | RelationalGTExpression()
		 * | RelationalLEExpression()
		 * | RelationalGEExpression()
		 */
		@Override
		public void visit(RelationalOptionalExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "<"
		 * f1 ::= RelationalExpression()
		 */
		@Override
		public void visit(RelationalLTExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((RelationalExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ">"
		 * f1 ::= RelationalExpression()
		 */
		@Override
		public void visit(RelationalGTExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((RelationalExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "<="
		 * f1 ::= RelationalExpression()
		 */
		@Override
		public void visit(RelationalLEExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((RelationalExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ">="
		 * f1 ::= RelationalExpression()
		 */
		@Override
		public void visit(RelationalGEExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((RelationalExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= AdditiveExpression()
		 * f1 ::= ( ShiftOptionalExpression() )?
		 */
		@Override
		public void visit(ShiftExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((AdditiveExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ShiftLeftExpression()
		 * | ShiftRightExpression()
		 */
		@Override
		public void visit(ShiftOptionalExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ">>"
		 * f1 ::= ShiftExpression()
		 */
		@Override
		public void visit(ShiftLeftExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((ShiftExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "<<"
		 * f1 ::= ShiftExpression()
		 */
		@Override
		public void visit(ShiftRightExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((ShiftExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= MultiplicativeExpression()
		 * f1 ::= ( AdditiveOptionalExpression() )?
		 */
		@Override
		public void visit(AdditiveExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((MultiplicativeExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= AdditivePlusExpression()
		 * | AdditiveMinusExpression()
		 */
		@Override
		public void visit(AdditiveOptionalExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "+"
		 * f1 ::= AdditiveExpression()
		 */
		@Override
		public void visit(AdditivePlusExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((AdditiveExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "-"
		 * f1 ::= AdditiveExpression()
		 */
		@Override
		public void visit(AdditiveMinusExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((AdditiveExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= CastExpression()
		 * f1 ::= ( MultiplicativeOptionalExpression() )?
		 */
		@Override
		public void visit(MultiplicativeExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((CastExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= MultiplicativeMultiExpression()
		 * | MultiplicativeDivExpression()
		 * | MultiplicativeModExpression()
		 */
		@Override
		public void visit(MultiplicativeOptionalExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "*"
		 * f1 ::= MultiplicativeExpression()
		 */
		@Override
		public void visit(MultiplicativeMultiExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((MultiplicativeExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "/"
		 * f1 ::= MultiplicativeExpression()
		 */
		@Override
		public void visit(MultiplicativeDivExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((MultiplicativeExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "%"
		 * f1 ::= MultiplicativeExpression()
		 */
		@Override
		public void visit(MultiplicativeModExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((MultiplicativeExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= CastExpressionTyped()
		 * | UnaryExpression()
		 */
		@Override
		public void visit(CastExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "("
		 * f1 ::= TypeName()
		 * f2 ::= ")"
		 * f3 ::= CastExpression()
		 */
		@Override
		public void visit(CastExpressionTyped n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((TypeName) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((CastExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= UnaryExpressionPreIncrement()
		 * | UnaryExpressionPreDecrement()
		 * | UnarySizeofExpression()
		 * | UnaryCastExpression()
		 * | PostfixExpression()
		 */
		@Override
		public void visit(UnaryExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "++"
		 * f1 ::= UnaryExpression()
		 */
		@Override
		public void visit(UnaryExpressionPreIncrement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((UnaryExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "--"
		 * f1 ::= UnaryExpression()
		 */
		@Override
		public void visit(UnaryExpressionPreDecrement n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((UnaryExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= UnaryOperator()
		 * f1 ::= CastExpression()
		 */
		@Override
		public void visit(UnaryCastExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((UnaryOperator) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((CastExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= SizeofTypeName()
		 * | SizeofUnaryExpression()
		 */
		@Override
		public void visit(UnarySizeofExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <SIZEOF>
		 * f1 ::= UnaryExpression()
		 */
		@Override
		public void visit(SizeofUnaryExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((UnaryExpression) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <SIZEOF>
		 * f1 ::= "("
		 * f2 ::= TypeName()
		 * f3 ::= ")"
		 */
		@Override
		public void visit(SizeofTypeName n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((TypeName) newNode);
				replaced = true;
				return;
			}
			if (n.getF3() == oldNode) {
				n.setF3((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "&"
		 * | "*"
		 * | "+"
		 * | "-"
		 * | "~"
		 * | "!"
		 */
		@Override
		public void visit(UnaryOperator n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= PrimaryExpression()
		 * f1 ::= PostfixOperationsList()
		 */
		@Override
		public void visit(PostfixExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((PrimaryExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((PostfixOperationsList) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= ( APostfixOperation() )*
		 */
		@Override
		public void visit(PostfixOperationsList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= BracketExpression()
		 * | ArgumentList()
		 * | DotId()
		 * | ArrowId()
		 * | PlusPlus()
		 * | MinusMinus()
		 */
		@Override
		public void visit(APostfixOperation n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "++"
		 */
		@Override
		public void visit(PlusPlus n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "--"
		 */
		@Override
		public void visit(MinusMinus n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "["
		 * f1 ::= Expression()
		 * f2 ::= "]"
		 */
		@Override
		public void visit(BracketExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "("
		 * f1 ::= ( ExpressionList() )?
		 * f2 ::= ")"
		 */
		@Override
		public void visit(ArgumentList n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeOptional) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "."
		 * f1 ::= <IDENTIFIER>
		 */
		@Override
		public void visit(DotId n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "->"
		 * f1 ::= <IDENTIFIER>
		 */
		@Override
		public void visit(ArrowId n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <IDENTIFIER>
		 * | Constant()
		 * | ExpressionClosed()
		 */
		@Override
		public void visit(PrimaryExpression n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= "("
		 * f1 ::= Expression()
		 * f2 ::= ")"
		 */
		@Override
		public void visit(ExpressionClosed n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeToken) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((Expression) newNode);
				replaced = true;
				return;
			}
			if (n.getF2() == oldNode) {
				n.setF2((NodeToken) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= AssignmentExpression()
		 * f1 ::= ( "," AssignmentExpression() )*
		 */
		@Override
		public void visit(ExpressionList n) {
			if (n.getF0() == oldNode) {
				n.setF0((AssignmentExpression) newNode);
				replaced = true;
				return;
			}
			if (n.getF1() == oldNode) {
				n.setF1((NodeListOptional) newNode);
				replaced = true;
				return;
			}
		}

		/**
		 * f0 ::= <INTEGER_LITERAL>
		 * | <FLOATING_POINT_LITERAL>
		 * | <CHARACTER_LITERAL>
		 * | ( <STRING_LITERAL> )+
		 */
		@Override
		public void visit(Constant n) {
			if (n.getF0() == oldNode) {
				n.setF0((NodeChoice) newNode);
				replaced = true;
				return;
			}
		}

		@Override
		public void visit(DummyFlushDirective n) {
			assert (false);
		}

		@Override
		public void visit(SimplePrimaryExpression n) {
			assert (false);
		}

		@Override
		public void visit(CallStatement n) {
			assert (false);
		}

		@Override
		public void visit(PreCallNode n) {
			assert (false);
		}

		@Override
		public void visit(PostCallNode n) {
			assert (false);
		}
	}
}
