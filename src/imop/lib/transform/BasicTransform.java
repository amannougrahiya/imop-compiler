...
public class BasicTransform {
    ...
    public static boolean crudeInsertNewAfterBase(Node baseNode, Node newNode) {
        ...
        if (tempNewNode == null) {
            // Create a wrapper node for newNode, of the type same as that of the wrapper of the baseNode
            newNode = FrontEnd.parseAlone(newNode.getInfo().getString(), baseNode.getClass());
        } else {
            newNode = tempNewNode;
        }
    ...
    public static boolean crudeSwapBaseWithNew(Node baseNode, Node newNode) {
        ...
        // Note: The following code works for the case when newNode is an immediate successor of the baseNode.
     ...
     */
    public static <T extends Node> boolean crudeReplaceOldWithNew(T baseNode, T newNode) throws IllegalArgumentException, IllegalAccessException {
        return CrudeReplaceNode.replace(baseNode, newNode);
    }

    /**
     * Splits compound statement into finer pieces. e.g., {S1; S2; S3;} is split into {S1;} {S2;} {S3;}
     *
     * @param node
     *
    ...
    public static boolean splitCompoundStatement(CompoundStatement node) {
        ...
        for (Node element : node.getF1().getNodes()) {
            CompoundStatementElement newCSE = FrontEnd.parseAlone("{" + element.getInfo().getString() + "}", CompoundStatementElement.class);
            newCSE.getF0().setChoice(element);
            newList.add(newCSE);
        }

        // Change the node $node$ with the newly created list of compoundStatementElements.
        ...
        // Now, update the analysis outputs for the newly created vector of compound statements.
    ...
    /**
     * Returns a copy of the given node with its IDs renamed as per the {@code nameMap}.
     * <br>
    ...
    public static Node obtainRenamedNode(Node node, HashMap<String, String> nameMap) {
        ...
        if (node instanceof DummyFlushDirective) {
            ...
            // This needs to be done as a "Statement" since "inside" node may have labels, due to which it might not get parsed as its CFG type.
            ...
            // Make sure that same post-call node is transferred from old to new call-statement,
            // so that the list in renamingOnNodes.keySet() remains valid.
            CallStatement oldCallStmt = oldPre.getParent();
            replacementNode = new CallStatement(oldCallStmt.getFunctionDesignatorNode(), newPre, oldCallStmt.getPostCallNode());
            ...
            // Replace the enclosing call-statement with the new call-statement.
            // Make sure that same pre-call node is transferred from old to new call-statement,
            // so that the list in renamingOnNodes.keySet() remains valid.
            CallStatement oldCallStmt = oldPost.getParent();
            replacementNode = new CallStatement(oldCallStmt.getFunctionDesignatorNode(), oldCallStmt.getPreCallNode(), newPost);
        } else {
            String replacementString = StringGetter.getRenamedString(node, nameMap);
    ...
    /**
     * Replaces {@code node} by a new node that has its IDs renamed as per the {@code nameMap}.
     *
     * @param node
     *         old node in which replacements as per the nameMaps have to be done.
     * @param nameMap
     *         a map from old id names to new id names.
     *
     * @return new node with replacements done as per the nameMap.
        ...
        for (String strSrc : nameMap.keySet()) {
            for (String strDest : nameMap.values()) {
                if (strSrc.equals(strDest)) {
                    Thread.dumpStack();
                    Misc.exitDueToError("Cannot perform conlifcting renaming for " + strSrc + " to " + nameMap.get(strSrc) + " when another renaming has that name as the destination.");
                }
            }
        }
    }
    ...
    public static List<SideEffect> pushDeclarationUp(Declaration decl) {
        ...
        if (scope == null) {
            Misc.warnDueToLackOfFeature("Could not find any enclosing block for the declaration that needs to be moved higher.", null);
    ...
    /**
     * If the initializer contains any side-effects, this method updates the declaration such that the initializer has
     * been simplified. This method returns true, if the simplification was performed.
     *
     * @param decl
    ...
    public static boolean removeSideEffectsFromInitializer(Declaration decl) {
        ...
        // Too bad! Now, we need to simplify the declaration, adjust the indices, and notify.
        ...
        Statement simplifyingExp = FrontEnd.parseAndNormalize(tempName + " = " + init.toString() + ";", Statement.class);
     ...
     *
     * @param node
     *         node from within which empty OpenMP constructs have to be removed.
     */
    ...
    //		for (FunctionDefinition func : Misc.getInheritedEnclosee(program, FunctionDefinition.class)) {
    ...
    //				Statement assign = FrontEnd.parseAndNormalize(newTempName + " = " + predicate + ";", Statement.class);
    //				// Note that assign might not be the best option always. Consider for's e1 and e3.
    ...
    /**
     * Adds {@code predecessor} as the only immediate predecessor of {@code baseNode} on all the paths.
     *
     * @param baseNode
     *         node before which a CFG node has to be inserted.
     * @param predecessor
     *         node which has to be inserted immediately before {@code baseNode}
     *
     * @deprecated
     */
    @Deprecated
    public static void deprecated_insertImmediatePredecessor(Node baseNode, Node predecessor) {
        ...
        Deprecated_ImmediatePredecessorInserter updater = new Deprecated_ImmediatePredecessorInserter(baseNode, predecessor);
    ...
    public static void deprecated_insertImmediateSuccessor(Node baseNode, Node successor) {
        baseNode = Misc.getCFGNodeFor(baseNode);
        Node parentNode = Misc.getEnclosingCFGNonLeafNode(baseNode);
        Deprecated_ImmediatePredecessorInserter updater = new Deprecated_ImmediatePredecessorInserter(baseNode, successor);
        parentNode.accept(updater);
    }
