...
public class DriverModule {
    ...
    public static void askQueries() {
        ...
        for (int i = 0; i < 100; i++) {
            for (DummyFlushDirective dfd1 : Misc.getExactEncloseeList(Program.getRoot(), DummyFlushDirective.class)) {
                for (DummyFlushDirective dfd2 : Misc.getExactEncloseeList(Program.getRoot(), DummyFlushDirective.class)) {
                    long timLoc = System.nanoTime();
                    if (Misc.doIntersect(new HashSet<>(dfd1.getInfo().getNodePhaseInfo().getPhaseSet()), new HashSet<>(dfd2.getInfo().getNodePhaseInfo().getPhaseSet())) &&
    ...
    /**
     * Performs synchronization optimizations on various loop statements in the input program, if required.
     *
     * @param root
    ...
    /**
     * Performs in-place synchronization optimizations in {@code itStmt}, by applying code-percolation, loop unrolling
     * and memory renaming. These translations are performed in a way that guarantees consistency of various
     * data-structures that represent the semantics of the input program.
     *
     * @param itStmt
     *         iteration-statement (a {@link WhileStatement}, a {@link ForStatement}, or a {@link DoStatement}), on
     *         which we need to apply synchronization elimination optimizations.
     *
    ...
    private static boolean run(IterationStatement itStmt) {
        ...
        // Step 4: Process each phase that may have at least one starting barrier in itStmt, until fixed point.
        ...
        do {
            ...
            // Step b. Extract the border node, and remove the associated border node marker.
            ...
            // Step c. Remove anti-dependence across the old and new portion of the loop body.
            ...
            if (!lastStep) {
                //				if (newPhaseCountPerIteration < oldPhaseCountPerIteration || unrollFactor < maxUnrollFactor) {
        ...
        //		for (int i = 1; i < 8; i++) {
        //			IterationStatement copyStmt = FrontEnd.parseAndNormalize(itStmt.toString(), IterationStatement.class);
     ...
     *
     * @param encloser
     *         enclosing node, within which the length of swap-chain, if any, has to be found.
     *
    ...
    /**
     * Obtain clean writes of shared variables in phases present in {@code phaseSet}, within the loop whose contents are
     * {@code itContents}.
     * <br>
     * Note: We skip those clean writes which write to a symbol of complicated type that does not overload "=" operator
     ...
     *
     * @return set of clean-writes (mostly {@link ExpressionStatement} nodes) present in {@code itContents}, occurring
     * in {@code phaseSet}.
     */
    private static Set<Node> extractCleanWrites(Set<Node> itContents, Set<Phase> borderPhases) {
        ...
        for (Phase ph : borderPhases) {
            for (BeginPhasePoint bpp : ph.getBeginPoints()) {
                ...
                outer:
                for (Node reachable : bpp.getReachableNodes()) {
                    ...
                    // Ignore all those nodes which may have successors or predecessors outside the loop.
    ...
    /**
     * This method starts at the {@code borderNode}, and tries to remove an anti-edge across it, by renaming shared
     * variables, if required, up till exit points from this new copy of the original iteration body.
     *
     * @param itStmt
     *         iteration-statement that has recently been added with a new copy of original iteration.
     * @param cleanWrites
     *         set of those nodes which are reachable from BPPs that belong to the itSmt, and that start a phase of
     *         which the first node in the newly added iteration is a part.
     * @param newBorderNode
     *         borderNode that separates the new copy with the next.
     *
     * @return true, if any code change was performed.
     */
    private static boolean performNoUpdateRenaming(IterationStatement itStmt, Set<Node> cleanWrites, Node newBorderNode) {
        ...
        for (Node cleanNode : cleanWrites) {
            ...
            // Make sure that the cleanNode itself is treated as an insideNode, if there exists any other insideNode.
            if (insideNodes.size() != 0) {
                insideNodes.add(cleanNode);
            } else {
                // If there are no nodes of interest for a given cell, make both maps empty for that cell.
                frontierNodes.clear();
            }
        ...
        // Step 2.b Set to obtain a set of identifiers that must be swapped back before any given node.
        ...
        for (Node frontier : swapBackMap.keySet()) {
            ...
            // add the node before the frontier (if there is no data clash). Otherwise, proceed down.
            /*
            ...
            for (String symName : keys) {
                ...
                for (Node succ : frontier.getInfo().getCFGInfo().getInterProceduralLeafSuccessors()) {
                    ...
                    CollectorVisitor.collectNodeSetInGenericGraph(startPoint, endPoints, (n) -> {
                        ...
                        for (NodeWithStack succNode : n.getNode().getInfo().getCFGInfo().getInterProceduralLeafSuccessors(n.getCallStack())) {
    ...
    /**
     * Modifies the iteration-statement {@code itStmt}, by incrementing its unrolling factor by one, using {@code
     * origBody}.
     * <br>
     * Note that this method places a special label marker: "__imopLoopBorder", which can be used to extract the loop
     ...
     */
    private static void unrollLoopWithOriginalBodyAndEnvironmentUpdate(IterationStatement itStmt, Statement origBody, HashMap<Symbol, Symbol> environmentUpdate) {
        // NOTE: It so turns out that environmentalUpdate is not required in this method.
    ...
    private static boolean intraIterationCodePercolation(IterationStatement itStmt) {
        ...
        phaseList.addAll(CollectorVisitor.collectNodeListInGenericGraph(entryPhases, lastPhases, (ph) -> false, (ph) -> {
    ...
    /**
     * This method performs copy elimination on each node in the parent block of {@code itStmt}, followed by dead code
     * removal, in an attempt to reduce the shared dependences.
     *
     * @param itStmt
     *         loop from within whose parent copy elimination has to be performed.
     *
    ...
    /**
     * To obtain the maximum number of abstract phases that one execution of this loop may observe.
     *
     * @param itStmt
     *         loop to be tested.
     *
     * @return maximum number of abstract phases that can be encountered in one execution of this loop.
     */
    public static int phaseCountPerExecution(IterationStatement itStmt) {
        int maxLength = 1;
        Set<NodeWithStack> internalNodes = itStmt.getInfo().getCFGInfo().getIntraTaskCFGLeafContentsOfSameParLevel(new CallStack());
    ...
    /**
     * Obtains all those phases that either start in this loop, or enter through the BeginNode and end in this loop.
     *
     * @param itStmt
     *         loop to be processed.
     *
     * @return set of all those phases that either start in this loop, or enter through the BeginNode and end in this
     * loop.
     */
    @SuppressWarnings("unused")
    @Deprecated
    ...
    @SuppressWarnings("unused")
    @Deprecated
    private static boolean mayDistortEnvironment(CellSet cells, Set<Node> frontiers, IterationStatement itStmt, Node aboveNode, Node belowNode) {
    ...
    /**
     * Checks if there is any anti-dependence across the back-edge of {@code itStmt}, for a shared variable.
     *
     * @param itStmt
     *         loop that needs to be tested.
     *
     * @return true if there exists an anti-dependence on a shared variable across the back-edge of {@code itStmt}.
     */
    @SuppressWarnings("unused")
    @Deprecated
