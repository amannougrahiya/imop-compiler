...
public class SCC implements DFable {
	...
	public SCC(Set<Node> nodes) {
		// We allow generation of only those SCCs that have at least two nodes within them.
	...
	public static void initializeSCC() {
		...
		// Before starting with the Tarjan's algorithm, reset the DFS index of each CFG node in the program.
		for (SCC old : allSCCs) {
			flushSCC(old);
		}
		...
		allSCCs.clear();
		allSCCNodes.clear();
		//		for (Node leaf : Program.getRoot().getInfo().getCFGInfo().getLexicalCFGLeafContents()) {
		...
		SCC.tarjanCount++;
		assert (SCC.internalStack
				.isEmpty()) : "The stack of nodes is not empty at the end of Tarjan's algorithm! Its contents are:\n"
						+ SCC.internalStack;
		...
		//		System.out.println("We still have " + SCC.internalStack.size() + " nodes on the stack!");
	}
	...
	private static void tarjan(Node v, Set<Node> visited) {
		...
		if (v instanceof BarrierDirective) {
			...
			neighbours = vInfo.getInterTaskLeafSuccessorNodes(SVEDimension.SVE_INSENSITIVE);
		}
