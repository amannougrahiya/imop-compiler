...
public abstract class FlowAnalysis<F extends FlowAnalysis.FlowFact> extends GJDepthFirstProcess<F, F> {
	 ...
	 * method stores these nodes in an internal structure for later use.
	 * 
	 * @param updateSeedSet
	...
	/**
	 * This method re-runs this analysis starting with all (still connected)
	 * nodes from nodesToBeUpdated, and resets the isInvalid bit. As a result of
	 * this method invocation, the corresponding IDFA flow-fact of this pass
	 * should match the expected value.<br>
	 * IMPORTANT: For points-to analysis, note that this method will not trigger
	 * any update unless the state of points-to analysis is marked as
	...
	/**
	 * Super-class for all the specializations of {@code FlowFact}s, which are
	 * used
	...
	public static abstract class FlowFact {
		 ...
		 * 
		 * @return
		 *         a String that represents the human-readable form of this
		 *         FlowFact.
		 */
		 ...
		 * overridden to update the receiver such that it is equal to the meet
		 * of
		 * the initial state of the receiver with {@link other}.
		 * 
		 * An implementation can use the set of cells, {@link cellSet}, to
		 * make the meet more precise. The set {@link cellSet} should contain
		 * all
		 * those cells which may contribute to the update in data-flow fact of a
		 * node.
		 * <p>
		 ...
		 *            context.</i></li>
		 *            </ul>
		 * @return true if this flow-fact has been updated as a result of this
		 *         meet
		 *         operation.
		 * 
		 */
	 ...
	 * NOTE: The definitions of this method would already be provided in
	 * specific subclasses.
	 * Client analyses should not override this method.
	 * 
	 * @param funcDef
	 */
	 ...
	 * Note that usually this flow-fact should reflect the state that should
	 * be the IN of the main's body.
	 * 
	 * @return
	 *         FlowFact to be used when a node has no predecessors.
	 */
	 ...
	 * flow-facts of various nodes for this analysis reflect the state of the
	 * program.
	 * 
	 * @return
	 *         true, if the state maybe invalid.
	 */
	 ...
	 * {@link #restartAnalysisFromStoredNodes()}), this method is called first,
	 * so
	 * that the stabilizing pass itself shouldn't go into a recursion were it to
	 * use the getIN and getOUT on the same analysis (e.g., in case of points-to
	 * analysis).
	 */
	 ...
	 * models the effect of the edge on {@code edgeIN} to return
	 * {@code edgeOUT}.
	 * 
	 * @param edgeIN
	 * @param predecessor
	...
	/**
	 * Called before starting of the {@link #restartAnalysisFromStoredNodes()},
	 * this method clears and populates the map
	 * {@link #reachablePredecessorsOfSeeds}, where each entry maps a seed-node
	 * (from {@link #nodesToBeUpdated}) to a set of predecessor nodes that are
	 * reachable from the seed-node.
	 * UPDATE: This method has been intentionally made intra-task, for
	 * efficiency purposes.
	 */
