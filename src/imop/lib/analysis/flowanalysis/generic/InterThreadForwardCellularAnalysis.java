 ...
 */
public abstract class InterThreadForwardCellularAnalysis<F extends CellularDataFlowAnalysis.CellularFlowMap<?>> extends CellularDataFlowAnalysis<F> {
    ...
    //		} else {
    //			return n.getInfo().getCFGInfo().getSCC().getReversePostOrder() + "-" + n.getReversePostOrder() + n;
    //		}
    //	}
    //
    ...
    public void run(FunctionDefinition funcDef) {
        ...
        do {
            ...
            if (nodeToBeAnalysed instanceof BarrierDirective || (nodeToBeAnalysed instanceof EndNode &&
                 ...
                 * New code above.
                 */
                //				typeList += nodeToBeAnalysed.getClass().getSimpleName() + foo(nodeToBeAnalysed) + ";\n ";
        ...
        for (AbstractPhase<?, ?> ph : new HashSet<>(n.getInfo().getNodePhaseInfo().getPhaseSet())) {
            ...
            for (AbstractPhasePointable endingPhasePoint : ph.getEndPoints()) {
                ...
                if (siblingBarrier == n) {
                    ...
                    // OLD CODE: changed = changed | flowFactOUT.merge(siblingFlowFactIN, n.getInfo().getSharedCellsAtNode());
    ...
    protected final void processWhenUpdated(Node node) {
        ...
        if (node instanceof BarrierDirective) {
            if (oldIN == null) {
                // This was the first processing of the barrier, ever. Others would get affected only if the in was changed.
                if (inChanged) {
                    this.addAllSiblingBarriersToWorkList((BarrierDirective) node);
                }
            } else if (oldIN != newIN) {
                // This was the first processing of the barrier in this round. If the objects differ semantically, others would get affected.
                if (!oldIN.equals(newIN)) {
                    this.addAllSiblingBarriersToWorkList((BarrierDirective) node);
                }
            } else {
                // This is any-but-first processing of the barrier in this round. Others would be impacted only if anything changed in IN.
                if (inChanged) {
                    this.addAllSiblingBarriersToWorkList((BarrierDirective) node);
                }
            }
        ...
        for (AbstractPhase<?, ?> ph : new HashSet<>(n.getInfo().getNodePhaseInfo().getPhaseSet())) {
            ...
            for (AbstractPhasePointable endingPhasePoint : ph.getEndPoints()) {
                ...
                if (sibSCCNum < thisBarrNum) {
                    ...
                    // The future SCC barrier would add us whenever its IN changes. For now, ignore it.
                    // Since the future SCC has not been stabilized, let's ignore that barrier, but without
