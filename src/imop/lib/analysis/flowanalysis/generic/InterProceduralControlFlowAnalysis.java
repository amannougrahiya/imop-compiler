...
public abstract class InterProceduralControlFlowAnalysis<F extends FlowAnalysis.FlowFact> extends ControlFlowAnalysis<F> {
    ...
    public final void cloneFlowFactsToAllWithin(Node affectedNode) {
        ...
        for (Node predNode : predecessors) {
            @SuppressWarnings("unchecked") F predOUT = (F) predNode.getInfo().getCurrentOUT(analysisName);
    ...
    /**
     * This method can be overridden to perform any extra merge operations, if needed, on the IN of a {@link
     ...
     * @param flowFactOne
     *         the IN/OUT flow-fact for the implicit assignment of the {@code argument} to the {@code parameter}.
     *
     * @return the OUT/IN flow-fact, as a result of the implicit assignment of the {@code argument} to the {@code
     * parameter}.
     */
