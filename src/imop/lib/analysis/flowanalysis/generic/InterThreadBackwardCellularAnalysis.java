...
public abstract class InterThreadBackwardCellularAnalysis<F extends CellularDataFlowAnalysis.CellularFlowMap<?>> extends CellularDataFlowAnalysis<F> {
    ...
    public final void cloneFlowFactsToAllWithin(Node affectedNode) {
        ...
        Set<IDFAEdge> successors = nodeInfo.getCFGInfo().getInterTaskLeafSuccessorEdges(this.analysisDimension.getSVEDimension());
    ...
    /**
     * Processing that is performed when the program has been updated. Note that OUT objects would be reinitialized for
        ...
        for (AbstractPhase<?, ?> ph : new HashSet<>(n.getInfo().getNodePhaseInfo().getPhaseSet())) {
            for (AbstractPhasePointable endingPhasePoint : ph.getEndPoints()) {
                ...
                if (siblingBarrier == n) {
                    ...
                    // OLD CODE: changed = changed | flowFactIN.merge(siblingFlowFactOUT, n.getInfo().getSharedCellsAtNode());
