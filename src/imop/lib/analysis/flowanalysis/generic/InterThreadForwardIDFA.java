...
public abstract class InterThreadForwardIDFA<F extends FlowFact> extends GJDepthFirstCFG<F, F> {
    ...
    public InterThreadForwardIDFA(AnalysisName analysisName, AnalysisDimension analysisDimension, AnalysisFlowNature flowNature) {
    ...
    /**
     * Obtain flowFact to be used when a node has no predecessors. Note that usually this flow-fact should reflect the
     ...
     *
     * @param funcDef
     *         function-definition on which this analysis has to be performed.
     */
    ...
    /**
     * This method is usually called by getIN and getOUT methods to check if flow-facts of various nodes for this
    ...
    /**
     * After deciding to run the stabilizing pass (by calling {@link #restartAnalysisFromStoredNodes()}), this method is
     * called first, so that the stabilizing pass itself shouldn't go into a recursion were it to use the getIN and
    ...
    /**
     * Given a set of nodes starting which this IDFA pass should be re-run, this method stores these nodes in an
     * internal structure for later use.
     *
     * @param updateSeedSet
     *         set of nodes starting which this IDFA pass should be re-run.
     */
    ...
    /**
     * This method re-runs this analysis starting with all (still connected) nodes from nodesToBeUpdated, and resets the
     * isInvalid bit. As a result of this method invocation, the corresponding IDFA flow-fact of this pass should match
     * the expected value.<br> IMPORTANT: For points-to analysis, note that this method will not trigger any update
     ...
     * just one entry point, and no internal barriers/flushes). Note that since the {@code affectedNode} has been
     * checked to not affect the flow-analysis, we should use the same object for IN as well as OUT of each node.<br>
     * Important: If the OUT of predecessors of the entry node is not stable, we will still use the unstable values for
     * now; not that the stabilization process, whenever triggered, would take care of these nodes as well.
     *
     * @param affectedNode
     *         node which does not seem to affect the analysis (and hence, for which we just copy same IN and OUT to all
     *         its internal nodes).
     */
    ...
    public void cloneFlowFactsToAllWithinForDataFlow(Node affectedNode) {
        ...
        Set<IDFAEdge> predecessors = nodeInfo.getCFGInfo().getInterTaskLeafPredecessorEdges(this.analysisDimension.getSVEDimension());
    ...
    /**
     * Given an edge from {@code predecessor} to {@code successor}, this method models the effect of the edge on {@code
    ...
    /**
     * Called before starting of the {@link #restartAnalysisFromStoredNodes()}, this method clears and populates the map
     * {@link #reachablePredecessorsOfSeeds}, where each entry maps a seed-node (from {@link #nodesToBeUpdated}) to a
    ...
    private void populateReachablePredecessorMap() {
        ...
        for (Node node : this.nodesToBeUpdated) {
            ...
            CollectorVisitor.collectNodeSetInGenericGraph(startPoint, null, n -> {
                if (n.getNode().getInfo().getCFGInfo().getInterProceduralLeafSuccessors(n.getCallStack()).stream().anyMatch(n2 ->
                        n2.getNode() == node)) {
            ...
            //						.getInterTaskLeafSuccessorNodes(n.getCallStack(), this.analysisDimension.getSVEDimension())
    ...
    /**
     * Processing that is performed when no updates have been done to the program. Note that IN objects will never
    ...
    protected void processWhenNotUpdatedForDataFlow(Node node) {
        ...
        if (propagateFurther) {
            if (this.analysisDimension.getProceduralDimension() == ProceduralDimension.INTRA_PROCEDURAL) {
                ...
                for (IDFAEdge idfaEdge : nodeInfo.getCFGInfo().getInterTaskLeafSuccessorEdges(this.analysisDimension.getSVEDimension())) {
    ...
    /**
     * adds missing information from the out of the corresponding precallnode to the given {@code incompleteff}, which
    ...
    private <H extends Immutable> boolean processPostCallNodes(PostCallNode node, F incompleteFF) {
        ...
        //		}
        //		int initialSize = (postIN.flowMap.isUniversal()) ? Integer.MAX_VALUE : postIN.flowMap.size();
    ...
    /**
     * This method removes those entries from the flow-map {@code fullFF} that do not correspond to accessible symbols
     * at the function which the given begin-node, {@code node}, begins.
     *
     * @param node
     *         a begin-node
     * @param fullFF
     *         state of the OUT of the begin-node; note that this object's values may get modified.
     */
    private void processBeginNodes(BeginNode node, F fullFF) {
        if (!(fullFF instanceof CellularFlowMap<?>)) {
            ...
            if (node.getParent() instanceof FunctionDefinition) {
                ...
                //				removalSet.addAll(cellFullFF.flowMap.nonGenericKeySet().stream().filter(c -> !cellsHere.contains(c))
    ...
    /**
     * This method removes those entries from the flow-map {@code fullFF} which correspond to local keys of the scope
     * that the given end-node, {@code node}, ends.
     *
     * @param node
     *         an end-node.
     * @param fullFF
     *         state of OUT of the end-node; note that this object's values may get modified.
     */
    ...
    /**
     * Processing that is performed when the program has been updated. Note that IN objects would be reinitialized for
    ...
    /**
     * f0 ::= OmpPragma() f1 ::= <SECTIONS> f2 ::= NowaitDataClauseList() f3 ::= OmpEol() f4 ::= SectionsScope()
     */
    @Override
    ...
    /**
     * f0 ::= OmpPragma() f1 ::= <SINGLE> f2 ::= SingleClauseList() f3 ::= OmpEol() f4 ::= Statement()
     */
    @Override
    ...
    /**
     * f0 ::= OmpPragma() f1 ::= <TASK> f2 ::= ( TaskClause() )* f3 ::= OmpEol() f4 ::= Statement()
     */
    @Override
    ...
    /**
     * f0 ::= OmpPragma() f1 ::= <PARALLEL> f2 ::= <FOR> f3 ::= UniqueParallelOrUniqueForOrDataClauseList() f4 ::=
    ...
    /**
     * f0 ::= OmpPragma() f1 ::= <PARALLEL> f2 ::= <SECTIONS> f3 ::= UniqueParallelOrDataClauseList() f4 ::= OmpEol() f5
     * ::= SectionsScope()
     */
    @Override
    ...
    /**
     * f0 ::= OmpPragma() f1 ::= <CRITICAL> f2 ::= ( RegionPhrase() )? f3 ::= OmpEol() f4 ::= Statement()
     */
    @Override
    ...
    /**
     * f0 ::= OmpPragma() f1 ::= <ATOMIC> f2 ::= ( AtomicClause() )? f3 ::= OmpEol() f4 ::= Statement()
     */
    @Override
    ...
    /**
     * f0 ::= <FOR> f1 ::= "(" f2 ::= ( Expression() )? f3 ::= ";" f4 ::= ( Expression() )? f5 ::= ";" f6 ::= (
    ...
    /**
     * Given a parameter-declaration {@code parameter}, and a simple-primary-expression {@code argument} that represents
     * the argument for this parameter from some call-site, this method should model the flow-function of the write to
     * the parameter that happens implicitly.
     *
     * @param parameter
     *         a {@code ParameterDeclaration} which needs to be assigned with the {@code argument}.
     * @param argument
     *         a {@code SimplePrimaryExpression} which is assigned to the {@code parameter}.
     * @param flowFactIN
     *         the IN flow-fact for the implicit assignment of the {@code argument} to the {@code parameter}.
     *
     * @return the OUT flow-fact, as a result of the implicit assignment of the {@code argument} to the {@code
     * parameter}.
     */
    ...
    /**
     * f0 ::= PostIncrementId() | PostDecrementId() | PreIncrementId() | PreDecrementId() | ShortAssignPlus() |
     * ShortAssignMinus() | OmpForAdditive() | OmpForSubtractive() | OmpForMultiplicative()
     */
    @Override
    ...
    public final F visit(BarrierDirective n, F flowFactIN) {
        ...
        //		if (changed) {
        //			for (Phase ph : new HashSet<>(n.getInfo().getNodePhaseInfo().getStablePhaseSet())) {
    ...
    /**
     * Obtain a set of successor nodes for this {@code node}, along with symbols via which information may flow on the
     * edge connecting the successors to this {@code node}.
     *
     * @param node
     *         node whose successors have to be found.
     *
     * @return a set of successor nodes for this {@code node}, along with symbols via which information may flow on the
     * edge connecting the successors to this {@code node}.
     *
     * @deprecated
     */
    ...
    /**
     * Obtain a set of predecessor nodes for this {@code node}, along with symbols via which information may flow on the
     * edge connecting the predecessors to this {@code node}.
     *
     * @param node
     *         node whose predecessors have to be found.
     *
     * @return a set of predecessor nodes for this {@code node}, along with symbols via which information may flow on
