...
public abstract class InterThreadBackwardNonCellularAnalysis<F extends FlowAnalysis.FlowFact> extends NonCellularDataFlowAnalysis<F> {
     ...
     * given node are set to be the meet of IN of the successors of the exit point of the node (there must be just one
     * exit point) . Note that since the {@code affectedNode} has been checked to not affect the flow-analysis, we
     * should use the same object for IN as well as OUT of each node.<br> Important: If the IN of successors of the exit
     * node is not stable, we will still use the unstable values for now; not that the stabilization process, whenever
