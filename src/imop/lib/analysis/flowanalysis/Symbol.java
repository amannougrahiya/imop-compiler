...
/**
 * This class represents an abstraction of a variable (or stack-element) in C, along with functions.
 *
...
public class Symbol extends Cell {
    ...
    public Symbol(String name, Type type, Node declaringNode, Scopeable definingScope) {
        ...
        if (!Cell.dontSave) {
            //			if (!FrontEnd.getK2Builtins().getInfo().getSymbolTable().containsKey(this.name)) {
            //				if (!FrontEnd.getMacBuiltins().getInfo().getSymbolTable().containsKey(this.name)) {
            //					if (!FrontEnd.getOtherBuiltins().getInfo().getSymbolTable().containsKey(this.name)) {
            //						allCells.add(this);
            //					}
            //				}
            //			}
    ...
    //
    //	/**
    //	 * Obtain a set with a singleton symbol object, which is used to denote all the shared symbols in a given context.
    //	 * @return
    //	 *	a set with a singleton symbol object, which is used to denote all the shared symbols in a given context.
    //	 */
    ...
    /**
     * Declaration, ParameterDeclaration, or FunctionDefinition, which declares this symbol.
     *
     * @return Declaration, ParameterDeclaration, or FunctionDefinition, which declares this symbol.
     */
    ...
    /**
     * Enclosing scope (CompoundStatement, FunctionDefinition, or TranslationUnit) in which this symbol was declared.
     *
     * @return enclosing scope (CompoundStatement, FunctionDefinition, or TranslationUnit) in which this symbol was
     * declared.
     */
    ...
    /**
     * Creates a new temporary with same assignable type as that of this symbol, and adds the declaration of the newly
    ...
    /**
     * Given a node, this method returns the default points-to set for this Symbol at that node, when no points-to
     * analysis has been run.
     *
     * @param node
     *         node at which the points-to information is required.
     *
     * @return the base points-to set, which should be returned until points-to analysis has started.
     */
