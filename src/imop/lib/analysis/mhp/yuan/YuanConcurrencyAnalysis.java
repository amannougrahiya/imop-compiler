...
public class YuanConcurrencyAnalysis {
    ...
    public static void reinitAggregatePhases(ParallelConstruct parConstruct) {
        ...
        // Step 2: Within the generated barrier tree, match all barrier pairs and store them in btPairs.
        ...
        if (parConstruct.getInfo().getBTPairs().isEmpty()) {
            ...
            for (YuanStaticPhase currSP : allStaticPhases) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    Set<YuanStaticPhase> matchingSet = currSet.stream().filter(s -> s.matchesWith(currSP)).collect(Collectors.toCollection(HashSet::new));
    ...
    public static class YuanStaticPhase {
        ...
        public YuanStaticPhase(Node startingBarrier, Node endingBarrier, ParallelConstruct owner) {
            ...
            stmts.addAll(Misc.setIntersection(startingBarrier.getInfo().getFWRNodes(), endingBarrier.getInfo().getBWRNodes()));
