...
public class Phase extends AbstractPhase<BeginPhasePoint, EndPhasePoint> {
    ...
    /**
     * Resets the list of stale phases. Should be called before parsing of the program.
     */
    ...
    /**
     * Tests if node {@code n} (or its BeginNode, if {@code n} is a non-leaf node) is allowed to co-exist with other
     * statements in phases that end at the {@code rootBarrier}. <br> A node can be shifted to a phase if any of these
     * conditions are true:
     * <ul>
     ...
     *
     * @return true if node {@code n} can be moved to the phases that end at {@code rootBarrier}.
     */
    public static boolean isNodeAllowedInNewPhasesAbove(Node n, BarrierDirective rootBarrier) {
        ...
        for (AbstractPhase<?, ?> ph : rootBarrier.getInfo().getNodePhaseInfo().getPhaseSet()) {
            for (Node otherNode : ph.getNodeSet()) {
                ...
                if (Misc.doIntersect(inputPhasesOfN, otherNode.getInfo().getNodePhaseInfo().getInputPhasesOfOldTimes())) {
                    continue;
                }
    ...
    /**
     * Tests if node {@code n} (or its BeginNode, if {@code n} is a non-leaf node) is allowed to co-exist with other
     * statements in phases that start after the {@code rootBarrier}. <br> A node can be shifted to a phase if any of
        ...
        for (AbstractPhase<?, ?> absPhAbove : rootBarrier.getInfo().getNodePhaseInfo().getPhaseSet()) {
            ...
            for (Node nodeBelow : nodesBelow) {
                if (belowBPP.stream().filter(bpp -> bpp.getNode() ==
                        rootBarrier).anyMatch(bpp -> bpp.getReachableNodes().contains(nodeBelow))) {
                ...
                if (belowBPP.stream().filter(bpp -> bpp.getReachableNodes().contains(nodeBelow) &&
                        bpp.getNode().getInfo().getNodePhaseInfo().getPhaseSet().contains(phAbove)).anyMatch(bpp -> CoExistenceChecker.canCoExistInPhase(bpp.getNode(), rootBarrier, phAbove))) {
                    // Refer to rule #3 in "Synchronization elimination --> Modeling SVE-sensitivity."
                    coExistBelow.add(nodeBelow);
                    continue;
                }
    ...
    public void stabilizeThisPhase(Set<Phase> visitedPhases, Set<BeginPhasePoint> staleBeginPhasePoints, Set<BeginPhasePoint> stabilizedBPPsDuringStabilization, Phase oldImage) {
        /*
        ...
        if (bppToProcess.isEmpty()) {
             ...
             */
            Phase succPhase = (this.succPhases == null ||
                    this.succPhases.isEmpty()) ? null : (Phase) Misc.getAnyElement(this.succPhases);
            if (succPhase != null) {
                succPhase.stabilizeThisPhase(visitedPhases, staleBeginPhasePoints, stabilizedBPPsDuringStabilization, succPhase);
            }
            return;
        }
        /*
         ...
         * Process the successor phase, and return.
         */
        succPhase = (this.succPhases == null ||
                this.succPhases.isEmpty()) ? null : (Phase) Misc.getAnyElement(this.succPhases);
        if (succPhase != null) {
            succPhase.stabilizeThisPhase(visitedPhases, staleBeginPhasePoints, stabilizedBPPsDuringStabilization, oldSuccessor);
        }
        return;
    }
    ...
    private Phase getNextPhase(Set<Phase> visitedPhases) {
        ...
        outer:
        for (Phase otherPhase : visitedPhases) {
    ...
    /**
     * Recomputes the set of nodes contained in this phase, and set of end-points for this phase.
     *
     * @return true, if the set of end-points may have changed as a result of this call.
     */
    ...
    /**
     * To find whether this phase starts with begin-phase points that correspond to the {@code endPPset}, a set of
     * end-phase points.
     *
     * @param endPPSet
     *         a set of end-phase points.
     *
     * @return true, if this phase starts with begin-phase points corresponding to end-phase points set {@code
     * endPPSet}.
     */
    @SuppressWarnings("unlikely-arg-type")
    ...
    /**
     * Adds {@code endPP} to {@link #endPoints} of this phase. This method also ensures that the PFG is made consistent
     * upon this change.
     *
     * @param endPP
    ...
    /**
     * Percolate code upwards into this phase from the phases that succeed this phase.
     *
    ...
    /**
     * Removes {@code endPP} from {@link #endPoints} of this phase. This method also ensures that the PFG is made
    ...
    /**
     * This method belongs to the "lazy update" module for stabilizing phase information only when required since the
     * last change to the program. It will ensure that the phase flow graph is correct as per the current status of the
     * program.
     */
    @Deprecated
    ...
    /**
     * This method ensures that the current set of end-phase points for this phase gets reflected in the next phase.
    ...
    /**
     * This method ensures that the current set of end-points and node-set are consistent with the ones that are
     * attainable from the current set of begin-points of this node. This method performs minimal updates to existing
     * sets, while adjusting them. Since it calls update-enabled methods for adding/removing endPhasePoints, and nodes,
     * the future path in PFG, and phase/inter-task information of affected nodes, get updated automatically. The
    ...
    /**
     * Removes all those phases that are not reachable from the start phase of {@code parConstruct} anymore.
     *
     * @param parCons
