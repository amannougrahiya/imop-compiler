...
public class MHPAnalyzer {
    ...
    /**
     * @param parConstruct:
     *         The Parallel Construct on which MHP Analysis has to be performed.
     */
    ...
    /**
     * This method performs MHP analysis over all the parallelConstructs in the specified {@code root}. At the end, the
     * nodes which may run in parallel have their phaseInfo.phaseList populated. Field {@link
    ...
    /**
     * This method starts the MHP Analysis on parConstruct, removing all the previously generated phases.
     */
    public void initMHP() {
        /*
         * Step 1:
    ...
    /**
     * This method calls the phase marker. It reads the last entry in allPhaseLists to get the start points and then
     * applies the interprocedural reachability analysis (or some alternative) on these start points. Note that the new
    ...
    private void processNextPhase(Phase ph) {
        ...
        if (lastPhase == null) {
            beginPoints.add(BeginPhasePoint.getBeginPhasePoint(new NodeWithStack(parInfo.getCFGInfo().getNestedCFG().getBegin(), new CallStack()), ph));
            ...
            for (PhasePoint aPoint : lastPhase.getEndPoints()) {
                if (aPoint.getNode() instanceof EndNode) {
                    ...
                    startPoints.addAll(aPoint.getNode().getInfo().getCFGInfo().getInterProceduralLeafSuccessors(aPoint.getCallStack()));
                }
            }
        }
    ...
    /**
     * This method checks whether we shall stop the MHP Analysis. It reads the allPhaseList to take this decision.
     * Multiple heuristics can be added to this function over time. Note: "Heuristic 1" below has side-effects on
    ...
    private boolean shouldProceedWithMHP() {
        ...
        outer:
        for (Phase otherPhase : allPhaseList) {
