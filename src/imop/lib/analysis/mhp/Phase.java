 ...
 */
public class Phase {
	private final ParallelConstruct parConstruct;
	private final int phaseId;
	private Set<Node> nodeSet = new HashSet<>(); // This is the set of nodes which are a part of this phase.
	private Set<BeginPhasePoint> beginPoints = new HashSet<>();
	private Set<EndPhasePoint> endPoints = new HashSet<>();
	private Phase succPhase;
	private List<Phase> predPhases = new ArrayList<>();
	...
	public Phase(ParallelConstructInfo owner) {
		this.parConstruct = (ParallelConstruct) owner.getNode();
		this.phaseId = ++counter;
		owner.getAllPhaseList().add(this);
	}
	...
	/**
	 * Create a new edge in the phase-flow graph from {@code source} phase to
	 * the {@code destination} phase.
	 * 
	 * @param source
	 ...
	 */
	public static void connectPhases(Phase source, Phase destination) {
		source.succPhase = destination;
		destination.predPhases.add(source);
	}

	/**
	 * Remove an existing edge in the phase-flow graph from {@code source} phase
	 * to {@code destination} phase.
	 * 
	 * @param source
	 *            source phase of an existing edge that has to be removed.
	 * @param destination
	 *            destination phase of an existing edge that has to be removed.
	 */
	public static void disconnectPhases(Phase source, Phase destination) {
		source.succPhase = null;
		if (destination != null && destination.predPhases != null) {
			destination.predPhases.remove(source);
		}
	}

	/**
	 * Tests if node {@code n} (or its BeginNode, if {@code n} is a non-leaf
	 * node) is allowed to co-exist with other statements in phases that end at
	 * the {@code rootBarrier}. <br>
	 * A node can be shifted to a phase if any of these conditions are true:
	 * <ul>
	 ...
	 * @param rootBarrier
	 *            base barrier above which we intend to move the node {@code n}.
	 * @return
	 *         true if node {@code n} can be moved to the phases that end at
	 *         {@code rootBarrier}.
	 */
	public static boolean isNodeAllowedInNewPhasesAbove(Node n, BarrierDirective rootBarrier) {
		...
		Set<Phase> inputPhasesOfN = n.getInfo().getNodePhaseInfo().getInputPhasesOfOldTimes();
		for (Phase ph : rootBarrier.getInfo().getNodePhaseInfo().getPhaseSet()) {
			for (Node otherNode : ph.getNodeSet()) {
				// Ignore those nodes that had shared at least one phase with {@code n} in the input code.
	...
	/**
	 * Tests if node {@code n} (or its BeginNode, if {@code n} is a non-leaf
	 * node) is allowed to co-exist with other statements in phases that start
	 * after
	 * the {@code rootBarrier}. <br>
	 * A node can be shifted to a phase if any of these conditions are true:
	 * <ul>
	 ...
	 *            node for which code motion has to be tested.
	 * @param rootBarrier
	 *            base barrier below which we intend to move the node {@code n}.
	 * @return
	 *         true if node {@code n} can be moved to the phases that end at
	 *         {@code rootBarrier}.
	 */
	public static boolean isNodeAllowedInNewPhasesBelow(Node n, BarrierDirective rootBarrier) {
		...
		Set<Phase> inputPhasesOfN = n.getInfo().getNodePhaseInfo().getInputPhasesOfOldTimes();
		for (Phase phAbove : rootBarrier.getInfo().getNodePhaseInfo().getPhaseSet()) {
			...
			for (Node nBelow : nodesBelow) {
				 ...
				 */
				Set<Phase> n2OldPhases = nBelow.getInfo().getNodePhaseInfo().getInputPhasesOfOldTimes();
	...
	public static void removeUnreachablePhases(ParallelConstruct parCons) {
		List<Phase> allPhaseList = parCons.getInfo().getAllPhaseList();
	...
	public static void resetStaticFields() {
		...
		Phase.stabilizationTime = 0;
		Phase.counter = 0;
	}

	/**
	 * This method belongs to the "lazy update" module for stabilizing phase
	 * information only when required since the last change to the program.
	 * It will ensure that the phase flow graph is correct as per the current
	...
	public static void deprecated_stabilizePhases() {
		...
		//		System.err.println("\tStabilization of phase completed in " + diff / (1.0 * 1e9) + "s.");
	...
	public void stabilizeThisPhase(Set<Phase> visitedPhases, Set<BeginPhasePoint> staleBeginPhasePoints,
		...
		if (bppToProcess.isEmpty()) {
			 ...
			 */
			if (this.succPhase != null) {
				this.succPhase.stabilizeThisPhase(visitedPhases, staleBeginPhasePoints,
						stabilizedBPPsDuringStabilization, this.succPhase);
			}
			return;
		}
		/*
		...
		Phase oldSuccessor = this.succPhase;
		if (ePPChanged || isANewPhase) {
			...
			if (this.succPhase != null) {
				Phase.disconnectPhases(this, this.succPhase);
			}
		 ...
		 */
		if (this.succPhase != null) {
			this.succPhase.stabilizeThisPhase(visitedPhases, staleBeginPhasePoints, stabilizedBPPsDuringStabilization,
					oldSuccessor);
		}
		return;
	}
	...
	/**
	 * Add the specified end-point to the end-points of this phase.
	 * This method does not support automated updates.
	 * 
	 * @return
	 */
	public boolean addEndPointNoUdpate(EndPhasePoint newEndPP) {
	...
	public Set<BeginPhasePoint> getStaleBeginPoints() {
		return this.beginPoints;
	}
	...
	public ParallelConstruct getParConstruct() {
		return parConstruct;
	}
	...
	/**
	 * Recomputes the set of nodes contained in this phase, and set of
	 * end-points for this phase.
	 * 
	 * @return
	 *         true, if the set of end-points may have changed as a result of
	 *         this call.
	 */
	...
	public void setBeginPointsNoUpdate(List<BeginPhasePoint> beginPoints) {
		this.beginPoints = new HashSet<>();
	 ...
	 * 
	 * @return
	 *         true, if cleaning of the unreachable phases is required.
	 */
	private boolean deprecated_settleNextPhase() {
		...
		List<Phase> allPhaseList = this.parConstruct.getInfo().getAllPhaseList();
	 ...
	 * This method performs minimal updates to existing sets,
	 * while adjusting them. Since it calls update-enabled methods for
	 * adding/removing endPhasePoints, and nodes, the future path in PFG, and
	 * phase/inter-task information of affected nodes, get updated
	 * automatically.
	 * The changes are propagated further in the PFG, as and when needed.
	 */
	 ...
	 * to the {@code endPPset}, a set of end-phase points.
	 * 
	 * @param endPPSet
	 *            a set of end-phase points.
	 * @return
	 *         true, if this phase starts with begin-phase points corresponding
	 *         to end-phase points set {@code endPPSet}.
	 */
	@SuppressWarnings("unlikely-arg-type")
	private boolean startsWithBeginPoints(Set<EndPhasePoint> endPPSet) {
	...
	public int hashCode() {
		...
		return result;
		//		return Arrays.hashCode(new Object[] {this.getPhaseId()});
	}
	 ...
	 * ensures that the PFG is made consistent upon this change.
	 * 
	 * @param endPP
	 ...
	 * 
	 * @return
	 *         true, if any code has been moved up.
	 */
	@Deprecated
	public boolean deprecated_old_PercolateCodeUpwards() {
		...
		for (PhasePoint endPP : this.endPoints) {
			...
			if (endPointNode instanceof BarrierDirective) {
				...
				// else, this is a case of nested parallelism, which we do not handle, as of now.	
	...
	/**
	 * Removes {@code endPP} from {@link #endPoints} of this phase. This method
	 * also
	 * ensures that the PFG is made consistent upon this change.
	 * 
	 * @param endPP
