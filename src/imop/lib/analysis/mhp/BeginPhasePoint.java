...
public class BeginPhasePoint extends PhasePoint {
	...
	private static Set<BeginPhasePoint> allBeginPhasePoints = new HashSet<>();
	...
	/**
	 * Stabilizes the phase information, starting with all those begin-phase
	 * points that have been marked as stale.
	 */
	...
	public static void stabilizeStaleBeginPhasePoints() {
		...
		for (ParallelConstruct pc : affectedParCons) {
			 ...
			 */
			Set<Phase> staleConnectedSet = new HashSet<>(pc.getInfo().getConnectedPhases());
			/*
			 * Stabilize each phase, starting with the first phase of PC.
			 */
			Phase firstPhase = pc.getInfo().getFirstPhase();
			 ...
			 */
			Set<Phase> freshConnectedSet = new HashSet<>(pc.getInfo().getConnectedPhases());
	...
	private BeginPhasePoint(Node sink, CallStack callStack) {
		...
		BeginPhasePoint.allBeginPhasePoints.add(this);
	}
	...
	public static BeginPhasePoint getBeginPhasePoint(Node sink, CallStack callStack, Phase ph) {
		...
		for (BeginPhasePoint bpp : BeginPhasePoint.allBeginPhasePoints) {
	...
	public static void flushALLMHPData() {
		...
		BeginPhasePoint.allBeginPhasePoints.clear();
	...
	public static void resetStaticFields() {
		BeginPhasePoint.allBeginPhasePoints.clear();
		BeginPhasePoint.getStaleBeginPhasePoints().clear();
		//		BeginPhasePoint.stabilizationTime = 0;
	}

	public static Set<BeginPhasePoint> getAllBeginPhasePoints() {
		if (Program.enableUnmodifiability) {
			return Collections.unmodifiableSet(BeginPhasePoint.allBeginPhasePoints);
		} else {
			return BeginPhasePoint.allBeginPhasePoints;
		}
	}

	/**
	 * Obtain set of all those BeginPhasePoints from which {@code node} may be
	 * reachable, on barrier-free path (inclusive).
	 * <br>
	 * Note that this method would also trigger resetting of
	 * {@link #setsInvalid} of all the {@link BeginPhasePoint} nodes,
	 * after refreshing the corresponding lists of reachable nodes and barriers.
	 * <br>
	 * Furthermore, if {@code node} is same as the node field of a
	 * BeginPhasePoint, that BeginPhasePoint is also considered as a valid entry
	 * to be returned.
	 * 
	 * @param node
	 ...
	 * @return
	 *         set of all those BeginPhasePoints from which {@code node} may be
	 *         reachable on barrier-free path of same parallelism-nesting level.
	 */
	...
	public static Set<BeginPhasePoint> getRelatedBPPsNoStaleRemoval(Node node) {
		...
		if (node instanceof EndNode && node.getParent() instanceof ParallelConstruct) {
			...
			for (Phase ph : node.getInfo().getNodePhaseInfo().getStalePhaseSet()) {
	...
	public static void deprecated_stabilizeStaleBeginPhasePoints() {
		...
		long diff = System.nanoTime() - timer;
		stabilizationTime += diff;
		//		System.err.println("\tStabilization of BPPs completed in " + diff / (1.0 * 1e9) + "s.");
	}
	 ...
	 * 
	 * @param nodeToBeAdded
	 *            node that has to be added to the reachability set of this
	 *            phase point.
	 */
	...
	/**
	 * Populate the reachable nodes, and next set of barriers, for
	 * this node.
	 * This method also triggers updates to the PFG, when needed.
	 */
	@Deprecated
	 ...
	 * without reflecting this change in the corresponding phases that may start
	 * at this BeginPhasePoint.
	 * 
	 * @return
	 *         true, if the state of existing sets changed.
	 */
	...
	/**
	 * Remove the node {@code node} from the reachability set of this phase
	 * point.
	 * <br>
	 * Note that this method does not attempt removal of any other nodes from
	 * the reachability set of this phase point.
	 * 
	 * @param node
	 *            node that has to be removed from the reachability set of this
	 *            phase point.
	 * @return
	 *         true, if {@code leafNode} was found in the reachability set and
	 *         removed.
	 */
	...
	/**
	 * Invalidates the reachability and nextBarrier sets for this
	 * BeginPhasePoint.
	 */
	 ...
	 * of {@code nodeSet} might be reachable.
	 * <br>
	 * Note that this method would also trigger resetting of
	 * {@link #setsInvalid} of all the {@link BeginPhasePoint} nodes,
	 * after refreshing the corresponding lists of reachable nodes and barriers.
	 ...
	 *            BeginPhasePoints from which one or more elements of the set
	 *            are reachable.
	 * @return
	 *         set of BeginPhasePoints from which one or more elements of
	 *         {@code nodeSet} are reachable on barrier-free path of same
	 *         nesting level of parallelism.
	 */
	@Deprecated
	 ...
	 * phase point.
	 * <br>
	 * Note that this method would not have any side-effects on sets of other
	 * phase points.
	 * 
	 * @param endPP
	 *            a phase point that has to be added in the
	 *            {@link #nextBarrierSet nextBarrierSet} of this node.
	 */
	@Deprecated
	...
	public Set<Node> deprecated_getStableReachables() {
		...
		//		Thread.dumpStack();
		//		assert (false) : "ERROR: " + n.getInfo().getDebugString() + " IS DISCONNECTED TO THE BARRIER "
	 ...
	 * Populate the reachable nodes, and next set of barriers, for
	 * this node.
	 * This method also triggers updates to the PFG, when needed, but doesn't
	 ...
	 * <br>
	 * Note that this method does not have side-effects on the set of other
	 * phase points.
	 * 
	 * @param endPP
	 ...
	 *            a new set of next end-phase points (barriers), for this
	 *            begin-phase points.
	 */
	@Deprecated
	...
	//	private static void setStaleBeginPhasePoints(Set<BeginPhasePoint> staleBeginPhasePoints) {
