...
public class CoExistenceChecker {
	...
	/**
	 * Checks if there exists any phase in which n1 and n2 may co-exist with
	 * each other.
	 * 
	 * @param n1
	 * @param n2
	 * @return
	 *         true if there exists any common phase for n1 and n2, where both
	 *         may co-exist with each other; false otherwise.
	 */
	...
	/**
	 * 
	 * Assuming that nodes {@code n1} and {@code n2} may happen in parallel,
	 * this method conservatively checks if it is possible for different threads
	 * to encounter n1 and n2, using the SVE (single-valued expression) analysis
	 * on the common control predicates of both the nodes.
	 * 
	 * @param n1
	 *            a CFG node.
	 * @param n2
	...
	private static boolean canCoExistInPhase(Node n1, Node n2, AbstractPhase<?, ?> ph, Set<NodePair> nodePairs,
		...
		for (ReversePath rp : n1Paths.controlPredicatePaths) {
			if (rp.bPP == null // i.e., the path starts at the entry point of a function.
					|| ph.getBeginPoints().contains(rp.bPP) // i.e., the BPP corresponding to the path starts the phase ph. 
	...
	/**
	 * Checks whether {@code variable} has been written anywhere in the phases
	 * in which {@code node} may execute.
	 * 
	 * @param node
	 *            a CFG node.
	 * @param variable
	 ...
	 *            call-stack).
	 * @return
	 *         true if there may exist a write to
	 */
	...
	/**
	 * Tests whether the node {@code rd} has all its control-predicates in all
	 * its phases as SVEs or not.
	 * <br>
	 * When this method returns true, it implies that {@code rd} will either be
	 * executed by all the threads or by none of them.
	 * 
	 * @param rd
	 ...
	 *            call-stack).
	 * @return
	 *         true if either all threads will execute {@code rd} in its any
	 *         given phase, or none.
	 */
