...
public class CoExistenceChecker {
    ...
    /**
     * Removes all caching related to {@code node} from {@link CoExistenceChecker#knownCoExistingNodesInPhase} and
    ...
    /**
     * Checks if there exists any phase in which n1 and n2 may co-exist with each other.
     *
     * @param n1
     * @param n2
     *
     * @return true if there exists any common phase for n1 and n2, where both may co-exist with each other; false
     * otherwise.
     */
    ...
    /**
     * Assuming that nodes {@code n1} and {@code n2} may happen in parallel, this method conservatively checks if it is
     * possible for different threads to encounter n1 and n2, using the SVE (single-valued expression) analysis on the
     ...
     *         an abstract phase.
     * @param nodePairs
     *         pair of nodes for which the query is being asked recursively.
     *
     * @return true, if both the nodes may be executed by different threads together in any runtime instance of phase
     * {@code ph}.
     */
    ...
    /**
     * Checks whether {@code variable} has been written anywhere in the phases in which {@code node} may execute.
     *
     * @param node
     *         a CFG node.
     * @param variable
     ...
     * @param nodePairs
     *         nodes for which co-existence check is under process (on call-stack).
     *
     * @return true if there may exist a write to
     */
    public static boolean isWrittenInPhase(Node node, Symbol variable, Set<Expression> expSet, Set<NodePair> nodePairs) {
    ...
    /**
     * Tests whether the node {@code rd} has all its control-predicates in all its phases as SVEs or not.
     * <br>
     * When this method returns true, it implies that {@code rd} will either be executed by all the threads or by none
     * of them.
     *
     * @param rd
     ...
     *         nodes for which co-existence check is under process (on call-stack).
     *
     * @return true if either all threads will execute {@code rd} in its any given phase, or none.
     */
    static boolean existsForAll(Node rd, Set<Expression> expSet, Set<NodePair> nodePairs) {
        ...
        for (AbstractPhase<?, ?> ph : cfgNode.getInfo().getNodePhaseInfo().getPhaseSet()) {
             ...
             * false.
             */
            if (ph.getBeginPoints().stream().filter(bpp -> !bpp.getReachableNodes().contains(cfgNode)).anyMatch(bpp -> canCoExistInPhase(bpp.getNodeFromInterface().getInfo().getCFGInfo().getLeafSuccessors().get(0), cfgNode, ph, nodePairs, expSet))) {
            ...
            CollectorVisitor.collectNodeSetInGenericGraph(nWS, new HashSet<>(), n -> (
                    n.getNode() instanceof BarrierDirective || (n.getNode() instanceof BeginNode &&
                            ((BeginNode) n.getNode()).getParent() instanceof ParallelConstruct)), n -> {
                Set<NodeWithStack> preds = n.getNode().getInfo().getCFGInfo().getParallelConstructFreeInterProceduralLeafPredecessors(n.getCallStack());
                /*
     ...
     */
    @Deprecated
    static boolean deprecated_canCoExistInPhase(Node n1, Node n2, Phase ph, Set<NodePair> nodePairs, Set<Expression> expSet) {
        assert (Program.concurrencyAlgorithm != Program.ConcurrencyAlgorithm.YUANMHP);
        if (Program.sveSensitive == SVEDimension.SVE_INSENSITIVE) {
            return true;
        }
        ...
        CollectorVisitor.collectNodeSetInGenericGraph(n1SF, endPointN1, n -> (n.getNode() instanceof BarrierDirective ||
                (n.getNode() instanceof BeginNode &&
                        ((BeginNode) n.getNode()).getParent() instanceof ParallelConstruct) ||
                (Misc.isAPredicate(n.getNode()) && !n.getNode().getInfo().isSCOPPed(bothNodes)) || n.getNode() ==
                n2SF.getNode()), n -> n.getNode().getInfo().getCFGInfo().getParallelConstructFreeInterProceduralLeafPredecessors(n.getCallStack()));
        ...
        CollectorVisitor.collectNodeSetInGenericGraph(n2SF, endPointN2, n -> (n.getNode() instanceof BarrierDirective ||
                (n.getNode() instanceof BeginNode &&
                        ((BeginNode) n.getNode()).getParent() instanceof ParallelConstruct) ||
                (Misc.isAPredicate(n.getNode()) && !n.getNode().getInfo().isSCOPPed(bothNodes)) || n.getNode() ==
                n1SF.getNode()), n -> n.getNode().getInfo().getCFGInfo().getParallelConstructFreeInterProceduralLeafPredecessors(n.getCallStack()));
            ...
            for (BeginPhasePoint n2Mn1BPP : Misc.setMinus(n2BPP, n1BPP)) {
                ...
                for (AbstractPhase<?, ?> prevPhaseAbs : ph.getPredPhases()) {
                    ...
                    if (prevPhase.getNodeSet().contains(prevB1) && prevPhase.getNodeSet().contains(prevB2)) {
                        if (CoExistenceChecker.deprecated_canCoExistInPhase(prevB1, prevB2, prevPhase, nodePairs, expSet)) {
                            nodePairs.remove(pair);
                            return true;
                        }
                    }
        ...
        CollectorVisitor.collectNodeSetInGenericGraph(n1SF, new HashSet<>(), n -> (
                n.getNode() instanceof BarrierDirective || (n.getNode() instanceof BeginNode &&
                        ((BeginNode) n.getNode()).getParent() instanceof ParallelConstruct)), n -> {
            Set<NodeWithStack> preds = n.getNode().getInfo().getCFGInfo().getParallelConstructFreeInterProceduralLeafPredecessors(n.getCallStack());
            /*
        ...
        CollectorVisitor.collectNodeSetInGenericGraph(n2SF, new HashSet<>(), n -> (
                n.getNode() instanceof BarrierDirective || (n.getNode() instanceof BeginNode &&
                        ((BeginNode) n.getNode()).getParent() instanceof ParallelConstruct)), n -> {
            Set<NodeWithStack> preds = n.getNode().getInfo().getCFGInfo().getParallelConstructFreeInterProceduralLeafPredecessors(n.getCallStack());
            /*
        ...
        //		Set<NodeWithStack> finalSet = commonSet.stream().map(c -> new NodeWithStack(c, new CallStack()))
        ...
        while (!commonSet.isEmpty()) {
            ...
            CollectorVisitor.collectNodeSetInGenericGraph(frontierSet, nextFrontier, n ->
                    commonSet.contains(n.getNode()) ||
                            finalSet.stream().anyMatch(f -> f.getNode() == n.getNode()), n -> {
                Set<NodeWithStack> retSet = n.getNode().getInfo().getCFGInfo().getParallelConstructFreeInterProceduralLeafPredecessors(n.getCallStack());
                retSet.removeAll(retSet.stream().filter(p -> (p.getNode() instanceof BarrierDirective) ||
                        (p.getNode() instanceof BeginNode &&
                                (((BeginNode) p.getNode()).getParent() instanceof ParallelConstruct))).collect(Collectors.toSet()));
                return retSet;
            });
        ...
        //		System.out.println("After " + i + " level(s) of backward BFS, remaining predicate count is " + finalSet.size()
        //				+ " out of " + commonSize + ".");
