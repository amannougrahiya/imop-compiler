...
public class CoExistenceChecker {
	...
	private static boolean canCoExistInPhase(Node n1, Node n2, AbstractPhase<?, ?> ph, Set<NodePair> nodePairs,
		 ...
		 */
		if (CoExistenceChecker.haveAnyValidPathPairs(n1, n2, pathsOfN1, pathsOfN2, ph, nodePairs, expSet)) {
			knownCoExistingNodesInPhase.add(nodePhasePair);
			knownCoExistingNodes.add(nodePair);
			nodePairs.remove(nodePair);
			return true;
	...
	private static boolean haveAnyValidPathPairs(Node n1, Node n2, Set<ReversePath> pathsOfN1,
		...
		for (ReversePath path1 : pathsOfN1) {
			for (ReversePath path2 : pathsOfN2) {
				 ...
				 */
				if (CoExistenceChecker.isPathPairValid(n1, n2, path1, path2, ph, nodePairs, expSet)) {
					return true;
				}
			}
		}
		...
		if (path1.startPoint != null && path2.startPoint != null && path1.startPoint != path2.startPoint) {
			...
			if (isP1Barrier && isP2Barrier) {
				if (path1.startPoint != path2.startPoint) {
					if (!CoExistenceChecker.canBarriersCoExistInPhase((Node) path1.startPoint, (Node) path2.startPoint,
							ph, nodePairs, expSet)) {
						return false;
					}
				}
				...
				if (isP1Barrier) {
					...
					if (isP1Call) {
						...
						if (calledDefs.isEmpty()) {
							options = new HashSet<>(1);
							options.add(p1Call.getPreCallNode());
						} else {
							options = new HashSet<>(calledDefs.size());
							for (FunctionDefinition func : calledDefs) {
								options.add(func.getInfo().getCFGInfo().getNestedCFG().getEnd());
				...
				if (!options.stream().anyMatch(other -> ph.getNodeSet().contains(other)
						&& CoExistenceChecker.canCoExistInPhase(base, other, ph, nodePairs, expSet))) {
					return false;
				}
			} else {
				// Here, we always pick n1 to be our base node.
				Node base = n1;
				Set<Node> options;
				if (isP2Call) {
					CallStatement p2Call = ((PostCallNode) path2.startPoint).getParent();
		...
		if (oneIsEntry && twoIsEntry) {
			...
			for (AbstractPhase<?, ?> predPhaseAbs : ph.getPredPhases()) {
				...
				if (nodeSet.contains(b1) && nodeSet.contains(b2)) {
					if (CoExistenceChecker.canCoExistInPhase(b1, b2, predPhase, nodePairs, expSet)) {
						return true;
					}
				}
			}
			...
			return CoExistenceChecker.canCoExistInPhase(b2, b1.getInfo().getCFGInfo().getLeafSuccessors().get(0), absPh,
					nodePairs, expSet);
		} else if (twoIsEntry) {
			// Check if a successor of the BPP2 can co-exist with BPP3 in ph.
			return CoExistenceChecker.canCoExistInPhase(b1, b2.getInfo().getCFGInfo().getLeafSuccessors().get(0), absPh,
					nodePairs, expSet);
		} else {
			...
			if (phNodeSet.contains(b1) && phNodeSet.contains(b2)) {
				...
				// ph.
				return CoExistenceChecker.canCoExistInPhase(b1, b2, absPh, nodePairs, expSet);
			} else {
				return true; // Conservatively.
			}
		}
