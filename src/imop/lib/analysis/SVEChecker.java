...
public class SVEChecker {
    ...
    /**
     * Names of functions that may return different values for same arguments. (These functions are not functions,
        ...
        for (Cell r : readList) {
            ...
            if (exp.getInfo().getSharingAttribute(variable) == DataSharingAttribute.SHARED) {
                ...
                if (CoExistenceChecker.isWrittenInPhase(exp, variable, expSet, nodePairs)) {
                    expSet.remove(exp);
                    //					System.out.println(exp + " is an MVE since it is a shared variable written in the current phase.");
                    SVEChecker.multiValuedExpressions.add(exp);
                    return false;
                } else {
                    continue;
                ...
                if (!ensureSameValue(variable, exp, expSet, nodePairs)) {
                    expSet.remove(exp);
                    //					System.out.println(exp + " is an MVE since the reaching definition(s) of " + variable
    ...
    /**
     * Checks if all threads will read the same value of private variable {@code sym} at the node {@code expression}.
     *
     * @param sym
     * @param expression
     * @param expSet
     * @param nodePairs
     *
     * @return true, if all threads will read same value for the private variable {@code sym} at the node {@code
    ...
    public static boolean ensureSameValue(Symbol sym, Node exp, Set<Expression> expSet, Set<NodePair> nodePairs) {
        ...
        if (reachingDefinitions == null || reachingDefinitions.isEmpty()) {
            //			System.err.println("No definition of " + sym + " at " + sym.getDeclaringNode() + " reaches " + exp
     ...
     *
     * @return true if {@code node} would write same value to a symbol, for any thread that may execute this node.
     */
    ...
    private static boolean writesSingleValue(Node node, Set<Expression> expSet, Set<NodePair> nodePairs) {
        if (node instanceof Declaration) {
            ...
            if (SVEChecker.variableFunctions.contains(calledFunction.getName())) {
                ...
                //				System.out.println(node + " performs a single-write since all arguments are single-valued.");
                return true;
            }
            ...
            //			System.out.println(node + " performs a" + (ret ? " single-write" : " multi-write") + " as an Expression.");
            return ret;
        }
    ...
    public static void extractSVEPragmas() {
        for (CompoundStatement compStmt : Misc.getInheritedPostOrderEnclosee(Program.getRoot(), CompoundStatement.class)) {
            /*
            ...
            for (Object elementObj : elements.stream().filter(n -> n instanceof UnknownPragma).toArray()) {
                ...
                if (annotatedElem instanceof IfStatement) {
                    ...
                    Misc.warnDueToLackOfFeature(
                            "Ignoring the sve annotation at line #" + Misc.getLineNum(element), null);
                }
            }
        }
