...
public class CriticalConstructCFGInfo extends CFGInfo {

    public CriticalConstructCFGInfo(Node owner) {
        super(owner);
    }

    public List<SideEffect> setBody(Statement stmt) {
        List<SideEffect> sideEffectList = new ArrayList<>();
        CriticalConstruct owner = (CriticalConstruct) this.getOwner();
        ...
        Set<BeginPhasePoint> affectedBeginPhasePoints = AutomatedUpdater.updateBPPOrGetAffectedBPPSetUponRemoval(owner.getF4());
    ...
    public Statement getBody() {
        return (Statement) Misc.getInternalFirstCFGNode(((CriticalConstruct) getOwner()).getF4());
    }
