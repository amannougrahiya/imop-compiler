...
public class CFGInfo {
    ...
    /**
     * Clears the list of successors of the node node. Furthermore, removes the node node as a predecessor from its
     * successors.
     */
    ...
    /**
     * Clears the list of predecessor of the node node. Furthermore, removes the node node as a successor from its
     * predecessors.
     */
    ...
    /**
     * Obtain intra-procedural intra-task successors at the same nesting level of a CFG node. Note that for an {@link
     * EndNode}, this method returns an empty list. Each {@link CallStatement}, and {@link DummyFlushDirective}, is
     * treated as any other node.
     *
     * @return intra-procedural intra-task successors at same nesting level. (Note: an empty list is returned for {@link
     * EndNode}.)
     */
    ...
    /**
     * Obtain intra-procedural intra-task successors at the same nesting level of a CFG node. Note that for an {@link
     * BeginNode}, this method returns an empty list. Each {@link CallStatement}, and {@link DummyFlushDirective}, is
     * treated as any other node.
     *
     * @return intra-procedural intra-task successors at same nesting level. (Note: an empty list is returned for {@link
     * BeginNode}.)
    ...
    /**
     * Obtain CFG successors of a node, such that for an {@link EndNode}, we obtain the successors of the parent node.
     *
     * @return CFG successors of a node, such that for an {@link EndNode}, we obtain the successors of the parent node.
     */
    ...
    /**
     * Obtain CFG predecessors of a node, such that for a {@link BeginNode}, we obtain the predecessors of the parent
     * node.
     *
     * @return CFG predecessors of a node, such that for a {@link BeginNode}, we obtain the predecessors of the parent
     * node.
     */
    ...
    /**
     * Obtain a set of leaf nodes that are immediate successors of the owner node in a context-insensitive path on the
     * super graph.
     *
     * @return a set of leaf nodes that are immediate successors of the owner node in a context-insensitive path on the
     * super graph.
     */
    ...
    /**
     * Obtain a set of leaf nodes that are immediate successors of the owner node in a context-sensitive path on the
     * super graph.
     *
     * @param callStack
     *         call stack used for context-sensitive traversal.
     *
     * @return a set of leaf nodes that are immediate successors of the owner node in a context-sensitive path on the
     * super graph.
     */
    public Set<NodeWithStack> getInterProceduralLeafSuccessors(CallStack callStack) {
        ...
        if (baseNode instanceof PreCallNode) {
            ...
            for (FunctionDefinition funcDef : calledDefinitions) {
                ...
                interProcSuccNodeStacks.add(new NodeWithStack(funcDef.getInfo().getCFGInfo().getNestedCFG().getBegin(), newStack));
            }
            return interProcSuccNodeStacks;
        }
        ...
        for (Node successor : baseNode.getInfo().getCFGInfo().getSuccessors()) {
            if (Misc.isCFGNonLeafNode(successor)) {
                interProcSuccNodeStacks.add(new NodeWithStack(successor.getInfo().getCFGInfo().getNestedCFG().getBegin(), callStack));
            } else {
    ...
    /**
     * Obtain a set of leaf nodes that are immediate predecessors of the owner node in a context-insensitive path on the
     * super graph.
     *
     * @return a set of leaf nodes that are immediate predecessors of the owner node in a context-insensitive path on
     * the super graph.
     */
    ...
    /**
     * Obtain a set of leaf nodes that are immediate predecessors of the owner node in a context-sensitive path on the
     * super graph.
     *
     * @param callStack
     *         call stack used for context-sensitive traversal.
     *
     * @return a set of leaf nodes that are immediate predecessors of the owner node in a context-sensitive path on the
     * super graph.
     */
    public Set<NodeWithStack> getInterProceduralLeafPredecessors(CallStack callStack) {
        ...
        for (Node predecessor : baseNode.getInfo().getCFGInfo().getPredecessors()) {
            if (Misc.isCFGNonLeafNode(predecessor)) {
                interProcPredNodeStacks.add(new NodeWithStack(predecessor.getInfo().getCFGInfo().getNestedCFG().getEnd(), callStack));
            } else {
    ...
    /**
     * Obtain a list of successor leaf nodes, while preserving the order present in block-level successors (if they are
     * present in that list).
     *
     * @return list of successor leaf nodes, in order in which they are present in the block-level successors.
     */
    ...
    /**
     * Obtain a list of predecessor leaf nodes, while preserving the order present in block-level predecessors (if they
     * are present in that list).
     *
     * @return list of predecessor leaf nodes, in order in which they are present in the block-level predecessors.
     */
    ...
    /**
     * Returns the inter-procedural leaf successors of the owner, except when it is a BeginNode of a ParallelConstruct.
     * When owner is a BeginNode of a ParallelConstruct, the immediate inter-procedural leaf successors of the enclosing
    ...
    /**
     * Returns the inter-procedural leaf predecessors of the owner, except when it is an EndNode of a ParallelConstruct.
     * When owner is an EndNode of a ParallelConstruct, the immediate inter-procedural leaf predecessors of the
    ...
    /**
     * Returns a set of all the CFG nodes that are present within the owner node, lexically.
     *
     * @return a set of all the CFG nodes that are present within the owner node, lexically.
     */
    ...
    /**
     * Returns a set of all the leaf CFG nodes that are present within the owner node, lexically.
     *
     * @return a set of all the leaf CFG nodes that are present within the owner node, lexically.
     */
    ...
    /**
     * Returns a set of all the leaf CFG nodes that are present on valid inter-procedural context-insensitive paths
     * between the BeginNode and EndNode of the owner node, if owner node is a non-leaf node. Otherwise, this method
     * returns a set containing just the owner node.
     * <br>
     * NOTE: This method DOES NOT traverse on reachable paths that originate from internal jumps and move outside the
     * node.
     *
     * @return set of all the leaf CFG nodes that are present in the valid inter-procedural paths.
     */
        ...
        for (Node n : this.getLexicalCFGContents()) {
            if (!Misc.isCFGLeafNode(n)) {
                ...
                if (n instanceof PreCallNode) {
                    ...
                    returnSet.addAll(CollectorVisitor.collectNodesIntraTaskForward(beginNode, endPoints, node -> node ==
                            endNode));
     ...
     * returns a set containing just the owner node.
     * <br>
     * NOTE: This method traverses on all reachable paths (even those that originate from internal jumps and move
     * outside the node).
     *
     * @return set of all the leaf CFG nodes that are present in the valid inter-procedural paths.
     *
    ...
    public Set<Node> getReachableIntraTaskCFGLeafContents() {
        Set<Node> returnSet = new HashSet<>();
        // Old code: Commented to remain consistent with the semantics of a CallStatement.
    ...
    /**
     * Returns a set of all the leaf CFG nodes that are present on valid inter-procedural context-sensitive paths
     * between the BeginNode and EndNode of the owner node, if owner node is a non-leaf node. Otherwise, this method
     * returns a set containing just the owner node.
     *
     * @return set of all the leaf CFG nodes that are present in the valid inter-procedural paths.
    ...
    public Set<NodeWithStack> getIntraTaskCFGLeafContents(CallStack callStack) {
        ...
        if (Misc.isCFGLeafNode(this.owner)) {
            ...
            NodeWithStack beginNodeWithStack = new NodeWithStack(this.owner.getInfo().getCFGInfo().getNestedCFG().getBegin(), callStack);
            ...
            returnSet.addAll(CollectorVisitor.collectNodesIntraTaskForwardContextSensitive(beginNodeWithStack, endNodeWithStackSet, n ->
                    n.getNode() == endNode));
    ...
    public Set<NodeWithStack> getIntraTaskCFGLeafContentsOfSameParLevel(CallStack callStack) {
        ...
        if (Misc.isCFGLeafNode(this.owner)) {
            ...
            Node endNode = this.owner.getInfo().getCFGInfo().getNestedCFG().getEnd();
            returnSet.add(beginNodeWithStack);
            returnSet.addAll(CollectorVisitor.collectNodesIntraTaskForwardOfSameParLevel(beginNodeWithStack, endNodeWithStackSet, n ->
                    n.getNode() == endNode));
            returnSet.addAll(endNodeWithStackSet);
        }
        return returnSet;
    ...
    /**
     * From among the lexical leaf contents of this node, this method returns a subset of those leaf CFG nodes from
     * which BeginNode of this node is not reachable, while moving backwards.
     * <br>
     * Note: To obtain the complete set, we should perform a simple backward traversal from the returned set, within the
     * given node.
     * <br>
     * While this method's counterpart {@link #getEndUnreachableLeafHeaders()}} is useful, one might wish to use {@link
     * NodeInfo#getInJumpDestinations()} instead of this method.
     *
     * @return subset of those leaf CFG nodes from which BeginNode of this node is unreachable.
     */
    public Set<Node> getBeginUnreachableLeafHeaders() {
        Set<Node> returnSet = new HashSet<>();
        Set<Node> lexicalContents = this.getLexicalCFGLeafContents();
        BeginNode beginNode = this.getNestedCFG().getBegin();
        Set<Node> beginReachables = CollectorVisitor.collectNodeSetInGenericGraph(beginNode, new HashSet<>(), n -> false, n -> n.getInfo().getCFGInfo().getLeafSuccessors().stream().filter(succ -> {
    ...
    /**
     * From among the lexical leaf contents of this node, this method returns a subset (headers of the paths) of those
     * leaf CFG nodes from which EndNode of this node is not reachable.
     * <br>
     * Note: To obtain the complete set, we should perform a simple forward traversal from the returned set, within the
     * given node.
     *
     * @return subset of those leaf CFG nodes from which EndNode of this node is unreachable.
     */
    public Set<Node> getEndUnreachableLeafHeaders() {
        Set<Node> returnSet = new HashSet<>();
        Set<Node> lexicalContents = this.getLexicalCFGLeafContents();
        EndNode endNode = this.getNestedCFG().getEnd();
        Set<Node> endReachables = CollectorVisitor.collectNodeSetInGenericGraph(endNode, new HashSet<>(), n -> false, n -> n.getInfo().getCFGInfo().getLeafPredecessors().stream().filter(pred -> {
    ...
    /**
     * Returns a set of all the CFG leaf nodes that are reachable starting the immediate successors of this node, on
     * valid paths.
     * <p>
     * Note that when called on a non-leaf node, this traversal will not traverse inside the non-leaf node, but would
     * start with its same nesting level successors.
     *
     * @return a set of all the CFG leaf nodes that are reachable starting the immediate successors of this node, on
     * valid paths.
     */
    ...
    /**
     * Returns a set of all the CFG leaf nodes that are reachable backwards starting with the immediate predecessors of
     * this node, on valid paths.
     * <p>
     * Note that when called on a non-leaf node, this traversal will not traverse inside the non-leaf node, but would
     * start with its same nesting level successors.
     *
     * @return a set of all the CFG leaf nodes that are reachable backwards starting with the immediate predecessors of
     * this node, on valid paths.
     */
    ...
    /**
     * Obtain a set of predecessor node edges for this {@code node}, along with symbols via which information may flow
     * on the edge connecting the predecessors to this {@code node}.
     *
     * @param node
     *         node whose predecessors have to be found.
     *
     * @return a set of predecessor node edges for this {@code node}, along with symbols via which information may flow
     * on the edge connecting the predecessors to this {@code node}.
     */
    ...
    /**
     * Obtain a set of predecessor node edges for this {@code node}, along with symbols via which information may flow
     * on the edge connecting the predecessors to this {@code node}; for variables in {@code set}.
     *
     * @param node
     *         node whose predecessors have to be found.
     * @param set
     *         set of cells for which communication edges have to be considered; if this set is {@code null}, we assume
    ...
    /**
     * Obtain a set of successor node edges for this {@code node}, along with symbols via which information may flow on
     * the edge connecting the successors to this {@code node}.
     *
     * @param node
     *         node whose successors have to be found.
     *
     * @return a set of successor node edges for this {@code node}, along with symbols via which information may flow on
     * the edge connecting the successors to this {@code node}.
     */
    ...
    /**
     * Obtain a set of successor node edges for this {@code node}, along with symbols via which information may flow on
     * the edge connecting the successors to this {@code node}; for variables in {@code set}.
     *
     * @param node
     *         node whose successors have to be found.
     * @param set
     ...
     * on the edge connecting the predecessors to this {@code node}.
     */
    public Set<IDFAEdgeWithStack> getInterTaskLeafPredecessorEdgesForVariables(CallStack callStack, CellSet set, SVEDimension sveDimension) {
        Node node = this.owner;
        ...
        if (node instanceof DummyFlushDirective) {
            ...
            for (InterTaskEdge interTaskEdge : headFlush.getInfo().getIncomingInterTaskEdges()) {
                ...
                if (communicationCells.isEmpty()) {
                    ...
                    predecessors.add(new IDFAEdgeWithStack(new NodeWithStack(tailFlush, new CallStack()), communicationCells));
                }
            }
        }
        return predecessors;
     ...
     * on the edge connecting the predecessors to this {@code node}.
     */
    public Set<NodeWithStack> getInterTaskLeafPredecessorNodesForVariables(CallStack callStack, CellSet set, SVEDimension sveDimension) {
        Set<NodeWithStack> returnSet = new HashSet<>();
        for (IDFAEdgeWithStack edgeWithStack : this.getInterTaskLeafPredecessorEdgesForVariables(callStack, set, sveDimension)) {
     ...
     * the edge connecting the successors to this {@code node}.
     */
    public Set<IDFAEdgeWithStack> getInterTaskLeafSuccessorEdgesForVariables(CallStack callStack, CellSet set, SVEDimension sveDimension) {
        Node node = this.owner;
        ...
        if (node instanceof DummyFlushDirective) {
            ...
            for (InterTaskEdge interTaskEdge : tailFlush.getInfo().getOutgoingInterTaskEdges()) {
                ...
                if (communicationCells.isEmpty()) {
                    ...
                    successors.add(new IDFAEdgeWithStack(new NodeWithStack(headFlush, new CallStack()), communicationCells));
                }
            }
        }
        return successors;
     ...
     * the edge connecting the successors to this {@code node}.
     */
    public Set<NodeWithStack> getInterTaskLeafSuccessorNodesForVariables(CallStack callStack, CellSet set, SVEDimension sveDimension) {
        Set<NodeWithStack> returnSet = new HashSet<>();
    ...
    public static CellSet getCommunicationVariables(DummyFlushDirective tailFlush, DummyFlushDirective headFlush) {
        ...
        Set<Node> beginNodes = commonUniNodes.stream().filter(n -> n instanceof BeginNode &&
                (n.getParent() instanceof MasterConstruct ||
                        n.getParent() instanceof SingleConstruct)).collect(Collectors.toSet());
        for (Node x : beginNodes) {
            for (Node y : beginNodes) {
                if (x == y) {
                    continue;
                }
                commVars.addAll(Misc.setIntersection(x.getParent().getInfo().getSharedWrites(), y.getParent().getInfo().getSharedReads()));
                commVars.addAll(Misc.setIntersection(y.getParent().getInfo().getSharedWrites(), x.getParent().getInfo().getSharedReads()));
            }
        }
    ...
    /**
     * This method returns true if control entering to the node "via any point within the block" may flow out of the end
     * of the node.
     * <br>
     * This method should not be called until the CFG edges have been created within <code>n</code>
     *
     * @return true, if starting the control-flow from the begin of the node node, the end of the node node can be
     * reached.
     */
    ...
    /**
     * This method should be overridden by all the subclasses of CFGInfo.<br> It should return the various CFG
     * components of the {@code owner} node.<br> Note: This method should be made {@code abstract} in CFGInfo, after we
    ...
    /**
     * Obtain the body, if any, of this node, if it is a non-leaf CFG node that contains a body.
     *
    ...
    /**
     * Removes CFG edges from and to source and destination, respectively. Note that this also triggers adjustment of
