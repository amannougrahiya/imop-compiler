...
public class CFGInfo {
	 ...
	 * 
	 * @return
	 *         the RPO index of the SCC corresponding to this node.
	 */
	 ...
	 * Furthermore, removes the node node as a predecessor from its successors.
	 */
	 ...
	 * Furthermore, removes the node node as a successor from its predecessors.
	 */
	 ...
	 * of a CFG node.
	 * Note that for an {@link EndNode}, this method returns an empty list.
	 * Each {@link CallStatement}, and {@link DummyFlushDirective}, is treated
	 * as any other node.
	 * 
	 * @return
	 *         intra-procedural intra-task successors at same nesting level.
	 ...
	 * Obtain intra-procedural intra-task successors at the same nesting level
	 * of a CFG node.
	 * Note that for an {@link BeginNode}, this method returns an empty list.
	 * Each {@link CallStatement}, and {@link DummyFlushDirective}, is treated
	 * as any other node.
	 * 
	 * @return
	...
	/**
	 * Obtain CFG successors of a node, such that for an {@link EndNode}, we
	 * obtain the
	 * successors of the parent node.
	 * 
	 * @return
	 *         CFG successors of a node, such that for an {@link EndNode}, we
	 *         obtain
	 ...
	 * 
	 * @return
	 *         CFG predecessors of a node, such that for a {@link BeginNode}, we
	 *         obtain
	 *         the predecessors of the parent node.
	 */
	 ...
	 * 
	 * @return
	 *         a set of leaf nodes that are immediate successors of the owner
	 *         node in a context-insensitive path on the super graph.
	 */
	 ...
	 * 
	 * @return
	 *         a set of leaf nodes that are immediate predecessors of the owner
	 *         node in a context-insensitive path on the super graph.
	 */
	...
	/**
	 * Obtain a list of successor leaf nodes, while preserving the order present
	 * in block-level successors (if they are present in that list).
	 * 
	 * @return
	 *         list of successor leaf nodes, in order in which they are present
	 *         in the block-level successors.
	 */
	...
	/**
	 * Obtain a list of predecessor leaf nodes, while preserving the order
	 * present in block-level predecessors (if they are present in that list).
	 * 
	 * @return
	 *         list of predecessor leaf nodes, in order in which they are
	 *         present in the block-level predecessors.
	 */
	 ...
	 * except when it is a BeginNode of a ParallelConstruct. When
	 * owner is a BeginNode of a ParallelConstruct, the immediate
	 * inter-procedural leaf successors of the enclosing parallel construct are
	 * returned.
	 * 
	 * @param phasePoint
	 * @return
	 ...
	 * except when it is an EndNode of a ParallelConstruct. When
	 * owner is an EndNode of a ParallelConstruct, the immediate
	 * inter-procedural leaf predecessors of the enclosing parallel construct
	 * are
	 * returned.
	 * 
	 * @param phasePoint
	 ...
	 * 
	 * @return
	 *         a set of all the CFG nodes that are present within the owner
	 *         node, lexically.
	 */
	 ...
	 * 
	 * @return
	 *         a set of all the leaf CFG nodes that are present within the owner
	 *         node, lexically.
	 */
	 ...
	 * Otherwise, this method returns a set containing just the owner node.
	 * <br>
	 * NOTE: This method DOES NOT traverse on reachable paths that
	 * originate from internal jumps and move outside the node.
	 * 
	 * @return
	 *         set of all the leaf CFG nodes that are present in the valid
	 ...
	 * Otherwise, this method returns a set containing just the owner node.
	 * <br>
	 * NOTE: This method traverses on all reachable paths (even those that
	 * originate from internal jumps and move outside the node).
	 * 
	 * @return
	 *         set of all the leaf CFG nodes that are present in the valid
	 ...
	 * subset of those leaf CFG nodes from which BeginNode of this node is not
	 * reachable, while moving backwards.
	 * <br>
	 * Note: To obtain the complete set, we should perform a simple backward
	 * traversal from the returned set, within the given node.
	 * <br>
	 * While this method's counterpart {@link #getEndUnreachableLeafHeaders()}}
	 * is useful, one might wish to use {@link NodeInfo#getInJumpDestinations()}
	 * instead of this method.
	 * 
	 * @return
	 *         subset of those leaf CFG nodes from which BeginNode of this node
	 *         is unreachable.
	 */
	...
	/**
	 * From among the lexical leaf contents of this node, this method returns a
	 * subset (headers of the paths) of those leaf CFG nodes from which EndNode
	 * of this node is not reachable.
	 * <br>
	 * Note: To obtain the complete set, we should perform a simple forward
	 * traversal from the returned set, within the given node.
	 * 
	 * @return
	 *         subset of those leaf CFG nodes from which EndNode of this node is
	 *         unreachable.
	 */
	 ...
	 * <p>
	 * Note that when called on a non-leaf node, this traversal will not
	 * traverse inside the non-leaf node, but would start with its same nesting
	 * level successors.
	 * 
	 * @return
	 *         a set of all the CFG leaf nodes that are reachable starting the
	 *         immediate successors of this node, on valid paths.
	 */
	 ...
	 * 
	 * @return
	 *         a set of all the CFG leaf nodes that are reachable backwards
	 *         starting with the immediate predecessors of this node, on valid
	 *         paths.
	 */
	 ...
	 * @return
	 *         a set of predecessor node edges for this {@code node}, along with
	 *         symbols via which information may flow on the edge connecting the
	 *         predecessors to this {@code node}.
	 */
	 ...
	 * Obtain a set of predecessor node edges for this {@code node}, along with
	 * symbols via which information may flow on the edge connecting the
	 * predecessors to this {@code node}; for variables in {@code set}.
	 * 
	 * @param node
	 *            node whose predecessors have to be found.
	 * @param set
	 *            set of cells for which communication edges have to be
	 *            considered; if this set is {@code null}, we assume it to be a
	 *            global set.
	 * @return
	 *         a set of predecessor node edges for this {@code node}, along with
	 *         symbols via which information may flow on the edge connecting the
		...
		if (node instanceof DummyFlushDirective) {
			...
			for (InterTaskEdge interTaskEdge : headFlush.getInfo().getIncomingInterTaskEdges()) {
				...
				if (communicationCells.isEmpty()) {
					...
					if (sveDimension == SVEDimension.SVE_SENSITIVE
	 ...
	 * @return
	 *         a set of successor node edges for this {@code node}, along with
	 *         symbols via which information may flow on the edge connecting the
	 *         successors to this {@code node}.
	 */
	 ...
	 * Obtain a set of successor node edges for this {@code node}, along with
	 * symbols via which information may flow on the edge connecting the
	 * successors to this {@code node}; for variables in {@code set}.
	 * 
	 * @param node
	 *            node whose successors have to be found.
	 * @param set
	...
	/**
	 * This method returns true if control entering to the node
	 ...
	 *         the node node, the end of the node node can be reached.
	 */
	public boolean isEndReachable() {
		assert (Misc.isCFGNode(this.getOwner())) : "Note that " + this.getOwner().getClass().getSimpleName()
				+ " is not a CFG node.";
	 ...
	 * It should return the various CFG components of the {@code owner}
	 * node.<br>
	 ...
	 * 
	 * @return
	 *         body, if any, of this node; otherwise {@code null}.
	 */
	...
	/**
	 * Adds CFG edges from and to source and destination, respectively,
	 ...
	 * Note that this also triggers adjustment of EndReachability, if required.
	 * 
	 * @param source
	 *            source node, for the CFG edge.
	 * @param destination
