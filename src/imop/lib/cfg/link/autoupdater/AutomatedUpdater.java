...
public class AutomatedUpdater {
	...
	/**
	 * Flushes various cached data that we cannot rely upon due to program
	 * update. <br>
	 * Note that this is the most naive (and inefficient) solution to maintain
	 * consistency, for any given caching.
	 */
	...
	/**
	 * List of updates that should be performed to make various representations
	 * of the program consistent, upon removal of a node. Note that these
	 * updates need to be called before the CFG and AST links have actually been
	...
	/**
	 * List of updates that should be performed to make various representations
	 * of the program consistent, upon addition of a node. Note that these
	 * updates need to be called after the CFG and AST links have actually been
	 * created for the new node.
	 */
	...
	/**
	 * Given a node that has been added to the program, this method
	 * sets the points-to instability flag, if required.<br>
	 * Furthermore, if the instability flag is not set, this method also sets
	 ...
	 * @param interSet
	 *            a set of lambda expressions (rather, method references), which
	 *            need to be called on the {@code node}.
	 */
	...
	/**
	 * This method traverses forward in the inter-procedural CFG, and
	...
	/**
	 * This methods sets the invalidation flags for the appropriate read/write
	 * sets of appropriate dummy-flushes when label list has been updated for
	 * {@code node}.
	 * <br>
	...
	/**
	 * Returns a set of those nodes starting which we need to rerun various
	 * forward iterative data-flow analyses, to make the related flow-facts
	 * consistent, upon removal of {@code oldNode}.
	 * <br>
	 ...
	 * @return
	 *         a set of those nodes starting which we need to rerun forward
	 *         data-flow analyses.
	 */
	 ...
	 *            node of interest.
	 * @return
	 *         a set of nodes that lie in those paths which will not be
	 *         reachable anymore after removal of the {@code oldNode}, when
	...
	/**
	 * Returns a set of those nodes starting which we need to rerun various
	 * backward iterative data-flow analyses, to make the related flow-facts
	 * consistent, upon removal of {@code oldNode}.
	 * <br>
	 * <i>Note that this method should be called before {@code oldNode}
	 * has already been removed. However, the reruns should start after the
	 ...
	 *            still connected.)
	 * @return
	 *         a set of those nodes starting which we need to rerun backward
	 *         data-flow analyses.
	 */
	 ...
	 * {@link MHPAnalyzer#initMHP()} on each {@link ParallelConstruct} present
	 * in the node.
	...
	/**
	 * This method attempts to adjust MHP information locally, if that can be
	 * done,
	 ...
	 * @return
	 *         {@code true}, if the MHP stabilization was successful.
	 */
	public static boolean stabilizeMHPLocallyUponAddition(Node node) {
		...
		for (Node pred : predSet) {
			...
			try {
				for (Phase ph : pred.getInfo().getNodePhaseInfo().getStalePhaseSet()) {
					for (Node n : internalContents) {
						ph.addNode(n);
					}
				}
	...
	public static boolean stabilizeMHPLocallyUponRemoval(Node node) {
		...
		Set<Node> internalContents = node.getInfo().getCFGInfo().getIntraTaskCFGLeafContents();
		for (Node n : internalContents) {
			for (Phase ph : new HashSet<>(node.getInfo().getNodePhaseInfo().getStalePhaseSet())) {
				ph.removeNode(n);
				for (BeginPhasePoint bpp : new HashSet<>(ph.getStaleBeginPoints())) {
	 ...
	 * inter-task edges, upon insertion of the node {@code node}.
	 * <br>
	...
	public static void updatePhaseAndInterTaskEdgesUponAddition(Node node) {
		...
		if (Misc.isCFGLeafNode(node)) {
			if (node instanceof BarrierDirective) {
				for (BeginPhasePoint bpp : BeginPhasePoint.getAllBeginPhasePoints()) {
					if (bpp.getNode() == node) {
	 ...
	 * {@link BeginPhasePoint#removeNode(Node)},
	 * {@link BeginPhasePoint#deprecated_removeNextBarrier(imop.lib.analysis.mhp.EndPhasePoint)},
	 * etc., this method just performs the updates, and returns an empty set.
	 * <br>
	 * Otherwise, this method is used to obtain a set of BeginPhasePoints on
	 * which we need to call
	 * {@link BeginPhasePoint#populateReachablesAndBarriers()}, after CFG
	 ...
	 * @return
	 *         a set of BeginPhasePoints whose sets need to be regenerated upon
	 *         removal of {@code node}, using
	 ...
	 * nodes that have resulted in changes to the sets of various begin-phase
