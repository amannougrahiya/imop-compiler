...
public class AutomatedUpdater {
    ...
    /**
     * This updater is used as a target for various automated updater for addition and removal of node {@code n} from
     * its CFG.
     *
     * @author aman
     */
    ...
    /**
     * Flushes various cached data that we cannot rely upon due to program update. <br> Note that this is the most naive
    ...
    /**
     * List of updates that should be performed to make various representations of the program consistent, upon removal
     * of a node. Note that these updates need to be called before the CFG and AST links have actually been removed for
     * the old node.
     */
    protected static List<Updater> updateSetForRemoval;

    /**
     * List of updates that should be performed to make various representations of the program consistent, upon addition
     * of a node. Note that these updates need to be called after the CFG and AST links have actually been created for
     * the new node.
     */
    ...
    static {
        ...
        updateSetForAddition.add(AutomatedUpdater::performInitOfMHPUponAddition); // TODO: Verify this order for various cases.
    ...
    /**
     * Given a node that has been added to the program, this method sets the points-to instability flag, if
     * required.<br> Furthermore, if the instability flag is not set, this method also sets the points-to IN and OUT
     * flow-facts of all the leaf nodes within {@code affectedNode} to be same as that of the meet of OUT of
    ...
    /**
     * Given a node that has to be removed from the program, this method sets the points-to instability flag, if
     * required.
     *
     * @param affectedNode
    ...
    /**
     * Invokes the driver method that performs automated updates for insertion of the node {@code insertedNode}.
     *
     * @param insertedNode
    ...
    /**
     * Invokes the driver module that performs automated updates for removal of the node {@code removedNode}.
     *
     * @param removedNode
    ...
    /**
     * Driver function that performs call-backs of various update methods specified in the {@code interSet}, after a
     * node {@code node} has been inserted in or removed from the CFG.
     *
     * @param node
     *         newly inserted, or to be removed, node.
     * @param interSet
     *         a set of lambda expressions (rather, method references), which need to be called on the {@code node}.
     */
    ...
    /**
     * This methods sets the invalidation flags for the appropriate read/write sets of appropriate dummy-flushes when
     * {@code oldNode} has been removed from a certain position in the CFG.
     *
     * @param oldNode
     *         a node that has to be removed from a certain point in the CFG. Note that this method should be called
    ...
    public static void updateSymbolsAtDummyFlushesUponRemoval(Node oldNode) {
        ...
        if (Misc.isCFGLeafNode(oldNode)) {
            ...
            // Invalidate the written-before and read-after symbol sets of all the dummy flushes
            ...
            for (Node successorInNewArea : CollectorVisitor.collectNodesIntraTaskForward(beginNode, new HashSet<>(), (n) -> (
                    n == endNode))) {
        ...
        CollectorVisitor.collectNodesIntraTaskBackward(backwardStartPoint, previousDummies, (n) -> (n instanceof DummyFlushDirective));
        ...
        CollectorVisitor.collectNodesIntraTaskForward(forwardStartPoint, futureDummies, (n) -> (n instanceof DummyFlushDirective));
    ...
    /**
     * This method sets the invalidation flags for the appropriate read/write sets of appropriate dummy-flushes when
    ...
    /**
     * This method traverses forward in the inter-procedural CFG, and invalidates the written-before symbol set of first
    ...
    public static void invalidateWriteBeforeCellsInSuccessorDummyFlushesOf(Set<Node> nodeSet) {
        for (Node node : nodeSet) {
            ...
            Set<Node> futureDummies = new HashSet<>();
            CollectorVisitor.collectNodesIntraTaskForward(node, futureDummies, (n) -> (n instanceof DummyFlushDirective));
            for (Node futureDummy : futureDummies) {
                DummyFlushDirective dummy = (DummyFlushDirective) futureDummy;
                dummy.invalidWrittenCellSet = true;
            }
    ...
    /**
     * This method traverses backward in the inter-procedural CFG, and invalidates the read-after symbol set of first
     * encountered DummyFlushes on all paths.
     *
     * @param nodeSet
     *         nodes starting which the invalidation traversal has to begin.
    ...
    public static void invalidateReadAfterCellsInPredecessorDummyFlushesOf(Set<Node> nodeSet) {
        for (Node node : nodeSet) {
            ...
            Set<Node> previousDummies = new HashSet<>();
            CollectorVisitor.collectNodesIntraTaskBackward(node, previousDummies, (n) -> (n instanceof DummyFlushDirective));
            for (Node previousDummy : previousDummies) {
                DummyFlushDirective dummy = (DummyFlushDirective) previousDummy;
                dummy.invalidReadCellSet = true;
            }
     ...
     * <br>
     * Note that this method should be called while the label to be removed or added are present in the {@code node}.
     *
     * @param node
    ...
    /**
     * Updates the set of call statements present in various non-leaf nodes that would lexically enclose {@code node}
    ...
    /**
     * Updates the set of call statements present in various non-leaf nodes that enclose {@code node} before it gets
    ...
    /**
     * Updates flow-fact information for various analyses, by rerunning the analyses, starting with {@code newNode}.
     * Note that this method is called after {code newNode} has been inserted in the CFG, and after all the inter- and
     * intra-procedural edge updates have been taken care of.
     *
     * @param newNode
     *         newly inserted node, starting which various analyses have to rerun.
     */
    public static void updateFlowFactsForAddition(Node newNode) {
        ...
        //		HashMap<AnalysisName, InterThreadForwardIDFA<?>> analysisMapForward = InterThreadForwardIDFA
        //				.getForwardAnalyses();
        ...
        //		HashMap<AnalysisName, InterThreadBackwardIDFA<?>> analysisMapBackward = InterThreadBackwardIDFA
        //				.getBackwardAnalyses();
    ...
    /**
     * Updates flow-fact information for various forward analyses, by rerunning the analyses on the set of nodes in
     * {@code nodeSet}.
     *
     * @param nodeSet
    ...
    public static void updateFlowFactsForward(Set<Node> nodeSet) {
        ...
        //		 */
        //		if (analysisMapForward.values().stream().anyMatch((a) -> a instanceof PointsToAnalysis)) {
        //			Optional<InterThreadForwardIDFA<?>> opt = analysisMapForward.values().stream()
    ...
    /**
     * Updates flow-fact information for various backward analyses, by rerunning the analyses on the set of nodes in
     * {@code nodeSet}.
     *
     * @param nodeSet
     *         set of nodes starting which backward analyses have to be rerun.
     */
    ...
    /**
     * Returns a set of those nodes starting which we need to rerun various forward iterative data-flow analyses, to
     ...
     * has already been removed. However, the reruns should start after the actual removal.</i>
     *
     * @param oldNode
     *         node that has to be removed from the CFG (while the node is still connected to the CFG).
     *
     * @return a set of those nodes starting which we need to rerun forward data-flow analyses.
     */
    ...
    /**
     * Returns a set of nodes that lie in those paths which will not be reachable anymore after removal of the {@code
     * oldNode}, when traversing in the forward direction.
     *
     * @param oldNode
     *         node of interest.
     *
     * @return a set of nodes that lie in those paths which will not be reachable anymore after removal of the {@code
     * oldNode}, when traversing in the forward direction.
     */
    ...
    /**
     * Returns a set of those nodes starting which we need to rerun various backward iterative data-flow analyses, to
     * make the related flow-facts consistent, upon removal of {@code oldNode}.
     * <br>
     * <i>Note that this method should be called before {@code oldNode}
     * has already been removed. However, the reruns should start after the actual removal.</i>
     *
     * @param oldNode
     *         node that has to be removed from the CFG. (However, which is still connected.)
     *
     * @return a set of those nodes starting which we need to rerun backward data-flow analyses.
     */
    ...
    public static Set<Node> unreachableAfterRemovalBackward(Node oldNode) {
        ...
        if (node instanceof CallStatement) {
            ...
            for (DummyFlushDirective destinationDummyFlush : Misc.getInheritedEnclosee(node, DummyFlushDirective.class)) {
                returnSet.addAll(destinationDummyFlush.getInfo().getInterTaskDummyPredecessors());
            }
            returnSet.addAll(node.getInfo().getInJumpSources());
            return returnSet;
    ...
    /**
     * This method models the affect of addition of an already existing label in {@code node} to {@code node}. This is
     * used while inserting {@code node} or any of its ancestors at a new place in CFG.
     *
     * @param node
     */
    ...
    /**
     * This method models the affect of removal of a label (without actually removing the label) from {@code node}, and
     * its nested components. It is used before removing {@code node} from its current position in the CFG.
     *
     * @param node
     */
    ...
    /**
     * This method automatically updates the symbol table, typedef table, and type tables upon addition of the node
     * {@code node}.
     *
     * @param node
    ...
    /**
     * This method automatically updates the symbol table, typedef table, and type tables upon removal of the node
     * {@code node}.
     *
     * @param node
    ...
    public static void updateSymbolTableOnNodeRemoval(Node node) {
        ...
        if (Misc.isCFGLeafNode(node)) {
            if (node instanceof Declaration) {
                Scopeable nestingScope = Misc.getEnclosingBlock(node);
                if (nestingScope instanceof CompoundStatement) {
                    ((CompoundStatement) nestingScope).getInfo().removeDeclarationFromSymbolOrTypeTable((Declaration) node);
                }
            }
        }
    }
    ...
    /**
     * Given a node that has been added to the program, this method invokes {@link MHPAnalyzer#initMHP()} on each {@link
     * ParallelConstruct} present in the node. Furthermore, if the set of input phases for any leaf CFG node lexically
     * enclosed within the given node is empty, then this method invokes {@link NodePhaseInfo#rememberCurrentPhases()}
     * on such leaf nodes.
     *
     * @param addedNode
     */
    ...
    /**
     * This method attempts to adjust MHP information locally, if that can be done, to model the effect of addition of
     * {@code node} If successful, it returns {@code true}.
     *
     * @param node
     *         the node that has been added to the program, for which MHP information needs to be stabilized.
     *
    ...
    public static boolean stabilizeMHPLocallyUponAddition(Node node) {
        ...
        if (node instanceof BarrierDirective || !node.getInfo().isControlConfined() ||
                node.getInfo().getCFGInfo().getIntraTaskCFGLeafContentsOfSameParLevel().stream().anyMatch(n -> n.getNode() instanceof BarrierDirective)) {
            return false;
        }
    ...
    /**
     * Performs automated updates to phase information, and information about inter-task edges, upon insertion of the
     * node {@code node}.
     * <br>
     * Note that {@code node} has already been inserted in its specific location in the CFG.
     *
     * @param node
    ...
    /**
     * In case of update to {@link BeginPhasePoint} done via {@link BeginPhasePoint#removeNode(Node)}, {@link
     * BeginPhasePoint#deprecated_removeNextBarrier(EndPhasePoint)}, etc., this method just performs the updates, and
     * returns an empty set.
     * <br>
     * Otherwise, this method is used to obtain a set of BeginPhasePoints on which we need to call {@link
     * BeginPhasePoint#populateReachablesAndBarriers()}, after CFG updates have been done. Note that {@code
     * BeginPhasePoint#setInvalidFlag()} is already called by this method, and need not be called again by the callee.
     *
     * @param node
     *         node that has to be removed from the CFG.
     *
     * @return a set of BeginPhasePoints whose sets need to be regenerated upon removal of {@code node}, using {@link
    ...
    /**
     * Update the phase and inter-task edges information upon removal of certain nodes that have resulted in changes to
     * the sets of various begin-phase points in {@code bppSet}.
     * <br>
     * Note that this method is called on the modified state of the CFG.
     *
     * @param bppSet
     *         set of begin-phase points whose sets have been changed due to removal of certain nodes.
     */
    ...
    /**
     * Given a set of nodes, starting which the CFG paths may change, this method calls changes to phase and inter-task
    ...
    public static void reinitMHP() {
        ...
        BeginPhasePoint.stabilizationTime += (System.nanoTime() - timer);
        //		DumpSnapshot.dumpPhases("stable" + Program.mhpUpdateCategory + AutomatedUpdater.reinitMHPCounter);
    }
    ...
    public static void reinitIDFA(FlowAnalysis<?> f) {
        ...
        //		if (f instanceof PointsToAnalysis) {
        //			DumpSnapshot.dumpPointsTo("stable" + Program.updateCategory + f.autoUpdateTriggerCounter);
        //		}
    }
