...
public class DumpSnapshot {
    ...
    public static void dumpLoopHeadersOnSTDOUT() {
        ...
        for (Node loopHeader : loopHeaders) {
            ...
            for (Node loopTail : NaturalLoopAnalysis.getLoopTails(loopHeader)) {
                ...
                HashMap<Symbol, InductionRange> map = NaturalLoopAnalysis.getBasicInductionValueRange(loopHeader, loopTail, loopContents);
                for (Symbol sym : map.keySet()) {
                    InductionRange idr = map.get(sym);
                    System.out.println("\t Symbol " + sym + " has following constraints: ");
                    System.out.println(idr.getRanges(IdOrConstToken.getNewConstantToken("0"), IdOrConstToken.getNewIdToken("numTh")));
                }
            }
        }
    }
    ...
    public static void dumpPhaseAndCopyInfo(String str) {
        ...
        //			String tempStr = "";
        //			if (n instanceof DummyFlushDirective) {
        //				for (Node interSucc : n.getInfo().getCFGInfo().getInterTaskLeafSuccessorNodes()) {
    ...
    public static void dumpPredicates(String string) {
        ...
        commetors.add((n) -> {
            String tempStr = "";
            FlowFact flow = n.getInfo().getOUT(Program.useInterProceduralPredicateAnalysis ? AnalysisName.PREDICATE_ANALYSIS : AnalysisName.INTRA_PREDICATE_ANALYSIS);
            if (flow != null) {
                tempStr += "OUT: " + flow.getString();
            }
            return tempStr;
    ...
    public static void dumpReadSources() {
        ...
        commetors.add((n) -> {
            String tempStr = "";
            DataDependenceForwardFF ff = (DataDependenceForwardFF) n.getInfo().getIN(AnalysisName.DATA_DEPENDENCE_FORWARD);
    ...
    public static void dumpWriteSources() {
        ...
        commetors.add((n) -> {
            String tempStr = "";
            DataDependenceForwardFF ff = (DataDependenceForwardFF) n.getInfo().getIN(AnalysisName.DATA_DEPENDENCE_FORWARD);
            DataDependenceForwardFF ffOUT = (DataDependenceForwardFF) n.getInfo().getOUT(AnalysisName.DATA_DEPENDENCE_FORWARD);
    ...
    /**
     * For debugging purposes This method dumps the mhp information obtained so far for all the leaf CFG nodes
     *
     * @param n
     */
    ...
    /**
     * For debugging purposes This method dumps the parallelRegion hashCodes for all the leaf CFG nodes
     *
     * @param n
     */
    ...
    public static void dumpRootOrError(String string, List<SideEffect> sideEffectList) {
        ...
        // Old code: Now, we have annotated each constraint as a comment at the node where the constraint occurred.
    ...
    /**
     * This method returns a BufferedWriter object which wraps within it a stream to write to a file for dot graphs
     * generation.
     *
     * @param fileName
     *
