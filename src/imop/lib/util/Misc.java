...
/**
 * This class contains all those functions which can commonly be required across multiple visitors.
 *
 * @author aman
 */
public class Misc {
     ...
     *
     * @return {@code true}, if a method with the given method name is present in the call-stack.
     */
    ...
    /**
     * This method populates the Info.lockSet of various CFG nodes in the root, on the basis of syntactic enclosure of
     * nodes with critical/atomic regions. Note: To remain conservative, any statement within a called function from a
     * critical region, is not marked as locked with that region, since the function may be called from other contexts
     * outside the region.
     *
     * @param root
     */
    ...
    /**
     * For a given root node, this method calls the appropriate visitors to perform reaching definitions analysis in
     * root.
     *
     * @param root
     */
    ...
    /**
     * This method tells whether two given vectors of PhasePoints have subType relation or not. If newPoints are
    ...
    /**
     * This method obtains the ParallelConstructs of n, and calls the ParallelRegionMarker on them.
     *
     * @param n
     */
    public static void markParallelRegions(Node n) {
        ...
        for (ParallelConstruct parConstruct : parConstructList) {
            ...
            // of its statements.
            parConstruct.getInfo().getCFGInfo().getNestedCFG().getBegin().accept(new ParallelRegionMarker(), parConstruct);
        }
    }
    ...
    /**
     * This method is used to put escape characters in a string having " and '. This is required to be called on str of
     * StringGetter
     *
     * @param str
     *
    ...
    /**
     * This method is used to put escape characters in a string having " and '. And, this method also removes the spaces
     * from given string.
     *
     * @param str
     *
    ...
    public static Declaration getRenamedDeclaration(Declaration inDecl, String newName) {
        ...
        // Find the exact node that specifies the name, and then use getNodeReplacedString.
    ...
    /**
     * Returns a declaration for given symbol name ({@code tempName}), as a copy of {@code baseDecl} with renaming
     * performed as per {@code initRenamingMap}.
     * <br>
     *
     * @param baseDecl
     *         a base declaration which is source of the copy that we wish to create.
     * @param tempName
     *         name of the new symbol to be created with the return declaration.
     * @param initRenamingMap
     *         renaming map to be followed in the initializer.
     *
     * @return a new declaration for symbol with name {@code tempName}, type as this type object, and static specifier
     * and renaming initializer as present in {@code baseDecl}, if any.
     */
    public static Declaration getRenamedDeclarationWithRenamedInit(Declaration baseDecl, String tempName, HashMap<String, String> initRenamingMap) {
        return (Declaration) BasicTransform.obtainRenamedNode(Misc.getRenamedDeclaration(baseDecl, tempName), initRenamingMap);
    }

    /**
     * // * @param name // * @param node // * @return Union entry for the given id named "name", when used in given // *
     * "node". Returns null, if entry not found in any of the outer // * nesting parents of "node". //
     */
    ...
    //				HashMap<String, UnionType> typeTable = ((RootInfo)scopeChoice.getInfo()).getTypeTable();
    ...
    //				HashMap<String, UnionType> unionTable = ((CompoundStatementInfo)scopeChoice.getInfo()).getTypeTable();
    ...
    //		while (scopeChoice != null) {
    //			if (scopeChoice instanceof TranslationUnit) {
    //				HashMap<String, EnumType> enumTable = ((RootInfo)scopeChoice.getInfo()).getEnumTable();
    ...
    //				// Do nothing.
    //			} else if (scopeChoice instanceof CompoundStatement) {
    //				HashMap<String, EnumType> enumTable = ((CompoundStatementInfo)scopeChoice.getInfo()).getEnumTable();
    //				if (enumTable.containsKey(name)) {
    //					return enumTable.get(name);
    //				}
    //			}
    ...
    //		while (scopeChoice != null) {
    //			if (scopeChoice instanceof TranslationUnit) {
    //				HashMap<String, StructType> structTable = ((RootInfo)scopeChoice.getInfo()).getStructTable();
    ...
    //				// Do nothing.
    //			} else if (scopeChoice instanceof CompoundStatement) {
    //				HashMap<String, StructType> structTable = ((CompoundStatementInfo)scopeChoice.getInfo()).getStructTable();
    //				if (structTable.containsKey(name)) {
    //					return structTable.get(name);
    //				}
    //			}
    ...
    /**
     * An elementary node is the last level of nested node which we use for getting reads/writes, generally. If input is
     * a ParallelConstruct, ParallelForConstruct, ParallelSectionsConstruct or TaskConstruct, then the corresponding
    ...
    public static NodePair getBlockNeighbours(Node sourceNode, Node destinationNode, PivotPoint pivotPoint) {
        // Remove the complexity of handling EndNode and BeginNode on one side of the edge.
    ...
    /**
     * Returns an object of exact-type {@code Statement} that (inclusively) wraps {@code node}.<br> Note that this
     * method doesn't create new AST ancestors for {@code node}, if they are already present.
     *
     * @param node
    ...
    /**
     * Without changing any part of the tree above {@code node}, this method tries to return back the
     * CompoundStatementElement object that wraps {@code node}.<br> If no such object exists, appropriate creation of
     * missing nodes is performed.
     *
     * @param node
     *         CFG node for which a wrapping CompoundStatementElement has to be found.
     *
     * @return an existing or new CompoundStatementElement that wraps the {@code node}.
     */
    ...
    /**
     * This method takes a CFG node and returns the first leaf node encountered within it. Should be used to find the
     * leaf successors (given n is a leaf/non-leaf successors). Note the behavior when n is a BeginNode or EndNode
     *
     * @param n
     *
     * @return
    ...
    /**
     * This method returns the first CFG node encountered in the depth-first traversal of the AST rooted at
    ...
    /**
     * This method returns a set of nodes of type $astType$, present within the AST of node $n$ (inclusive).
     *
     * @param n
    ...
    public static <T extends Node> Set<T> getInheritedEnclosee(Node n, Class<T> astType, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new HashSet<>();
        }
    ...
    /**
     * This method returns a set of nodes of type with given {@code classId}, present within the AST of node $n$
     * (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         type of node whose assignable objects are to be searched for.
     *
     * @return set of nodes under {@code n} that are of type with given {@code classId}.
     */
    ...
    public static <T extends Node> Set<T> getInheritedEncloseeWithId(Node n, int classId, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new HashSet<>();
        }
        if (classId == 897) {
    ...
    /**
     * This method returns a list of nodes of type $astType$, present within the AST of node $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     * @param astType
    ...
    public static <T extends Node> List<T> getInheritedEncloseeList(Node n, Class<T> astType, TraverseExpressions traverseExpressions) {
        if (n == null) {
    ...
    /**
     * This method returns a list of nodes of type with given {@code classId}, present within the AST of node $n$
     * (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         type of node whose assignable objects are to be searched for.
     *
     * @return list of nodes under {@code n} that are of type with given {@code classId}.
     */
    ...
    public static <T extends Node> List<T> getInheritedEncloseeListWithId(Node n, int classId, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new LinkedList<>();
        }
        if (classId == 897) {
        ...
        if (Node.superClassIds.contains(classId)) {
            InfiInheritedASTNodeListGetter<T> nodeGetter = new InfiInheritedASTNodeListGetter<>(classId, traverseExpressions);
            n.accept(nodeGetter);
            return nodeGetter.astContents;
        } else {
    ...
    /**
     * This method returns a set of nodes of exact type $astType$, present within the AST of node $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     * @param astType
     *         exact type of node whose assignable objects are to be searched for.
     *
    ...
    public static <T extends Node> Set<T> getExactEnclosee(Node n, Class<T> astType, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new HashSet<>();
        }
    ...
    /**
     * This method returns a set of nodes of exact type with given {@code classId}, present within the AST of node $n$
     * (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         exact type of node whose assignable objects are to be searched for.
     *
     * @return set of nodes under {@code n} that are of exact type with given {@code classId}.
     */
    ...
    public static <T extends Node> Set<T> getExactEncloseeWithId(Node n, int classId, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new HashSet<>();
        }
        if (classId % 3 == 0) {
    ...
    /**
     * This method returns a list of nodes of exact type $astType$, present within the AST of node $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     * @param astType
    ...
    public static <T extends Node> List<T> getExactEncloseeList(Node n, Class<T> astType, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new LinkedList<>();
        }
    ...
    /**
     * This method returns a list of nodes of exact type with given {@code classId}, present within the AST of node $n$
     * (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         exact type of node whose assignable objects are to be searched for.
     *
     * @return list of nodes under {@code n} that are of exact type with given {@code classId}.
     */
    ...
    public static <T extends Node> List<T> getExactEncloseeListWithId(Node n, int classId, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new LinkedList<>();
        }
        if (classId % 3 == 0) {
    ...
    /**
     * This method returns a set of nodes of any of the types in $astType$, present within the AST of node $n$
     * (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     * @param astTypeSet
     *         set of type of nodes whose assignable objects are to be searched for.
     *
     * @return set of nodes under {@code n} that are of any of the types in {@code astTypeSet}.
     */
    ...
    public static Set<Node> getInheritedEnclosee(Node n, Set<Class<? extends Node>> astTypeSet, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new HashSet<>();
        }
    ...
    /**
     * This method returns a set of nodes of any of the types with classId in $classIds$, present within the AST of node
     * $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     * @param classIds
     *         set of classId of types whose assignable objects are to be searched for.
     *
     * @return set of nodes under {@code n} that are of any of the types with classId in {@code classIds}.
     */
    ...
    public static Set<Node> getInheritedEncloseeWithId(Node n, Set<Integer> classIds, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new HashSet<>();
        }
        ...
        if (notAStmtType) {
            if (Misc.doIntersect(Node.superClassIds, classIds)) {
                InfiInheritedMultiASTNodesGetter nodeGetter = new InfiInheritedMultiASTNodesGetter(classIds, traverseExpressions);
                n.accept(nodeGetter);
                return nodeGetter.astContents;
            } else {
            ...
            if (hasASuperStmt) {
                InfiInheritedMultiTypeStatementGetter<Node> nodeGetter = new InfiInheritedMultiTypeStatementGetter<>(classIds, returnSet);
                n.accept(nodeGetter);
                return (Set<Node>) returnSet;
            } else {
                InfiExactMultiTypeStatementGetter<Node> nodeGetter = new InfiExactMultiTypeStatementGetter<>(classIds, returnSet);
                n.accept(nodeGetter);
                return (Set<Node>) returnSet;
    ...
    /**
     * This method returns a list of nodes of any of the types in $astType$, present within the AST of node $n$
     * (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         set of type of nodes whose assignable objects are to be searched for.
     *
     * @return list of nodes under {@code n} that are of any of the types in {@code astTypeSet}.
     */
    ...
    public static List<Node> getInheritedEncloseeList(Node n, Set<Class<? extends Node>> astTypeSet, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new LinkedList<>();
        }
    ...
    /**
     * This method returns a list of nodes of any of the types with classId in $classIds$, present within the AST of
     * node $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         set of classId of types whose assignable objects are to be searched for.
     *
     * @return list of nodes under {@code n} that are of any of the types with classId in {@code classIds}.
     */
    ...
    public static List<Node> getInheritedEncloseeListWithId(Node n, Set<Integer> classIds, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new LinkedList<>();
        }
        boolean notAStmtType = false;
        ...
        if (notAStmtType) {
            if (Misc.doIntersect(Node.superClassIds, classIds)) {
                InfiInheritedMultiASTNodeListGetter nodeGetter = new InfiInheritedMultiASTNodeListGetter(classIds, traverseExpressions);
                n.accept(nodeGetter);
                return nodeGetter.astContents;
            } else {
            ...
            if (hasASuperStmt) {
                InfiInheritedMultiTypeStatementGetter<Node> nodeGetter = new InfiInheritedMultiTypeStatementGetter<>(classIds, returnList);
                n.accept(nodeGetter);
                return (List<Node>) returnList;
            } else {
                InfiExactMultiTypeStatementGetter<Node> nodeGetter = new InfiExactMultiTypeStatementGetter<>(classIds, returnList);
                n.accept(nodeGetter);
                return (List<Node>) returnList;
    ...
    /**
     * This method returns a set of nodes of any of the exact-types in $astType$, present within the AST of node $n$
     * (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     * @param astTypeSet
     *         set of exact-type of nodes whose assignable objects are to be searched for.
     *
     * @return set of nodes under {@code n} that are of any of the exact-types in {@code astTypeSet}.
     */
    ...
    public static Set<Node> getExactEnclosee(Node n, Set<Class<? extends Node>> astTypeSet, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new HashSet<>();
        }
    ...
    /**
     * This method returns a set of nodes of any of the exact-types with classId in $classIds$, present within the AST
     * of node $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     * @param classIds
     *         set of classId of exact-types whose assignable objects are to be searched for.
     *
     * @return set of nodes under {@code n} that are of any of the exact-types with classId in {@code classIds}.
     */
    ...
    public static Set<Node> getExactEncloseeWithId(Node n, Set<Integer> classIds, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new HashSet<>();
        }
        boolean notAStmtType = false;
        ...
        if (notAStmtType) {
            InfiExactMultiTypeASTNodesGetter nodeGetter = new InfiExactMultiTypeASTNodesGetter(classIds, traverseExpressions);
            n.accept(nodeGetter);
            return nodeGetter.astContents;
        } else {
            Collection<Node> returnSet = new HashSet<>();
    ...
    /**
     * This method returns a list of nodes of any of the exact-types in $astType$, present within the AST of node $n$
     * (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         set of exact-type of nodes whose assignable objects are to be searched for.
     *
     * @return list of nodes under {@code n} that are of any of the exact-types in {@code astTypeSet}.
     */
    ...
    public static List<Node> getExactEncloseeList(Node n, Set<Class<? extends Node>> astTypeSet, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new LinkedList<>();
        }
    ...
    /**
     * This method returns a list of nodes of any of the exact-types with classId in $classIds$, present within the AST
     * of node $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         set of classId of exact-types whose assignable objects are to be searched for.
     *
     * @return list of nodes under {@code n} that are of any of the exact-types with classId in {@code classIds}.
     */
    ...
    public static List<Node> getExactEncloseeListWithId(Node n, Set<Integer> classIds, TraverseExpressions traverseExpressions) {
        if (n == null) {
            return new LinkedList<>();
        }
        boolean notAStmtType = false;
        ...
        if (notAStmtType) {
            InfiExactMultiTypeASTNodeListGetter nodeGetter = new InfiExactMultiTypeASTNodeListGetter(classIds, traverseExpressions);
            n.accept(nodeGetter);
            return nodeGetter.astContents;
        } else {
            Collection<Node> returnList = new ArrayList<>();
    ...
    /**
     * This method returns a pre-order list of nodes of type $astType$, present within the AST of node $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     * @param astType
     *         type of node whose assignable objects are to be searched for.
     *
     * @return pre-order list of nodes under {@code n} that are of type {@code astType}.
     */
    ...
    /**
     * This method returns a pre-order list of nodes of any of the types in $astType$, present within the AST of node
     * $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         set of type of nodes whose assignable objects are to be searched for.
     *
     * @return pre-order list of nodes under {@code n} that are of any of the types in {@code astTypeSet}.
     */
    ...
    /**
     * This method returns a pre-order list of nodes of exact-type $astType$, present within the AST of node $n$
     * (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         exact-type of node whose assignable objects are to be searched for.
     *
     * @return pre-order list of nodes under {@code n} that are of exact-type {@code astType}.
     */
    ...
    /**
     * This method returns a pre-order list of nodes of any of the exact-types in $astType$, present within the AST of
     * node $n$ (inclusive).
     *
     * @param n
     *         node under which search needs to be performed
     ...
     *         set of exact-type of nodes whose assignable objects are to be searched for.
     *
     * @return pre-order list of nodes under {@code n} that are of any of the exact-types in {@code astTypeSet}.
     */
    ...
    /**
     * This method returns a vector of nodes representing case statements and default statements in the current level.
     * In case of nested switches, the internal cases are not returned.
     *
     * @param n
     *         switch statement for which the list of cases and default are to be found.
     *
     * @return list of case/default statements for {@code switchStmt}
     *
     * @deprecated Use {@link SwitchStatementInfo#getCaseDefaultLabelStatementList()} instead.
     */
    @Deprecated
    ...
    /**
     * This method returns the column number of the first token encountered in the node n.
     *
     * @param n
     *
     * @return
    ...
    /**
     * This method returns the line number of the first token encountered in the node n.
     *
     * @param n
     *
     * @return
    ...
    public static Node getEnclosingNode(Node n, Set<Class<?>> classList) {
        ...
        if (n.getParent() == null) {
            ...
            //			}
            //			System.out.println("No owner node of any of the types (" + nameList.toString() + ") found for node "
     ...
     *
     * @return {@code null} if there is no enclosing node of class type {@code c} for node {@code n}.
     */
    @SuppressWarnings("unchecked")
    public static <T> T getEnclosingNode(Node n, Class<T> c) {
        ...
        if (n.getParent() == null) {
            //			System.out.println(
            //					"No owner node of type " + c.getSimpleName() + " found for node " + n.getClass().getSimpleName());
            return null;
        }
    ...
    public static Node getEnclosingLoopOrForConstruct(Node n) {
        ...
        Node parent = n.getParent();
        if (parent == null) {
            //			System.out.println("No owner loop found for node " + n.getClass().getSimpleName());
            return null;
        }
    ...
    /**
     * This method returns the enclosing loop or switch for the given statement (exclusively).
     *
     * @param n
     *
     * @return
    ...
    public static Node getEnclosingLoopOrSwitch(Node n) {
        ...
        Node parent = n.getParent();
        if (parent == null) {
            //			System.out.println("No owner loop/switch found for node " + n.getClass().getSimpleName());
            return null;
        }
    ...
    /**
     * This method returns the enclosing CompoundStatement, FunctionDefinition or TranslationUnit of a given node
     * (exclusively).
     *
     * @param n
     *
    ...
    //	 */
    //	public static Type_obsolete performArithmeticConversion(Type_obsolete one, Type_obsolete two) {
    //		if (one == null) {
    //			return two;
    //		} else if (two == null) {
    //			return one;
    ...
    //		DeclarationSpecifiers declSpecRet = Misc.performArithmeticConversion(declSpecOne, declSpecTwo);
    ...
    //	 *         and two.
    //	 */
    //	public static DeclarationSpecifiers performArithmeticConversion(DeclarationSpecifiers one,
    ...
    //		if ((hasLongType(one) && hasDoubleType(one)) || (hasLongType(two) && hasDoubleType(two))) {
    //			isLong = true;
    //			isDouble = true;
    ...
    //			isInt = true;
    //			isUnsigned = true;
    //		} else if ((hasLongType(one) && hasIntType(one)) || (hasLongType(two) && hasIntType(two))) {
    //			/*
    ...
    //			isLong = true;
    //			isInt = true;
    //		} else if ((hasIntType(one) && hasUnsignedType(one)) || (hasIntType(two) && hasUnsignedType(two))) {
    //			isInt = true;
    //			isUnsigned = true;
    //		} else if (hasStructOrUnionType(one) || hasEnumType(one) || hasStructOrUnionType(two) || hasEnumType(two)) {
    ...
    //			return (DeclarationSpecifiers) CParser.createCrudeASTNode(typeSpecifierString, DeclarationSpecifiers.class);
    //		}
    //		return null;
    //	}
    ...
    //	 *         function definition.
    //	 */
    //	public static Type_obsolete getTypeFromParameter(FunctionDefinition funcDef, String paramName) {
    //		String declSpecString = new String();
    //		String absOptDeclString = new String();
    //		ExpandedTypeStringGetter typeStringGetter = new ExpandedTypeStringGetter(paramName);
    //
    //		// Check whether the parameter declarations are new-style or old
    //		// UPDATE 25/08: They will be new-style always
    //		ADeclaratorOp firstDeclOpChoice = (ADeclaratorOp) funcDef.f1.f1.f1.f0.nodes.get(0);
    ...
    //				return null;
    //			ParameterTypeList paramTypeList = (ParameterTypeList) ((ParameterTypeListClosed) firstDeclOp).f1.node;
    //			ParameterDeclaration paraDecl = getParameterDeclarationFromParameterList(paramTypeList.f0, paramName);
    //
    ...
    //			absOptDeclString = typeStringGetter.absOptDeclPre + " " + typeStringGetter.absOptDeclPost;
    //		} else {
    //			System.out.println(
    //					"Exiting for " + paramName + ": " + ((FunctionDefinitionInfo) funcDef.getInfo()).getFunctionName());
    //			Thread.dumpStack();
    //			System.exit(0);
    //		}
    //		DeclarationSpecifiers declSpec = (DeclarationSpecifiers) CParser.createCrudeASTNode(declSpecString,
    ...
    //					HashMap<Operator, Integer> thisOpCount = Misc.countOperator((Expression) exp);
    ...
    /**
     * Given an element $elem$, this method returns that element ($wrapper$) of the $list$, which wraps $elem$ and for
    ...
    /**
     * Checks if there are any free identifiers in {@code n}, that have been declared in {@code scope}.
     *
     * @param n
     ...
     *
     * @return true, if {@code n} reads/writes to any free identifier that may be declared within {@code scope}.
     */
    ...
    /**
     * Checks if there are any free identifiers in {@code n}, that have been declared in any scope between {@code
     ...
     * @param point
     *         program point in whose scope the node {@code n} has to be inserted.
     *
     * @return true, if {@code n} reads/writes to any free identifier that may be declared within {@code scope}.
     */
    ...
    //			LabeledStatement labStmt = (LabeledStatement) Misc.getEnclosingNode(tempNode, LabeledStatement.class);
    //			labels.add(labStmt);
    ...
    /**
     * This method is used to query whether two nodes, n1 and n2 may happen in parallel. Note: If n1 and n2 may happen
     * in parallel, it doesn't necessarily mean that they constitute a data race. Constraints put up by atomicity are
     * ignored in this method due to the following reason: If a function call is made from the atomic region, then the
     * reads and writes within the called function won't be atomic. So to be conservative, we assume that even when n1
     * and n2 both are encapsulated in atomic regions, they may happen in parallel. (In fact, it is allowed to
     * interleave the constituent operations of n1 and n2, as long as the read/write of the target variable appears to
     * happen in isolation.)
     *
     * @param n1
     *         This should be a CFG node. Otherwise, the function takes the internal CFG node, if any. Otherwise, the
     *         CFG encloser node.
     * @param n2
     *         // Same as n1.
     *
    ...
    public static boolean mayHappenInParallel(Node n1, Node n2) {
        ...
        if (Program.concurrencyAlgorithm == Program.ConcurrencyAlgorithm.YUANMHP) {
            ifVal = !Misc.doIntersect((Set<YPhase>) n1.getInfo().getNodePhaseInfo().getPhaseSet(), (Set<YPhase>) n2.getInfo().getNodePhaseInfo().getPhaseSet());
        } else {
            ifVal = !Misc.doIntersect((Set<Phase>) n1.getInfo().getNodePhaseInfo().getPhaseSet(), (Set<Phase>) n2.getInfo().getNodePhaseInfo().getPhaseSet());
        }
        if (ifVal) {
    ...
    /**
     * Returns a vector of call-sites that are present within $n$ after the call-site $callSite$.
     *
     * @param n
     * @param callSite
     *
    ...
    /**
     * Returns a vector of call-sites that are present within $n$ before the call-site $callSite$ in a reverse order.
     *
     * @param n
     * @param callSite
     *
    ...
    /**
     * Returns a new list with elements of <code>inList</code> stored in reverse order.
     *
     * @param inList
     *         the input list.
     *
     * @return a new list with elements of <code>inList</code> stored in reverse order.
     */
    ...
    /**
     * Returns the singleton element of a set. If the set doesn't contain exactly one element, then null is returned.
     *
     * @param workList
     *
     * @return
    ...
    /**
     * Returns any element of an unordered set. If the set is empty, this method returns null.
     *
     * @param set
     *
     * @return
     ...
     *
     * @return a reference to simple-primary expression if {@code inExp} is an identifier or a constant; else null.
     */
     ...
     *
     * @param ompConstruct
     *         An OpenMP construct from which the clauses need to be extracted.
     *
    ...
    /**
     * Returns true if <code>exp</code> is an expression of a type that can hold an object type.
     *
     * @param exp
     *
     * @return true if the expression <code>exp</code> is of a type that can hold an object type.
     */
    ...
    /**
     * Returns a hash-set of the type of all the members that are recursively present in the contained aggregates
     * (arrays/structs) or unions. An empty set is returned when <code>type</code> is not an aggregate or a union type.
     *
     * @param type
     *
     * @return
     ...
     *
     * @return {@code node} if it is a CFG node; else the internal-CFG node, or the external-CFG node, in that order.
     */
    ...
    /**
     * Checks whether {@code exp} is a predicate (CFG expression), or not. Note that OmpForCondition also is considered
     * as a predicate.
     *
     * @param node
    ...
    /**
     * Checks whether {@code exp} is a CFG expression, or not. Note that OmpForInitExpression, OmpForReinitExpression,
     * and OmpForCondition are not considered as CFG expressions, for some unknown reasons. (Maybe we should consider
     ...
     * @param str
     *
     * @return true if {@code str} represents a mathematical expression without free-variables.
     */
    ...
    /**
     * Obtain the least common ancestor in the nesting tree of scopes, for scopes of {@code one} and {@code two}.
     *
     * @param one
     *         first node.
     * @param two
     *         second node.
     *
     * @return the least common ancestor in the nesting tree of scopes, for scopes of {@code one} and {@code two}.
     */
    ...
    public static String getStringFromInputStream(InputStream is) {
        @SuppressWarnings("resource") Scanner s = new Scanner(is).useDelimiter("\\A");
        return s.hasNext() ? s.next() : "";
    }

    public static void changeFunctionReturnTypeToStruct(FunctionDefinition func, String structName) {
        if (func != null) {
            DeclarationSpecifiers declSpec = FrontEnd.parseAndNormalize(
                    "struct " + structName, DeclarationSpecifiers.class);
    ...
    /**
     * Obtain a string equivalent of a ByteArrayInputStream, while consuming the stream completely.
     *
     * @param in
    ...
    /**
     * This method is used to query whether two nodes, n1 and n2, may result into an atomicity race condition.
     *
     * @param n1
     *         This should be a CFG node. Otherwise, the function takes the internal CFG node, if any. Otherwise, the
     *         CFG encloser node.
    ...
    public static boolean isAtomicityRacePair(Node n1, Node n2) {
        ...
        AtomicConstructInfo atomN1 = Misc.getEnclosingNode(n1, AtomicConstruct.class).getInfo(); // can't return null since lockSet contains La already.
        AtomicConstructInfo atomN2 = Misc.getEnclosingNode(n2, AtomicConstruct.class).getInfo(); // can't return null since lockSet contains La already.
    ...
    /**
     * This method returns the set of all the nodes on which $n$ depends, except the control dependees of $n$.
     *
     * @param n
     *
     * @return
    ...
    public static Set<Node> getAllDataDependees(Node n) {
        ...
        //			}
        //		} else {
        //			for (Node internal : n.getInfo().getCFGInfo().getLexicalIntraProceduralCFGContents()) {
        ...
        //			 */
        //			Set<Node> internalNodes = n.getInfo().getCFGInfo().getLexicalIntraProceduralCFGContents();
    ...
    private static boolean old_hasDefaultLabel(Node n) {
        // OLD CODE:
        //		SwitchRelevantStatementsGetter caseGetter = new SwitchRelevantStatementsGetter();
        //		n.accept(caseGetter);
     ...
     * @return
     *
     * @deprecated Use {@link #hasEqualCallSiteStacks(Stack<CallSite>,Stack<CallSite>)} instead
     */
    @Deprecated
    ...
    /**
     * @param nodeToken
     *         a {@link NodeToken} object for which we need to obtain the related symbol or free variable.
     *
     * @return reference to the symbol that is referred to by the {@code nodeToken}. If nodeToken refers to a
     * free-variable, a corresponding {@link FreeVariable} object is returned.
     */
    ...
    public static Typedef getTypedefEntry(String name, Node node) {
        ...
        while (scopeChoice != null) {
            if (scopeChoice instanceof TranslationUnit) {
                ...
                HashMap<String, Typedef> typedefTable = ((CompoundStatementInfo) scopeChoice.getInfo()).getTypedefTable();
                if (typedefTable.containsKey(name)) {
                    return typedefTable.get(name);
                }
            }
     ...
     * @param node
     *
     * @return Symbol entry for the given id named "name", when used in given "node". Returns null, if entry not found
     * in any of the outer nesting parents of "node". Note that if the {@class node} itself is a scope, then its symbol
    ...
    public static Symbol getSymbolEntry(String name, Node node) {
        ...
        //		Misc.warnDueToLackOfFeature("Kindly ensure that symbol " + name + " has been declared at line #" + Misc.getLineNum(node), node);
        //		System.err.println(enclosingNode);
        return null;
    }

    /**
     * Checks if the symbol with the given {@code name} is present in the provided {@code library}.
     *
     * @param name
     *         name of the identifier corresponding to which a Symbol has to be found.
     * @param library
     *         reference to any of the built-in libraries that contain built-in symbols.
     *
     * @return a symbol corresponding to the given {@code name}, if found in the {@code library}; {@code null}
     * otherwise.
     */
