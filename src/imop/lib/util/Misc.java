...
import imop.lib.analysis.mhp.OldLocksetMarker;
import imop.lib.analysis.mhp.ParallelRegionMarker;
...
import imop.lib.analysis.typeSystem.StructOrUnionMember;
...
//import imop.projects.operatorCounter.Operator;
//import imop.projects.operatorCounter.OperatorCounter;
...
public class Misc {
	...
	/**
	 * This method populates the Info.lockSet of various CFG nodes in the root,
	 * on the basis of syntactic enclosure of nodes with critical/atomic
	 * regions. Note: To remain conservative, any statement within a called
	 * function from a critical region, is not marked as locked with that
	 * region, since the function may be called from other contexts outside the
	 * region.
	 * 
	 * @param root
	 */
	...
	/**
	 * This method populates various data dependence structures of elementary
	 * R/W CFG nodes.
	 * 
	 * @param root
	 */
	...
	/**
	 * For a given root node, this method calls the appropriate visitors to
	 * perform reaching definitions analysis in root.
	 * 
	 * @param root
	 */
	 ...
	 * relation or not. If newPoints are subTypes of oldPoints, then we can stop
	 * processing.
	 * 
	 * @param newPoints
	 * @param oldPoints
	 ...
	 * This is required to be called on str of StringGetter
	 * 
	 * @param str
	 * @return
	 */
	...
	/**
	 * This method is used to put escape characters in a string having " and '.
	 * And, this method also removes the spaces from given string.
	 * 
	 * @param str
	 * @return
	 */
	...
	/**
	 * Returns a declaration for given symbol name ({@code tempName}), as a copy
	 * of {@code baseDecl} with renaming performed as per
	 * {@code initRenamingMap}.
	 * <br>
	 * 
	 * @param baseDecl
	 *            a base declaration which is source of the copy that we wish to
	 *            create.
	 * @param tempName
	 ...
	 * @return
	 *         a new declaration for symbol with name {@code tempName}, type as
	 *         this type object, and static specifier and renaming initializer
	 *         as present in {@code baseDecl}, if any.
	 */
	 ...
	 * getting reads/writes, generally. If input is a ParallelConstruct,
	 * ParallelForConstruct, ParallelSectionsConstruct or TaskConstruct, then
	 * the corresponding non-CFG fields (f1,f3, f3, f2) are returned.
	 * 
	 * @param n
	...
	/**
	 * Returns an object of exact-type {@code Statement} that (inclusively)
	 * wraps {@code node}.<br>
	 * Note that this method doesn't create new AST ancestors for {@code node},
	 * if they are already present.
	 * 
	 * @param node
	 *            a node of a proper sub-type of Statement.
	 * @return
	 *         a node of exact-type Statement, that wraps {@code node}.
	 * 
	 */
	...
	/**
	 * Without changing any part of the tree above {@code node}, this method
	 ...
	 * 
	 * @param node
	 *            CFG node for which a wrapping CompoundStatementElement has to
	 *            be found.
	 * @return
	 *         an existing or new CompoundStatementElement that wraps the
	 *         {@code node}.
	 */
	 ...
	 * within it. Should be used to find the leaf successors (given n is a
	 * leaf/non-leaf successors). Note the behavior when n is a BeginNode or
	 * EndNode
	 * 
	 * @param n
	 * @return
	 ...
	 * traversal
	 * of the AST rooted at <code>n</code> (inclusive).
	 * 
	 * @param n
	 *            node in which the CFG node has to be found.
	 * @return
	 *         the first CFG node encountered in the depth-first traversal of
	 *         <code>n</code>.
	 */
	 ...
	 * default statements
	 * in the current level. In case of nested switches, the internal cases are
	 * not returned.
	 * 
	 * @param n
	 *            switch statement for which the list of cases and default are
	 *            to be found.
	 * @return
	 *         list of case/default statements for {@code switchStmt}
	 * @deprecated Use
	 ...
	 *            a node.
	 * @return
	 *         all semantically (CFG) enclosing {@link BeginNode} nodes.
	 */
	 ...
	 * @return
	 *         null if there is no enclosing node of class type $c$ for node
	 *         $n$.
	 */
	...
	/**
	 * @param n
	 *            an AST/CFG node whose enclosing node (inclusive) has to be
	 *            found.
	 * @param c
	 *            an exact class type for the enclosing node.
	 * @return
	 *         {@code null} if there is no enclosing node of class type
	 *         {@code c} for node {@code n}.
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * @return
	 *         true, if {@code n} reads/writes to any free identifier that may
	 *         be
	...
	/**
	 * Checks if there are any free identifiers in {@code n}, that have been
	 * declared in any scope between {@code scope}, and the scope of
	 * {@code point}, inclusively.
	 * 
	 * @param n
	 ...
	 * parallel. Note: If n1 and n2 may happen in parallel, it doesn't
	 * necessarily mean that they constitute a data race. Constraints put up by
	 * atomicity are ignored in this method due to the following reason: If a
	 * function call is made from the atomic region, then the reads and writes
	 * within the called function won't be atomic. So to be conservative, we
	 * assume that even when n1 and n2 both are encapsulated in atomic regions,
	 * they may happen in parallel. (In fact, it is allowed to interleave the
	 * constituent operations of n1 and n2, as long as the read/write of the
	 * target variable appears to happen in isolation.)
	 * 
	 * @param n1
	 *            This should be a CFG node. Otherwise, the function takes the
	 *            internal CFG node, if any. Otherwise, the CFG encloser node.
	 * @param n2
	 *            // Same as n1.
	...
	public static boolean mayHappenInParallel(Node n1, Node n2) {
		...
		if (!Misc.doIntersect(n1.getInfo().getNodePhaseInfo().getPhaseSet(),
				n2.getInfo().getNodePhaseInfo().getPhaseSet())) {
	...
	/**
	 * Returns the singleton element of a set. If the set doesn't contain
	 * exactly one element, then null is returned.
	 * 
	 * @param workList
	 * @return
	 */
	 ...
	 * @return
	 *         a reference to simple-primary expression if {@code inExp}
	 *         is an identifier or a constant; else null.
	 */
	 ...
	 * @return
	 *         true if <code>inExp</code> directly wraps a function call.
	 */
	 ...
	 *            extracted.
	 * @return
	 *         A list of clauses that are present in
	 *         <code>ompConstruct</code>.
	 */
	...
	/**
	 * Returns true if <code>exp</code> is an expression of a type
	 * that can hold an object type.
	 * 
	 * @param exp
	 * @return
	 *         true if the expression <code>exp</code> is of a type that can
	 *         hold an object type.
	 */
	 ...
	 * An empty set is returned when <code>type</code> is not an aggregate or a
	 * union type.
	 * 
	 * @param type
	 * @return
	 ...
	 * @return
	 *         {@code node} if it is a CFG node; else the internal-CFG node, or
	 *         the external-CFG node, in that order.
	 */
	 ...
	 *            a node.
	 * @return
	 *         set of all the incomplete edges within the node {@code root}.
	 */
	...
	/**
	 * Checks whether {@code exp} is a predicate (CFG expression), or not.
	 * Note that OmpForCondition also is considered as a predicate.
	 * 
	 * @param node
	 *            node on which this check is requested.
	 * @return
	 *         true, if {@code node} is a predicate CFG node.
	 */
	...
	/**
	 * Checks whether {@code exp} is a CFG expression, or not.
	 * Note that OmpForInitExpression, OmpForReinitExpression, and
	 * OmpForCondition are not considered as CFG expressions, for some unknown
	 * reasons. (Maybe we should consider them to be Expression nodes.)
	 * 
	 * @param exp
	 *            expression that needs to be checked.
	 * @return
	 *         true, if the expression {@code exp} is a CFG expression.
	 */
	...
	/**
	 * Returns true if the string {@code str} can be evaluated to a constant
	 * value.
	 * 
	 * @param str
	 * @return
	 *         true if {@code str} represents a mathematical expression without
	 *         free-variables.
	 */
	...
	/**
	 * Obtain the least common ancestor in the nesting tree of scopes, for
	 * scopes of {@code one} and {@code two}.
	 * 
	 * @param one
	 *            first node.
	 * @param two
	 *            second node.
	 * @return
	 *         the least common ancestor in the nesting tree of scopes, for
	 *         scopes of {@code one} and {@code two}.
	 * 
	 */
	 ...
	 * 
	 * @param in
	 *            a ByteArrayInputStream, whose string equivalent has to be
	 *            obtained.
	 */
	...
	/**
	 * This method is used to query whether two nodes, n1 and n2, may result
	 * into an atomicity race condition.
	 * 
	 * @param n1
	 *            This should be a CFG node. Otherwise, the function takes the
	 *            internal CFG node, if any. Otherwise, the CFG encloser node.
	 ...
	 * @return
	 *         reference to the symbol that is referred to by the
	 *         {@code nodeToken}.
	 *         If nodeToken refers to a free-variable, a corresponding
	 *         {@link FreeVariable} object is returned.
	 * 
	 */
	 ...
	 *         "node". Returns null, if entry not found in any of the outer
	 *         nesting parents of "node".
	 *         Note that if the {@class node} itself is a scope, then its symbol
	...
	/**
	 * Checks if the symbol with the given {@code name} is present in the
	 ...
	 *            built-in symbols.
	 * @return
	 *         a symbol corresponding to the given {@code name}, if found in the
	 *         {@code library}; {@code null} otherwise.
	 */
