...
import imop.ast.node.external.MergeableClause;
...
import imop.lib.analysis.flowanalysis.dataflow.LockSetAnalysis.LockFlowFact;
...
import imop.lib.analysis.mhp.yuan.ConcurrencyAnalysis;
...
public class Main {
	...
	public static void demo10b(Statement stmt1, Statement stmt2) {
		stmt1 = (Statement) Misc.getCFGNodeFor(stmt1);
		stmt2 = (Statement) Misc.getCFGNodeFor(stmt2);
		if (!Misc.doIntersect(stmt1.getInfo().getNodePhaseInfo().getPhaseSet(),
				stmt2.getInfo().getNodePhaseInfo().getPhaseSet())) {
			System.out.println("Statements do not constitute a data-race pair.");
			return;
		}
		...
		LockFlowFact flowFact1 = (LockFlowFact) stmt1.getInfo().getIN(AnalysisName.LOCKSET_ANALYSIS);
		LockFlowFact flowFact2 = (LockFlowFact) stmt2.getInfo().getIN(AnalysisName.LOCKSET_ANALYSIS);
		if (flowFact1 == null || flowFact2 == null) {
			return;
		}
		HashMap<AbstractLock, Integer> map1 = flowFact1.getLockMapCopy();
		HashMap<AbstractLock, Integer> map2 = flowFact2.getLockMapCopy();
		if (map1 == null || map2 == null) {
			return;
		}
		System.out.println(map1 + "and" + map2);
		for (AbstractLock l1 : flowFact1.getLockMapCopy().keySet()) {
			for (AbstractLock l2 : flowFact2.getLockMapCopy().keySet()) {
				if (l1.equals(l2)) {
					System.out.println("Statements do not constitute a data-race pair.");
					return;
		...
		System.out.println("The following statements may conflict with each other: " + stmt1 + " and " + stmt2);
		System.exit(0);
	}

	public static void demo10a() {
		for (FunctionDefinition func : Program.getRoot().getInfo().getAllFunctionDefinitions()) {
			...
			if (stmt1 != null && stmt2 != null) {
				Main.demo10b(stmt1, stmt2);
			}
		}
	}

	public static void demo9() {
		for (Node barrierNode : Misc.getInheritedEncloseeList(Program.getRoot(), BarrierDirective.class)) {
	...
	public static void demo7() {
		...
		DumpSnapshot.dumpRoot("instrumented");
		System.exit(0);
	}
	...
	public static void demo4() {
		for (Declaration decl : Misc.getInheritedEnclosee(Program.getRoot(), Declaration.class)) {
			System.out.println(decl.getInfo().getDeclaredName());
		}
		Set<String> definedFuncNames = new HashSet<>();
		for (FunctionDefinition func : Misc.getInheritedEnclosee(Program.getRoot(), FunctionDefinition.class)) {
			definedFuncNames.add(func.getInfo().getFunctionName());
		}
		Set<String> declaredFuncNames = new HashSet<>();
		for (Symbol sym : Program.getRoot().getInfo().getSymbolTable().values()) {
			if (!sym.isAFunction()) {
				continue;
			}
			declaredFuncNames.add(sym.getName());
		}
		System.out.println(declaredFuncNames);
		System.out.println(definedFuncNames);
		System.out.println(Misc.setMinus(declaredFuncNames, definedFuncNames));

		for (Declaration decl : Misc.getInheritedEnclosee(Program.getRoot(), Declaration.class)) {
			Symbol s = decl.getInfo().getDeclaredSymbol();
			System.out.println(s.getName() + " has type " + s.getType());
		}
	...
	/**
	 * Performs queries on CFG components.
	 */
	...
	public static void demo11() {
		...
		IfStatement ifStmt = (IfStatement) funcDef.getInfo().getStatementWithLabel("ifStmt");
		ifStmt.getInfo().addEmptyElse();
	...
	public static void main(String[] args) throws FileNotFoundException, InterruptedException {
		...
		//		Statement newStmt = Program.getRoot().getInfo().getFunctionWithName("foo").getInfo().getStatementWithLabel("w");
		...
		//		testUnreachables();
		//		testCopyDetector();
		//		debugDeclarationEscalator(Misc.getAnyElement(Misc.getInheritedEnclosee(Program.root, WhileStatement.class)));
		//		testSVEness();
		//		testCoExistence();
		 ...
		 */
		//		for (FunctionDefinition funcDef : Program.root.getInfo().getAllFunctionDefinitions()) {
		...
		//						HashMap<String, Symbol> symbolTable = ((CompoundStatement) scope).getInfo().getSymbolTable();
		...
		//								System.out.println(sym.getDeclaringNode() + " is " + t.isKnownConstantSized());
		//							}
		//						}
		//					}
		//				}
		...
		//			Node beginNode = Main.root.getInfo().getMainFunction().getInfo().getCFGInfo().getNestedCFG().getBegin();
		//			Node endNode = Main.root.getInfo().getMainFunction().getInfo().getCFGInfo().getNestedCFG().getEnd();
		//			startSet.add(beginNode);
		//			System.out.println(CollectorVisitor.collectNodesForward(startSet, null, doesntMatter ::= false).stream()
		//					.filter((node) ::= !(node instanceof BeginNode || node instanceof EndNode)).count());
		//			System.out.println(
		//					CollectorVisitor.collectNodesForward(beginNode, new HashSet<>(), doesntMatter ::= false).size());
		//			System.out.println(
		//					CollectorVisitor.collectNodesBackward(endNode, new HashSet<>(), doesntMatter ::= false).size());
		//			System.out.println(CollectorVisitor.collectNodesForwardContextSensitive(
		//					new NodeWithStack(beginNode, new CallStack()), new HashSet<>(), doesntMatter ::= false).size());
		//			System.out.println(CollectorVisitor.collectNodesBackwardContextSensitive(
		//					new NodeWithStack(endNode, new CallStack()), new HashSet<>(), doesntMatter ::= false).size());
		/*
		...
		//						System.out.println("For node " + node.getClass().getSimpleName() + ": <<" + node
		...
		//			System.out.println("Time taken to create one copy (in milliseconds) : " + timeTaken / (1000000.0 * 28));
		 ...
		 */
		//			for (Node node : root.getInfo().getCFGNodes()) {
		//				ExpressionStatement expStmtTarget = (ExpressionStatement) CParser.justParse("x = 123;",
		...
		//				System.out.println("Context-insensitive inter-procedural successors of a(n) " + cfgNode.getClass().getSimpleName() + " " + cfgNode + " are --");
		//				int i = 0;
		//				for (Node succ: cfgNode.getInfo().getCFGInfo().getInterProceduralSuccessors()) {
		//					System.out.println(i++ + " : " + succ);
		//				}
		//				System.out.println("And predecessors are --");
		//				i = 0;
		//				for (Node pred: cfgNode.getInfo().getCFGInfo().getInterProceduralPredecessors()) {
		...
		//			CompoundStatement newStmt = (CompoundStatement) CParser.justParse("{bar(x, y);}", CompoundStatement.class);
		...
		//			for (Node parConsNode : Misc.getInheritedEnclosee(root, ParallelConstruct.class)) {
		//				ParallelConstruct parCons = (ParallelConstruct) parConsNode;
		//				System.err.println("Number of phases encountered in the parallel-construct at line #"
		//						+ Misc.getLineNum(parCons) + ":" + parCons.getInfo().getAllPhaseList().size());
		...
		//			Set<BeginPhasePoint> allBeginPhasePoints = BeginPhasePoint.getAllBeginPhasePointsCopy();
		//			System.err.println("Number of phases encountered in the program: " + allBeginPhasePoints.size());
		 ...
		 */
		//			OmpConstruct ompCons = (OmpConstruct) CParser.justParse("#pragma omp parallel \n {if (0) printf(\"abc\"); for (x = 0; x < 10; x++) i = i + 1; #pragma omp flush\n}", OmpConstruct.class);
		//			System.out.println(ompCons);
		//			for (Node expStmtNode : Misc.getInheritedEnclosee(ompCons, ExpressionStatement.class)) {
		...
		// root = (TranslationUnit) CParser.createRefinedASTNode(System.in, TranslationUnit.class);
		 ...
		 * rda.printFacts();
		 */
		//	System.err.println("==== Performing Memory Renaming to remove Anti-dependence on the shared variables ====");
		//	for (ParallelConstruct parCons : Misc.getParConstructs(root)) {
		//		RemoveAnti.removeAntiDependence(parCons);
		//	}
	...
	private static void checkSVEAnnotations() {
		...
		//		System.out.println("Time spent in call-stmt queries: " + NodeInfo.callQueryTimer / (1e9 * 1.0) + "s.");
		System.exit(0);
	}

	private static void criticalAnalyzer() {
		for (ParallelConstruct parCons : Misc.getInheritedEnclosee(Program.getRoot(), ParallelConstruct.class)) {
			for (Phase ph : parCons.getInfo().getConnectedPhases()) {
	...
	private static void testCopyDetector() {
		...
		//		for (ExpressionStatement exp : Misc.getInheritedEnclosee(Program.root, ExpressionStatement.class)) {
	...
	private static void testCopyElimination() {
		...
		//		for (SingleConstruct singleCons : Misc.getInheritedEnclosee(main, SingleConstruct.class)) {
		//			CompoundStatement compStmt = (CompoundStatement) singleCons.getInfo().getCFGInfo().getBody();
		...
		//		NodeRemover.removeNode(stmt);
		//		System.out.println("Statement has followers: " + stmt.getInfo().getAllUsesForwardsExclusively(w));
	...
	public static void testFencePercolation() {
		for (ParallelConstruct parConsNode : Misc.getInheritedEnclosee(Program.getRoot(), ParallelConstruct.class)) {
			for (Phase ph : parConsNode.getInfo().getConnectedPhases()) {
	...
	public static void testInsertImmediatePredecessor() {
		...
		for (FunctionDefinition function : Program.getRoot().getInfo().getAllFunctionDefinitions()) {
			for (Node n : function.getInfo().getCFGInfo().getLexicalCFGContents()) {
				...
				//					//					System.out.println(InsertImmediatePredecessor.insertAggressive(func.getInfo().getCFGInfo().getNestedCFG().getBegin(), expStmt));
				//					//					expStmt = FrontEnd.parseAndNormalize(i++ + ";", ExpressionStatement.class);
				//					//					System.out.println(InsertImmediatePredecessor.insertAggressive(func.getInfo().getCFGInfo().getNestedCFG().getEnd(), expStmt));
				//					//					expStmt = FrontEnd.parseAndNormalize(i++ + ";", ExpressionStatement.class);
				//					//					System.out.println(InsertImmediatePredecessor.insertAggressive(func.getInfo().getCFGInfo().getBody(), expStmt));
				//
				//					for (Node comp : func.getInfo().getCFGInfo().getAllComponents()) {
				//						expStmt = FrontEnd.parseAndNormalize(i++ + ";", ExpressionStatement.class);
				//						System.out.println(InsertImmediatePredecessor.insertAggressive(comp, expStmt));
				//					}
				//				}
				...
				//							.insertAggressive(parCons.getInfo().getCFGInfo().getNestedCFG().getEnd(), expStmt));
				...
				//					//					expStmt = FrontEnd.parseAndNormalize(i++ + ";", ExpressionStatement.class);
				//					System.out.println(InsertImmediatePredecessor
				//							.insertAggressive(parCons.getInfo().getCFGInfo().getReinitExpression(), expStmt));
				//					//					for (Node comp : parCons.getInfo().getCFGInfo().getAllComponents()) {
				//					//						expStmt = FrontEnd.parseAndNormalize("x = " + i++ + ";", ExpressionStatement.class);
				//					////						expStmt = FrontEnd.parseAndNormalize(i++ + ";", ExpressionStatement.class);
				//					//						System.out.println(InsertImmediatePredecessor.insertAggressive(comp, expStmt));
				...
				//					//					for (Node comp : parCons.getInfo().getCFGInfo().getAllComponents()) {
				//					//						//						expStmt = FrontEnd.parseAndNormalize("x = " + i++ + ";", ExpressionStatement.class);
				//					//						expStmt = FrontEnd.parseAndNormalize("i = " + i++ + ";", ExpressionStatement.class);
				//					//						System.out.println(InsertImmediatePredecessor.insertAggressive(comp, expStmt));
				//					//					}
				//				}
				//
				...
				//					expStmt = FrontEnd.parseAndNormalize("x = " + i++ + ";", ExpressionStatement.class);
				//					//					expStmt = FrontEnd.parseAndNormalize(i++ + ";", ExpressionStatement.class);
				//					Statement stmt = FrontEnd.parseAndNormalize("while(1) {continue;}", Statement.class);
				//					System.out.println(InsertImmediatePredecessor
				//							.insertAggressive(parCons.getInfo().getCFGInfo().getNestedCFG().getEnd(), stmt));
				//					//					for (Node comp : parCons.getInfo().getCFGInfo().getAllComponents()) {
				//					//						expStmt = FrontEnd.parseAndNormalize("x = " + i++ + ";", ExpressionStatement.class);
				//					//						//						expStmt = FrontEnd.parseAndNormalize(i++ + ";", ExpressionStatement.class);
				//					//						System.out.println(InsertImmediatePredecessor.insertAggressive(comp, expStmt));
				...
				//						expStmt = FrontEnd.parseAndNormalize(i++ + ";", ExpressionStatement.class);
				//						System.out.println(InsertImmediatePredecessor.insertAggressive(stmt, expStmt));
				//					}
				//				}
				//
				/////////////////////////////////////////////////////////////
				...
				//					expStmt = FrontEnd.parseAndNormalize((0.5 + i++) + ";", ExpressionStatement.class);
	...
	public static void testInsertImmediateSuccessor() {
		...
		for (FunctionDefinition function : Program.getRoot().getInfo().getAllFunctionDefinitions()) {
			for (Node n : function.getInfo().getCFGInfo().getLexicalCFGContents()) {
				...
				//							.insertAggressive(parCons.getInfo().getCFGInfo().getTerminationExpression(), expStmt));
				//
				//					//					for (Node comp : parCons.getInfo().getCFGInfo().getAllComponents()) {
				//					//						//						if (parCons.getInfo().getCFGInfo().getTerminationExpression() == comp) {
				//					//						//							continue;
	...
	public static void testInsertOnTheEdge() {
		...
		for (Node n : main.getInfo().getCFGInfo().getLexicalCFGContents()) {
			...
			if (n instanceof CompoundStatement) {
				...
				//					expStmt = FrontEnd.parseAndNormalize((200 + i++) + ";", ExpressionStatement.class);
				//					System.err.println(InsertOnTheEdge.insertOnTheEdge(b, succ, expStmt));
				//				}
				// EndNode
				//				for (Node pred : e.getInfo().getCFGInfo().getPredecessors()) {
				//					expStmt = FrontEnd.parseAndNormalize((100 + i++) + ";", ExpressionStatement.class);
				//					System.err.println(InsertOnTheEdge.insertOnTheEdge(pred, e, expStmt));
				//				}
				// Elsewhere.
				//				List<Node> elements = body.getInfo().getCFGInfo().getElementList();
	...
	private static void testNormalization() {
		...
		//				"{int y = 11; \n#pragma omp parallel for\nfor (i=0;i<10;i++){} int x = 10;}", CompoundStatement.class);
		...
		//		for (ForStatement forStmt : Misc.getInheritedEnclosee(Program.getRoot().getInfo().getMainFunction(),
		//				ForStatement.class)) {
		//			Expression exp = FrontEnd.parseAndNormalize("4 || 5 + foo()", Expression.class);
		//			forStmt.getInfo().getCFGInfo().setTerminationExpression(exp);
		//			ParallelForConstruct parForCons2 = FrontEnd.parseAndNormalize(parForCons.toString(),
		//					ParallelForConstruct.class);
		//			forStmt.getInfo().getCFGInfo().setBody(parForCons2);
		//		}
		//		for (WhileStatement whileStmt : Misc.getInheritedEnclosee(Program.getRoot().getInfo().getMainFunction(),
		//				WhileStatement.class)) {
		...
		//					.setBody((Statement) compStmt2.getInfo().getCFGInfo().getElementList().get(1));
		//		}
		System.out.println(Program.getRoot());
		DumpSnapshot.dumpRoot("final");
	...
	/**
	 * This sample pass adds a {@code printf} statement that prints
	 * {@code true}, before all pointer-dereference writes
	 * (i.e., before stores, such as {@code *p = ...;}.
	 * <p>
	 * NOTE: To use this method, paste it in the {@code Main} class, and invoke
	 * it after
	 * {@code Program.parseAndNormalize(args);} in {@code Main.main()}.
	 */
