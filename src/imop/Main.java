...
public class Main {
    ...
    private static void debugDeclarationEscalator(WhileStatement whileStmt) {
        DeclarationEscalator.pushAllDeclarationsUpFromLevel((CompoundStatement) whileStmt.getInfo().getCFGInfo().getBody());
    ...
    private static void profileStructure2() {
        for (IterationStatement itStmt : Misc.getInheritedEnclosee(Program.getRoot(), IterationStatement.class)) {
            ...
            for (NodeWithStack nws : itStmt.getInfo().getCFGInfo().getIntraTaskCFGLeafContentsOfSameParLevel(new CallStack())) {
    ...
    private static void temporaryChecker1() {
        ...
        SeedConstraint tempEquality = new SeedConstraint(sae1.getIndexExpression(), s1, IdOrConstToken.getNewIdToken("tid1"), sae2.getIndexExpression(), s2, IdOrConstToken.getNewIdToken("tid2"), OperatorToken.ASSIGN);
    ...
    private static void temporaryChecker3() {
        ...
        for (Node leaf1 : main.getInfo().getCFGInfo().getIntraTaskCFGLeafContents()) {
            ...
            SeedConstraint tempEquality = new SeedConstraint(e1, leaf1, IdOrConstToken.getNewIdToken("tid1"), e2, leaf1, IdOrConstToken.getNewIdToken("tid2"), OperatorToken.ASSIGN);
            Set<SeedConstraint> allEqualityConstraints = new HashSet<>();
            allEqualityConstraints.add(tempEquality);
            long timer = System.nanoTime();
            ConstraintsGenerator.reinitInductionSet();
    ...
    private static void testBarrierPercolation() {
        RedundantSynchronizationRemoval.removeBarriers(Program.getRoot());
        System.err.println("Skipping par-cons expansion and merge...\nMoving on to upward percolation and barrier removal.");
        ...
        for (BracketExpression brack : Misc.getInheritedEnclosee(Program.getRoot(), BracketExpression.class)) {
            ...
            System.err.println(
                    exp + ": " + tokens + "\n Postfix: " + ExpressionTokenizer.getPostfixTokens(exp) + "\n Prefix: " +
                            ExpressionTokenizer.getPrefixTokens(exp));
        }
        System.exit(0);
    }
        ...
        for (FunctionDefinition function : Program.getRoot().getInfo().getAllFunctionDefinitions()) {
            for (Node n : function.getInfo().getCFGInfo().getLexicalCFGContents()) {
                ...
                if (n instanceof ForStatement) {
                    ...
                    //					expStmt = FrontEnd.parseAndNormalize(i++ + ";", ExpressionStatement.class);
                    Statement stmt = FrontEnd.parseAndNormalize("continue;", Statement.class);
                    System.out.println(InsertImmediatePredecessor.insert(parCons.getInfo().getCFGInfo().getTerminationExpression(), stmt));
                    if (parCons.getInfo().getCFGInfo().hasTerminationExpression()) {
                        System.out.println(InsertImmediatePredecessor.insert(parCons.getInfo().getCFGInfo().getTerminationExpression(), expStmt));
                    }
    ...
    public static void testNodeReplacedString() {
        for (WhileStatement whileStmt : Misc.getInheritedEnclosee(Program.getRoot(), WhileStatement.class)) {
            System.out.println(whileStmt.getInfo().getNodeReplacedString(whileStmt.getInfo().getCFGInfo().getPredicate(), "33"));
        }
        System.exit(0);
    }
    ...
    private static void testNoNullParent() {
        for (Node n : Misc.getInheritedEnclosee(Program.getRoot(), Node.class)) {
            ...
            if (n.getParent() == null) {
                ...
                System.out.println(
                        "Following node has some issue: " + str + " of type " + n.getClass().getSimpleName());
            }
        }
    ...
    private static void testNormalization() {
        ...
        for (DoStatement doStmt : Misc.getInheritedEnclosee(Program.getRoot().getInfo().getMainFunction(), DoStatement.class)) {
    ...
    /**
     * This sample pass adds a {@code printf} statement that prints {@code true}, before all pointer-dereference writes
     * (i.e., before stores, such as {@code *p = ...;}.
     * <p>
     * NOTE: To use this method, paste it in the {@code Main} class, and invoke it after {@code
    ...
    public static void testPhaseInWhile() {
        ...
        for (WhileStatement itStmt : Misc.getInheritedEnclosee(main, WhileStatement.class)) {
            ...
            CompoundStatement copyStmt = FrontEnd.parseAlone("{if (1) {foo(); #pragma omp barrier\n} else {#pragma omp barrier\nfoo();}}", CompoundStatement.class);
    ...
    private static void testReachableUses() {
        FrontEnd.parseAndNormalize("int main() {int x; int y; x = 10; y = x; l1: x = y + 10; y = ++x + 11; x = x + 3; y = x; x = x + 1;}");
        FunctionDefinition main = Program.getRoot().getInfo().getMainFunction();
    ...
    private static void testReversePostOrder() {
        ...
        long timer = System.nanoTime();
        List<Node> reversePostOrder = TraversalOrderObtainer.obtainReversePostOrder(beginNode, n -> n.getInfo().getCFGInfo().getInterProceduralLeafSuccessors());
    ...
    private static void testSwapLength() {
        FrontEnd.parseAndNormalize("int main () {int A; int B; int C; int D; \n#pragma omp parallel\n{A=B;B=C;C=D;D=A;}}");
        FunctionDefinition funcDef = Program.getRoot().getInfo().getMainFunction();
        assert (funcDef != null);
    ...
    private static void testUnreachables() {
        FrontEnd.parseAndNormalize("int foo() {int p; p = 10; return p;} int main() {int x; x = 2; if (x > 3) {goto l2;}l1:while(x > 4){x = 11; foo();if(x>1){x = 10;goto l3;l2:;} x = 12; } l3: x = 14;}", TranslationUnit.class);
    ...
    public static void testZ3ExpressionCreator() {
        ...
        tokenList.addAll(Arrays.asList(OperatorToken.ASSIGN, OperatorToken.MINUS, OperatorToken.MUL, spe4, OperatorToken.PLUS, spe3, spe1, spe2, spe2));
