  ...
  *dy = 2.0 / (m-1);
  for (j=0; j<m; j++){
    for (i=0; i<n; i++){
      ...
      yy = -1.0 + *dy * (j-1);
      u[(j)*n+(i)] = 0.0;
      f[(j)*n+(i)] = -alpha * (1.0 - xx*xx) * (1.0 - yy*yy)
...
int main(int argc, char **argv){
    ...
    double dt, mflops;
    int NUMTHREADS;
    char *PARAM_NAMES[6] = {"Grid dimension: X dir =", "Grid dimension: Y dir =", "Helmhotlz constant =",
   ...
   OSCR_init (NUMTHREADS, "Jacobi Solver v1", "Use 'jacobi01' <n> <m> <alpha> <relax> <tol> <mits>", 6,
                PARAM_NAMES, DEFAULT_VALUES , 1, 1, TIMERS_NAMES,
                argc, argv);
    n = OSCR_getarg_int(1);
    m = OSCR_getarg_int(2);
