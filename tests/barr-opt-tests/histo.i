    ...
    for (y = 0; y < height; ++y)
    {
        for (x = 0; x < width; ++x)
        {
            ...
            if (value == 0){
                ...
                pixel_map[y*width+x] = HSVtoRGB(0.0,1.0,cbrt(1+ 63.0*((float)value)/((float)255))/4);
            }
        }
    }
...
int main(int argc, char* argv[]) {
  ...
  for (iter = 0; iter < numIterations; iter++){
    __builtin___memset_chk (histo, 0,histo_height*histo_width*sizeof(unsigned char), __builtin_object_size (histo, 0));
    unsigned int i;
