...
extern void POMP_Do_enter(struct ompregdescr* r);
extern void POMP_Do_exit(struct ompregdescr* r);
...
#pragma omp parallel shared(diff)
 {
  ...
  while(!done) {
   ...
   diff = 0;
#pragma omp barrier
   int mymin = (20/omp_get_num_threads())*omp_get_thread_num();
   int mymax = mymin + 20/omp_get_num_threads();
   int i, j;
   for (i = mymin; i < mymax; i++) {
    for(j = 0; j < 20; j++) {
     float temp = arr[i][j];
     arr[i][j] = 0.2 * (arr[i][j] + ((i+1)>=20?0:arr[i+1][j]) + ((j+1)>=20?0:arr[i][j+1]) + ((i-1)<0?0:arr[i-1][j]) + ((j-1)<0?0:arr[i][j-1]));
...
#pragma omp barrier
   if(((float)diff)/(20*20) < threshold || loop > 100000) {
    done = 1;
   }
#pragma omp barrier
  }
