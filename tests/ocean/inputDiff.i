...
#pragma omp parallel shared(diff)
    {
        ...
        while(!done) {
            ...
            diff = 0;
#pragma omp barrier
            int mymin = (1000/omp_get_num_threads())*omp_get_thread_num();
            int mymax = mymin + 1000/omp_get_num_threads();
            int i, j;
            for (i = mymin; i < mymax; i++) {
                for(j = 0; j < 1000; j++) {
                    float temp = arr[i][j];
                    arr[i][j] = 0.2 * (arr[i][j] + ((i+1)>=1000?0:arr[i+1][j]) + ((j+1)>=1000?0:arr[i][j+1]) + ((i-1)<0?0:arr[i-1][j]) + ((j-1)<0?0:arr[i][j-1]));
...
#pragma omp barrier
            if(((float)diff)/(1000*1000) < threshold || loop > 100000) {
                done = 1;
            }
#pragma omp barrier
        }
        ...
        fprintf(__stdoutp, "Time taken by thread %d, for %d loops is %ld microseconds, for diff %f.\n",omp_get_thread_num(), loop, 1000000*(tvStop.tv_sec-tvStart.tv_sec) + (tvStop.tv_usec-tvStart.tv_usec), diff/1000.0*1000.0);
    }
    for(p = 0; p < 1000; p++) {
        for(q = 0; q < 1000; q++) {
            fprintf(__stderrp, "%.2f\t", arr[p][q]);
