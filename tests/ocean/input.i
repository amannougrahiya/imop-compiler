...
#pragma omp barrier
        ...
        fprintf(__stdoutp, "Time taken by thread %d, for %d loops is %ld microseconds, with %lf.\n",omp_get_thread_num(), loop, 1000000*(tvStop.tv_sec-tvStart.tv_sec) + (tvStop.tv_usec-tvStart.tv_usec), diff);
    }
    for(p = 0; p < 1000; p++) {
        for(q = 0; q < 1000; q++) {
            fprintf(__stderrp, "%.2f\t", arr[p][q]);
