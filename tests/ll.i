...
#pragma omp barrier
        if (tid == 0) {
            ...
            while (curr != ((void *)0)) {
                size = 0;
                while (curr != ((void *)0) && size < CHUNK_SIZE) {
                    buf[size] = curr;
                    curr = curr->next;
                    size++;
                }
...
int main(int argc, char *argv[]) {
 ...
 size_t i;
    ...
    clock_gettime(_CLOCK_PROCESS_CPUTIME_ID, &t1);
    update_list(head);
    clock_gettime(_CLOCK_PROCESS_CPUTIME_ID, &t2);
