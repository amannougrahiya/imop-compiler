...
int main(int argc, char *argv[]) {
    double diff_imopVarPre12;
    double diff_imopVarPre6;
    double diff_imopVarPre3;
    double diff;
    double mean;
    double **u;
    double **w;
...
// #pragma omp dummyFlush BARRIER_START written() read([w, u, heapCell#2, heapCell#1, diff, i])
#pragma omp barrier
...
// #pragma omp dummyFlush BARRIER_START written() read([w, mean, heapCell#2, diff, heapCell#4, i])
#pragma omp barrier
#pragma omp for reduction(+:mean) nowait
        for (i = 1; i < 500 - 1; i++) {
            mean = mean + w[i][0] + w[i][500 - 1];
        }
// #pragma omp dummyFlush BARRIER_START written() read([w, mean, heapCell#2, diff, heapCell#4, j])
#pragma omp barrier
#pragma omp for reduction(+:mean) nowait
        for (j = 0; j < 500; j++) {
...
// #pragma omp dummyFlush BARRIER_START written() read([w, mean, heapCell#2, diff, i])
#pragma omp barrier
#pragma omp for nowait
        for (i = 1; i < 500 - 1; i++) {
            for (j = 1; j < 500 - 1; j++) {
...
// #pragma omp dummyFlush BARRIER_START written() read([printf, omp_get_wtime, w, u, diff, whilePred_imopVarPre0])
#pragma omp barrier
...
#pragma omp master
            {
                diff_imopVarPre3 = 0.0;
            }
            ;
#pragma omp master
            {
                diff_imopVarPre6 = 0.0;
            }
            ;
#pragma omp master
            {
                diff_imopVarPre12 = 0.0;
            }
#pragma omp master
            {
                diff = 0.0;
...
// #pragma omp dummyFlush BARRIER_START written([]) read([printf, omp_get_wtime, w, fabs, u, heapCell#2, heapCell#3, heapCell#1, diff, heapCell#4, i, nullCell])
#pragma omp barrier
            while (1) {
                my_diff = 0.0;
#pragma omp master
...
// #pragma omp dummyFlush CRITICAL_START written([]) read([diff_imopVarPre3])
#pragma omp critical
                {
                    if (diff_imopVarPre3 < my_diff) {
                        diff_imopVarPre3 = my_diff;
                    }
                }
// #pragma omp dummyFlush CRITICAL_END written([diff_imopVarPre3]) read([])
                my_diff = 0.0;
// #pragma omp dummyFlush BARRIER_START written([]) read([printf, fabs, w, diff_imopVarPre3, u, heapCell#2, heapCell#3, heapCell#1, heapCell#4, i, nullCell])
#pragma omp barrier
#pragma omp master
                {
                    iterations++;
                    if (iterations == iterations_print) {
                        printf("  %8d  %f\n", iterations, diff_imopVarPre3);
                        iterations_print = 2 * iterations_print;
                    }
                }
                whilePred_imopVarPre0 = epsilon <= diff_imopVarPre3;
#pragma omp for nowait
                for (i = 1; i < 500 - 1; i++) {
                    for (j = 1; j < 500 - 1; j++) {
                        u[i][j] = (w[i - 1][j] + w[i + 1][j] + w[i][j - 1] + w[i][j + 1]) / 4.0;
...
// #pragma omp dummyFlush CRITICAL_START written([heapCell#3, diff, heapCell#4, nullCell]) read([diff_imopVarPre6])
#pragma omp critical
                {
                    if (diff_imopVarPre6 < my_diff) {
                        diff_imopVarPre6 = my_diff;
                    }
                }
// #pragma omp dummyFlush CRITICAL_END written([diff_imopVarPre6]) read([])
                my_diff = 0.0;
// #pragma omp dummyFlush BARRIER_START written([]) read([printf, w, fabs, u, heapCell#2, heapCell#3, heapCell#1, heapCell#4, diff_imopVarPre6, i])
#pragma omp barrier
#pragma omp master
                {
                    diff_imopVarPre3 = 0.0;
...
#pragma omp master
                {
                    iterations++;
                    if (iterations == iterations_print) {
                        printf("  %8d  %f\n", iterations, diff_imopVarPre6);
                        iterations_print = 2 * iterations_print;
                    }
                }
                whilePred_imopVarPre0 = epsilon <= diff_imopVarPre6;
#pragma omp for nowait
                for (i = 1; i < 500 - 1; i++) {
                    for (j = 1; j < 500 - 1; j++) {
                        w[i][j] = (u[i - 1][j] + u[i + 1][j] + u[i][j - 1] + u[i][j + 1]) / 4.0;
...
#pragma omp single nowait
                    {
                        diff = diff_imopVarPre6;
                    }
// #pragma omp dummyFlush BARRIER_START written([]) read([printf, omp_get_wtime, diff])
#pragma omp barrier
                    break;
                }
#pragma omp for nowait
...
// #pragma omp dummyFlush CRITICAL_START written([diff_imopVarPre3, heapCell#4]) read([diff_imopVarPre12])
#pragma omp critical
                {
                    if (diff_imopVarPre12 < my_diff) {
                        diff_imopVarPre12 = my_diff;
                    }
                }
// #pragma omp dummyFlush CRITICAL_END written([diff_imopVarPre12]) read([])
                my_diff = 0.0;
// #pragma omp dummyFlush BARRIER_START written([]) read([diff_imopVarPre12, printf, w, u, heapCell#2, heapCell#1, heapCell#4, i])
#pragma omp barrier
#pragma omp master
                {
                    diff_imopVarPre6 = 0.0;
...
#pragma omp master
                {
                    iterations++;
                    if (iterations == iterations_print) {
                        printf("  %8d  %f\n", iterations, diff_imopVarPre12);
                        iterations_print = 2 * iterations_print;
                    }
                }
                whilePred_imopVarPre0 = epsilon <= diff_imopVarPre12;
#pragma omp for nowait
                for (i = 1; i < 500 - 1; i++) {
                    for (j = 1; j < 500 - 1; j++) {
                        u[i][j] = (w[i - 1][j] + w[i + 1][j] + w[i][j - 1] + w[i][j + 1]) / 4.0;
...
#pragma omp single nowait
                    {
                        diff = diff_imopVarPre12;
                    }
// #pragma omp dummyFlush BARRIER_START written([]) read([printf, omp_get_wtime, diff])
#pragma omp barrier
                    break;
                }
// #pragma omp dummyFlush BARRIER_START written([heapCell#3, diff_imopVarPre6]) read([fabs, w, u, heapCell#3, heapCell#2, heapCell#1, heapCell#4, i])
#pragma omp barrier
#pragma omp master
                {
                    diff_imopVarPre12 = 0.0;
...
// #pragma omp dummyFlush CRITICAL_START written([diff_imopVarPre12]) read([diff])
#pragma omp critical
                {
                    if (diff < my_diff) {
                        diff = my_diff;
