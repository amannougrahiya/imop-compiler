...
#pragma omp master
        ...
        // #pragma omp dummyFlush BARRIER_START written([nullCell, dx, dy, time_buffer, time_buffer.f]) read()
#pragma omp barrier
        int _imopVarPre147;
#pragma omp master
        {
...
#pragma omp barrier
        ...
        // #pragma omp dummyFlush BARRIER_START written() read([unew, dx, udiff, f.f, nx, rhs, u_exact, unew_norm, _imopVarPre150, u_norm, error, uexact.f, r8mat_rms, sin, unew.f, omp_get_wtime, uxxyy_exact, dy, udiff.f, itnew, _imopVarPre162, sqrt, f, ny, printf, uexact, itold])
#pragma omp barrier
#pragma omp master
        {
            printf("\n");
            ...
            for (j = 0; j < ny; j++) {
                for (i = 0; i < nx; i++) {
                    v = v + unew[i][j] * unew[i][j];
                }
            }
            double _imopVarPre160;
            double _imopVarPre161;
            _imopVarPre160 = v / (double) (nx * ny);
            _imopVarPre161 = sqrt(_imopVarPre160);
            unew_norm = _imopVarPre161;
            for (j = 0; j < ny; j++) {
...
#pragma omp master
            ...
            // #pragma omp dummyFlush BARRIER_START written([unew.f, diff, udiff.f, itnew, i, j, unew_norm, u_norm, error, uexact.f, converged, y, x, wtime, itold]) read()
#pragma omp barrier
            int i;
            int it;
            int j;
...
#pragma omp for nowait
                ...
                // #pragma omp dummyFlush BARRIER_START written([unew.f, itnew, udiff.f, i, j, unew_norm, u_norm, error, uexact.f, converged, y, x, wtime, u.f]) read()
#pragma omp barrier
#pragma omp for nowait
                for (j = 0; j < ny; j++) {
                    for (i = 0; i < nx; i++) {
                        ...
                        if (!_imopVarPre154) {
                            ...
                            if (!_imopVarPre155) {
                                ...
                                if (!_imopVarPre156) {
                                    _imopVarPre156 = j == ny - 1;
                                }
                                _imopVarPre155 = _imopVarPre156;
                            }
                            _imopVarPre154 = _imopVarPre155;
                        }
                        if (_imopVarPre154) {
                            unew[i][j] = f[i][j];
                        } else {
...
#pragma omp barrier
            ...
            // #pragma omp dummyFlush BARRIER_START written([unew.f, itnew, udiff.f, i, j, unew_norm, u_norm, error, uexact.f, converged, y, x, wtime]) read([nullCell, unew, time, u, udiff, diff, f.f, nx, strftime, unew_norm, localtime, error, uexact.f, time_buffer, u.f, r8mat_rms, unew.f, omp_get_wtime, udiff.f, itnew, sqrt, ny, timestamp, printf, uexact, converged, time_buffer.f, itold])
#pragma omp barrier
#pragma omp master
            {
                u_norm = unew_norm;
...
#pragma omp barrier
    ...
    _imopVarPre158 = omp_get_wtime();
    wtime = _imopVarPre158 - wtime;
    printf("\n");
        ...
        for (i = 0; i < nx; i++) {
            ...
            if (!_imopVarPre162) {
                ...
                if (!_imopVarPre163) {
                    _imopVarPre164 = j == 0;
                    if (!_imopVarPre164) {
                        _imopVarPre164 = j == ny - 1;
                    }
                    _imopVarPre163 = _imopVarPre164;
                }
                _imopVarPre162 = _imopVarPre163;
            }
            if (_imopVarPre162) {
                double _imopVarPre165;
                _imopVarPre165 = u_exact(x, y);
                f[i][j] = _imopVarPre165;
            } else {
                double _imopVarPre167;
                _imopVarPre167 = uxxyy_exact(x, y);
                f[i][j] = -_imopVarPre167;
            }
        }
    }
